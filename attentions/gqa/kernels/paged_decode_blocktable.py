...
#
# Paged GQA decode attention with block table indexing.
# Uses 2D block table: [num_seqs, max_num_blocks_per_seq] for O(1) lookup.
#
# Key features:
# - 2D kernel: Standard decode with parallel KV head processing
# - 3D kernel: Parallel softmax segments for very long sequences
# - Sliding window attention with tile-level pruning
# - ALiBi position encoding support
# - Softcap for attention scores (Gemma models)
# - FP8 quantization support for KV cache
# - Multimodal prefix ranges for bidirectional attention
#
# Authors:
...
def find_seq_idx(
    ...
    """Binary search to find sequence index from global query block index."""
    left: tl.int32 = 0
    right = num_seqs
    while left < right:
        mid = (left + right) // 2
...
def kernel_unified_attention_2d(
    ...
    """
    2D paged attention kernel for decode phase.

    Grid: (num_q_blocks, num_kv_heads)

    Uses block table for paged KV cache access:
    - block_tables_ptr[seq_idx, logical_block] -> physical_block_id
    - KV cache indexed by physical block
    """
...
def kernel_unified_attention_3d(
    ...
    For very long sequences, splits KV into segments processed in parallel.
    Each segment stores partial output + log-sum-exp for later reduction.
    """
    q_block_global_idx = tl.program_id(0)
    kv_head_idx = tl.program_id(1)
...
def reduce_segments(
    ...
    """
    Reduction kernel for 3D attention: combines partial segment outputs.

    Grid: (num_tokens, num_query_heads)

    Uses log-sum-exp to merge partial attention outputs from different segments.
    """
...
def unified_attention_blocktable(
    q,
    k,
    v,
    out,
    ...
    """
    Unified paged attention with block table indexing.
    ...
    Args:
        ...
        cu_seqlens_q: Cumulative sequence lengths [num_seqs + 1]
        ...
        window_size: Sliding window size tuple (left, right), -1 means no limit
        block_table: Block table [num_seqs, max_num_blocks_per_seq]
        softcap: Softcap value for attention scores (Gemma models)
        k_descale: Optional K cache FP8 scale
        v_descale: Optional V cache FP8 scale
        seq_threshold_3D: Threshold for using 3D kernel
        num_par_softmax_segments: Number of parallel softmax segments
        softmax_segm_output: Buffer for segment outputs
        softmax_segm_max: Buffer for segment max values
        softmax_segm_expsum: Buffer for segment exp sums
        alibi_slopes: Optional ALiBi slopes
        output_scale: Optional FP8 output scale
        qq_bias: Optional query-query bias
        sinks: Optional attention sinks
        mm_prefix_range: Optional multimodal prefix ranges
        use_alibi_sqrt: Whether to use sqrt ALiBi
    """
    ...
    # Upper bound for number of Q blocks
    ...
    # 1. No intermediate tiled softmax buffers for the 3D kernel have been allocated
    # 2. The batch includes at least one prefill request
