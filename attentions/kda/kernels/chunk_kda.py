...
from ops_utils import chunk_local_cumsum, prepare_chunk_indices, RCP_LN2

# Import from local GLA common modules
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "gla" / "kernels"))
from common.chunk_delta_h import chunk_gated_delta_rule_bwd_dhu, chunk_gated_delta_rule_fwd_h

# Import from local GLA chunk (for chunk_gla_fwd_o_gk) - use importlib to avoid name conflict
_gla_chunk_path = Path(__file__).parent.parent.parent / "gla" / "kernels" / "chunk_gla.py"
_gla_chunk_spec = importlib.util.spec_from_file_location("gla_chunk", _gla_chunk_path)
_gla_chunk = importlib.util.module_from_spec(_gla_chunk_spec)
_gla_chunk_spec.loader.exec_module(_gla_chunk)
chunk_gla_fwd_o_gk = _gla_chunk.chunk_gla_fwd_o_gk

# Add own directory to path for local imports
sys.path.insert(0, str(Path(__file__).parent))

# Import from local KDA kernel files
from chunk_bwd import chunk_kda_bwd_dAv, chunk_kda_bwd_wy_dqkg_fused
from chunk_intra import chunk_kda_bwd_intra, chunk_kda_fwd_intra
from gate import kda_gate_bwd, kda_gate_chunk_cumsum
