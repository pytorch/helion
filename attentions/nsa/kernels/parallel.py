...
# Use standalone utilities instead of fla package
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "common"))
sys.path.insert(0, str(Path(__file__).parent))
from fla_utils import autocast_custom_bwd, autocast_custom_fwd, contiguous
...
# Local SWA implementation instead of flash_attn
sys.path.insert(0, str(Path(__file__).parent.parent.parent / "swa" / "kernels"))
from flash_attention import FlashAttention


class FlashAttnGQAWrapper(torch.autograd.Function):
    """
    Wrapper for FlashAttention that handles GQA (grouped query attention)
    and tensor layout conversion.
    """

    @staticmethod
    def forward(ctx, q, k, v, window_size, attn_mode):
        """
        Args:
            q: (B, T, H_q, D) - queries
            k: (B, T, H_kv, D) - keys
            v: (B, T, H_kv, D_v) - values
            window_size: int or None - window size for causal_sliding_window
            attn_mode: str - "causal", "global", or "causal_sliding_window"
        Returns:
            o: (B, T, H_q, D_v) - output
        """
        B, T, H_q, D = q.shape
        H_kv = k.shape[2]
        D_v = v.shape[-1]
        ...
        # Store for backward
        repeat_factor = H_q // H_kv if H_q != H_kv else 1
        ctx.repeat_factor = repeat_factor
        ctx.H_kv = H_kv
        ctx.attn_mode = attn_mode
        ctx.window_size = window_size

        # Save original k and v before expansion (for backward)
        k_orig = k
        v_orig = v

        # Handle GQA by repeating k and v heads
        if repeat_factor > 1:
            k = k.repeat_interleave(repeat_factor, dim=2)
            v = v.repeat_interleave(repeat_factor, dim=2)

        # Convert from (B, T, H, D) to (B, H, T, D) for FlashAttention
        # Use contiguous to ensure proper memory layout
        q_t = q.transpose(1, 2).contiguous()
        k_t = k.transpose(1, 2).contiguous()
        v_t = v.transpose(1, 2).contiguous()

        # Run FlashAttention forward
        o_t = FlashAttention.apply(q_t, k_t, v_t, window_size, attn_mode)

        # Save original (unexpanded) k and v for backward
        ctx.save_for_backward(q, k_orig, v_orig, o_t)

        # Convert back from (B, H, T, D) to (B, T, H, D)
        return o_t.transpose(1, 2).contiguous()

    @staticmethod
    def backward(ctx, do):
        q, k, v, o_t = ctx.saved_tensors
        repeat_factor = ctx.repeat_factor
        H_kv = ctx.H_kv
        attn_mode = ctx.attn_mode
        window_size = ctx.window_size

        B, T, H_q, D = q.shape
        D_v = v.shape[-1]

        # Handle GQA by repeating k and v heads (same as forward)
        if repeat_factor > 1:
            k = k.repeat_interleave(repeat_factor, dim=2)
            v = v.repeat_interleave(repeat_factor, dim=2)

        # Convert tensors to (B, H, T, D) format
        q_t = q.transpose(1, 2).contiguous()
        k_t = k.transpose(1, 2).contiguous()
        v_t = v.transpose(1, 2).contiguous()

        # Convert do to (B, H, T, D) format
        do_t = do.transpose(1, 2).contiguous()

        # Re-run forward to get L (logsumexp) - needed for backward
        # This is inefficient but necessary since FlashAttention.backward needs L
        from forward import _attn_fwd
        import triton

        SEQ_LEN, HEAD_DIM = T, D
        softmax_factor = 1 / HEAD_DIM**0.5
        O_t = torch.empty_like(q_t)
        L = torch.empty((B, H_q, SEQ_LEN), device=q.device, dtype=torch.float32)
        ...
        _attn_fwd[grid](
            Q=q_t, K=k_t, V=v_t, softmax_factor=softmax_factor, L=L, O=O_t,
            stride_batch=q_t.stride(0), stride_head=q_t.stride(1),
            stride_seq=q_t.stride(2), stride_dim=q_t.stride(3),
            BATCH_SIZE=B, NUM_HEADS=H_q, SEQ_LEN=SEQ_LEN, HEAD_DIM=HEAD_DIM,
        ...
        Dk = torch.empty((B, H_q, SEQ_LEN), device=q.device, dtype=torch.float32)
        ...
        _attn_bwd_preprocess[preprocess_grid](
            O=O_t, dO=do_t, D=Dk, SEQ_LEN=SEQ_LEN, HEAD_DIM=HEAD_DIM,
        )

        bwd_grid = lambda args: (
            triton.cdiv(SEQ_LEN, args["BLOCK_SIZE_KV"]),
            B * H_q,
            1,
        )

        _attn_bwd_dk_dv[bwd_grid](
            Q=q_t, K=k_t, V=v_t, softmax_factor=softmax_factor,
            dO=do_t, dQ=dQ, dK=dK, dV=dV, L=L, D=Dk,
            stride_batch=q_t.stride(0), stride_head=q_t.stride(1),
            stride_seq=q_t.stride(2), stride_dim=q_t.stride(3),
            NUM_HEADS=H_q, SEQ_LEN=SEQ_LEN, HEAD_DIM=HEAD_DIM,
            WINDOW_SIZE=window_size, MODE=mode,
        )

        _attn_bwd_dq[bwd_grid](
            Q=q_t, K=k_t, V=v_t, softmax_factor=softmax_factor,
            dO=do_t, dQ=dQ, dK=dK, dV=dV, L=L, D=Dk,
            stride_batch=q_t.stride(0), stride_head=q_t.stride(1),
            stride_seq=q_t.stride(2), stride_dim=q_t.stride(3),
        ...
        # dQ, dK, dV are (B, H_q, T, D)
        # Convert gradients back to (B, T, H, D)
        dq = dQ.transpose(1, 2).contiguous()  # (B, T, H_q, D)
        dk_full = dK.transpose(1, 2).contiguous()  # (B, T, H_q, D)
        dv_full = dV.transpose(1, 2).contiguous()  # (B, T, H_q, D_v)

        # Handle GQA gradient aggregation for k and v
        if repeat_factor > 1:
            # dk_full has shape (B, T, H_q, D) where H_q = H_kv * repeat_factor
            # Need to sum over repeated heads: (B, T, H_kv, repeat_factor, D) -> (B, T, H_kv, D)
            D_k = dk_full.shape[-1]
            D_v_actual = dv_full.shape[-1]
            dk = dk_full.view(B, T, H_kv, repeat_factor, D_k).sum(dim=3)
            dv = dv_full.view(B, T, H_kv, repeat_factor, D_v_actual).sum(dim=3)
        else:
            dk = dk_full
            dv = dv_full
...
def flash_attn_func_local(q, k, v, causal=False, window_size=None):
    """
    Local implementation matching flash_attn_func API for causal sliding window attention.

    Args:
        q, k, v: (B, T, H, D) tensors. Supports GQA where H_q can be larger than H_kv
        causal: bool - whether to use causal masking
        window_size: tuple (left, right) or None - sliding window bounds
                     For causal sliding window: window_size[0] is how far back to look

    Returns:
        o: (B, T, H_q, D)
    """
    if window_size is not None and window_size[0] > 0:
        # Causal sliding window: query at position q attends to keys in [max(0, q - window_size), q]
        # window_size[0] is the left window size (how far back to look)
        # The total window size is window_size[0] + 1 (including the current position)
        window_total = window_size[0] + 1
        return FlashAttnGQAWrapper.apply(q, k, v, window_total, "causal_sliding_window")
    elif causal:
        return FlashAttnGQAWrapper.apply(q, k, v, None, "causal")
    else:
        return FlashAttnGQAWrapper.apply(q, k, v, None, "global")


def flash_attn_varlen_func_local(q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k,
                                  causal=False, window_size=None):
    """
    Local implementation matching flash_attn_varlen_func API for variable-length sequences.

    Args:
        q, k, v: (total_tokens, H, D) tensors (packed sequences). Supports GQA.
        cu_seqlens_q, cu_seqlens_k: (N+1,) cumulative sequence lengths
        max_seqlen_q, max_seqlen_k: max sequence lengths
        causal: bool
        window_size: tuple (left, right) or None

    Returns:
        o: (total_tokens, H_q, D)
    """
    total_tokens, H_q, D = q.shape
    num_seqs = len(cu_seqlens_q) - 1

    # Process each sequence separately and concatenate
    outputs = []
    for i in range(num_seqs):
        start = cu_seqlens_q[i].item()
        end = cu_seqlens_q[i + 1].item()

        # Extract sequence and add batch dimension
        q_seq = q[start:end].unsqueeze(0)  # (1, seq_len, H_q, D)
        k_seq = k[start:end].unsqueeze(0)
        v_seq = v[start:end].unsqueeze(0)

        # Use the regular flash_attn_func_local (handles GQA internally)
        o_seq = flash_attn_func_local(q_seq, k_seq, v_seq, causal=causal, window_size=window_size)
        outputs.append(o_seq.squeeze(0))  # (seq_len, H_q, D)
...
def parallel_nsa(
    ...
    if window_size > 0:
        if cu_seqlens is not None:
            ...
            o_swa = flash_attn_varlen_func_local(
...
def parallel_nsa_with_compression(
    ...
    r"""
    Wrapper for parallel_nsa with compression that returns (output, topk_indices).
    Matches the API of native_sparse_attention.ops.parallel.parallel_nsa_with_compression.
    ...
    Args:
        ...
        v (torch.Tensor):
            Values of shape `[B, T, H, V]` if `head_first=False` else `[B, H, T, V]`.
        g_cmp (torch.Tensor):
            Gate score for compressed attention.
        g_slc (torch.Tensor):
            Gate score for selected attention.
        g_swa (torch.Tensor):
            Gate score for sliding window attention.
        block_counts (Union[torch.LongTensor, int]):
    ...
    Returns:
        Tuple[torch.Tensor, torch.Tensor]:
            - o: Output tensor
            - topk: Top-k block indices
    """
    if scale is None:
        scale = k.shape[-1] ** -0.5
    if head_first:
        q_for_topk, k_for_topk = map(lambda x: rearrange(x, 'b h t d -> b t h d'), (q, k))
    else:
        q_for_topk, k_for_topk = q, k

    # Get compressed k for topk computation
    k_cmp = mean_pooling(k_for_topk, block_size, cu_seqlens)

    # Compute compression output and lse for topk
    o_cmp, lse_cmp = parallel_nsa_compression(
        q=q_for_topk,
        k=k_cmp,
        v=mean_pooling(k_for_topk, block_size, cu_seqlens),  # dummy v, just need lse
        block_size=block_size,
        scale=scale,
        offsets=cu_seqlens
    )
    ...
    # Get topk indices
    topk = parallel_nsa_topk(
        q=q_for_topk,
        k=k_cmp,
        lse=lse_cmp,
        block_counts=block_counts if isinstance(block_counts, int) else block_counts.max().item(),
        block_size=block_size,
        scale=scale,
        offsets=cu_seqlens
    )

    # Call parallel_nsa with g_cmp to enable compression path
    o = parallel_nsa(
        q=q,
        k=k,
        v=v,
