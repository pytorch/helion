...
.. autoattribute:: Settings.autotune_pool_rebenchmark_mode

   Select the pool-mode rebenchmark strategy. ``"grouped"`` (default) runs
   one grouped worker job for the candidates being rebenchmarked.
   ``"owner_isolated"`` runs one worker job per candidate on the worker that
   precompiled that candidate, using the same repeat count as grouped
   rebenchmarking. Controlled by
   ``HELION_AUTOTUNE_POOL_REBENCHMARK_MODE``.
...
| ``HELION_AUTOTUNE_POOL_REBENCHMARK_MODE`` | ``autotune_pool_rebenchmark_mode`` | Select pool rebenchmarking mode: ``"grouped"`` or ``"owner_isolated"``. |
