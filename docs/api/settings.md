...
.. autoattribute:: Settings.autotune_log
   ...
   When set, Helion writes per-config telemetry (run id, timestamp, config id, generation, status, perf, compile time, config) to ``<value>.csv`` and mirrors the autotune log to ``<value>.log`` (for population-based autotuners: ``PatternSearch``, ``DifferentialEvolution``). Both files append data, so runs that share one base path accumulate.
   CSV rows join to ``.meta.jsonl`` through two hashes. The ``run_id`` identifies the invocation. It is a hash of the kernel source, shapes, dtypes, device name, and code generation settings. The ``config_id`` identifies the config. The code generation settings in ``run_id`` are backend, dot_precision, fast_math, static_shapes, index_dtype, allow_warp_specialize, triton_do_not_specialize, pallas_interpret, debug_dtype_asserts, and persistent_reserved_sms. The JSONL record contains all settings. Started, completed, failed, and repeated rows use the same ``config_id`` for the same config. Each CSV row contains the full config. When ``autotune_log_details`` is enabled, the JSONL config map contains successful configs that have generated source. Controlled by HELION_AUTOTUNE_LOG.



.. autoattribute:: Settings.autotune_log_details

   Set ``HELION_AUTOTUNE_LOG_DETAILS=1`` and ``autotune_log`` to enable the cost-model sidecar. Helion appends one JSON record per run to ``<autotune_log>.meta.jsonl``. Each record contains the kernel identity, the full ``helion.settings``, an ``ir_graph``, and a ``configs`` map. The kernel identity contains ``run_id``, name, source, shapes, dtypes, and ``hardware_info``. Helion uses ``json.dumps(default=str)`` to serialize settings such as ``torch.dtype`` values and callables.

   Each config entry contains ``config``, ``generated_code``, ``source_hash``, and ``perf_stats``. The ``perf_stats`` value is an ordered list. It has one record for each argument set. A normal single-shape run has one record. Each record contains ``min``, ``median``, ``mean``, ``p90``, ``std``, and ``n_samples``. Helion continues to use the median to rank configs.

   Helion records a config only after the benchmark succeeds and the generated artifact is readable. A repeated successful measurement replaces the previous ``perf_stats`` list. Missing statistics, an invalid benchmark return type, and a missing or unreadable artifact raise an error. The default backend records Triton source. The CuTe, Pallas, and Metal backends record their own source.

   ``hardware_info`` is a structured snapshot collected lazily when the record is written. Hardware identity, required package versions, and declared accelerator properties must all be available. If any read fails, the record is not written and the error propagates. The snapshot covers cuda/rocm/xpu/tpu/cpu and is descriptive only, so it is excluded from ``run_id``. Fields:

   - ``device_kind``, ``device_name``, ``compute_capability``;
   - host ``cpu_num_threads``;
   - ``device_props``: an accelerator-only nested block (present on cuda/rocm/xpu, omitted on tpu/cpu) of required raw ``get_device_properties`` values keyed by the backend's torch attribute names (cuda: ``multi_processor_count``, ``max_threads_per_multi_processor``, ``max_threads_per_block``, ``warp_size``, ``shared_memory_per_block``, ``shared_memory_per_block_optin``, ``regs_per_multiprocessor``, ``total_memory``, ``L2_cache_size``; rocm: the same fields except the CUDA-only ``shared_memory_per_block_optin``; xpu: ``max_compute_units``, ``max_work_group_size``, ``local_mem_size``, ``total_memory``);
   - ``versions``: a backend-specific block — always ``torch`` and ``helion``, plus a toolkit-version key named for the toolkit (``cuda``/``hip``/``xpu``) and the backend's toolchain packages (``triton`` on cuda/rocm/xpu; ``jax``/``jaxlib``/``libtpu`` on tpu). CPU records carry only ``torch``/``helion``.
   ...
   Recover a successful sample by joining its CSV row to ``meta[run_id]["configs"][row["config_id"]]``. The entry contains the config, generated source, source hash, and per-shape statistics. ``run_id`` can recur across reruns, processes, and ``autotune_best_of_k``. Config maps with the same ``run_id`` are safe to combine because one ``config_id`` always identifies the same config. Searches restricted to user-pinned ``configs`` are excluded unless ``force_autotune`` is set. The CSV and log files are still written. If ``autotune_log`` is not set, this option collects no data and emits one warning.
