...
.. autoattribute:: Settings.autotune_log_details
   ...
   Each config entry contains ``config``, ``generated_code``, and ``source_hash``. Helion records an entry only after the config succeeds and its generated artifact is readable. A missing or unreadable artifact for a successful config raises an error. The default backend records Triton source. The CuTe, Pallas, and Metal backends record their own source.
   ...
   Recover a successful ``(config, perf)`` sample by joining its CSV row to ``meta[run_id]["configs"][row["config_id"]]``. The entry contains the config, generated source, and source hash. ``run_id`` can recur across reruns, processes, and ``autotune_best_of_k``. Config maps with the same ``run_id`` are safe to combine because one ``config_id`` always identifies the same config. Searches restricted to user-pinned ``configs`` are excluded unless ``force_autotune`` is set. The CSV and log files are still written. If ``autotune_log`` is not set, this option collects no data and emits one warning.
