...
.. autoattribute:: Settings.autotune_log_details
   ...
   - ``device_props``: a GPU-only nested block (present on cuda/rocm/xpu, omitted on tpu/cpu) of raw ``get_device_properties`` values keyed by the backend's torch attribute names (cuda/rocm: ``multi_processor_count``, ``max_threads_per_multi_processor``, ``max_threads_per_block``, ``warp_size``, ``shared_memory_per_block``, ``shared_memory_per_block_optin``, ``regs_per_multiprocessor``, ``total_memory``, ``L2_cache_size``; xpu: ``max_compute_units``, ``max_work_group_size``, ``local_mem_size``, ``total_memory``), per-field ``null`` on a probe miss;
