...
.. autoattribute:: Settings.autotune_log_details
   ...
   Opt-in to the cost-model dataset sidecar. When enabled (``HELION_AUTOTUNE_LOG_DETAILS=1``) and ``autotune_log`` is set, Helion appends one JSON record per run to ``<autotune_log>.meta.jsonl``: the kernel identity (``run_id``, name, source, shapes, dtypes, hardware, ``hardware_info``), the full ``helion.settings`` (JSON-safe via ``json.dumps(default=str)``, so ``torch.dtype``/callables become strings), an ``ir_graph`` (see below), and a ``configs`` map from ``config_id`` to an entry holding the tested ``config`` and the per-config ``generated_code`` (the full generated kernel source, captured because it differs per config; this can be large for big search spaces). The source is backend-specific — Triton on the default backend, but CuTe/Pallas/Metal on those backends — so use the record's ``settings.backend`` to interpret it.
