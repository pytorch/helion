...
.. autoattribute:: Settings.autotune_search_acc
   ...
   List of PTXAS config file paths to search during autotuning. Empty list (default) disables the feature. An empty string entry represents not passing a config. Controlled by ``HELION_AUTOTUNE_SEARCH_ACC`` (comma-separated paths).
...
| ``HELION_AUTOTUNE_SEARCH_ACC`` | ``autotune_search_acc`` | Comma-separated list of PTXAS config file paths to search during autotuning. |
