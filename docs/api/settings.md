...
.. autoattribute:: Settings.autotune_log_details
   ...
   - ``device_props``: a GPU-only nested block (present on cuda/rocm/xpu, omitted on tpu/cpu) of ``get_device_properties`` capacity/topology — ``sm_count``, ``max_threads_per_sm``, ``max_threads_per_block``, ``warp_size``, ``shared_mem_per_block``, ``regs_per_multiprocessor``, ``total_mem``, ``l2_cache_size`` (per-field ``null`` on a probe miss);
