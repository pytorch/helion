...
.. autoattribute:: Settings.autotune_log_details
   ...
   Opt-in to the cost-model dataset sidecar. When enabled (``HELION_AUTOTUNE_LOG_DETAILS=1``) and ``autotune_log`` is set, Helion appends one JSON record per run to ``<autotune_log>.meta.jsonl``: the kernel identity (``run_id``, name, source, shapes, dtypes, hardware, ``hardware_info``), the full ``helion.settings`` (JSON-safe via ``json.dumps(default=str)``, so ``torch.dtype``/callables become strings), an ``ir_graph`` (see below), and a ``configs`` map from ``config_id`` to an entry holding the tested ``config``, the per-config ``generated_code`` (the full generated kernel source, captured because it differs per config; this can be large for big search spaces), and ``perf_stats``. The source is backend-specific — Triton on the default backend, but CuTe/Pallas/Metal on those backends — so use the record's ``settings.backend`` to interpret it.
   ``perf_stats`` is ``{min, median, mean, p90, std, n_samples}`` (latency in ms; ``p90`` is the numpy type-7 percentile) — a richer description than the scalar the autotuner selects on. It holds the last *successful* benchmark of the config (a later failed re-benchmark does not overwrite it), and is all-``null`` with ``n_samples: 0`` for a config that never benchmarked. It is collected only on this opt-in path (benchmarking otherwise returns just the median), and is descriptive — not an input to selection or ``run_id``.
   ``hardware_info`` is a best-effort structured snapshot, collected lazily when the record is written. It covers cuda/rocm/xpu/tpu/cpu; missing fields are ``null`` (and the whole object is ``null`` only if collection unexpectedly fails); it is descriptive only and excluded from ``run_id``. Fields:

   - stable ``device_id`` (``kind:name:compute_capability``, device name sanitized so the split is unambiguous), plus ``device_kind``, ``device_name``, ``compute_capability``;
   - GPU capacity: ``sm_count``, thread/warp limits, ``shared_mem_per_block``, ``regs_per_multiprocessor``, ``total_mem``, ``l2_cache_size``;
   - host ``cpu_num_threads``;
   - ``versions``: ``torch``/``triton``/``helion`` always, plus exactly one backend key named by ``device_kind`` (``cuda``/``hip``/``xpu``/``xla``, value may be ``null``) — none on ``cpu``.
   ...
   Recover a measured ``(config, perf)`` sample by joining a CSV row to its record: ``meta[run_id]["configs"][row["config_id"]]["config"]`` (the generated source is alongside it at ``["generated_code"]``). ``run_id`` may recur (re-runs, processes, ``autotune_best_of_k``), but the ``configs`` maps are union-safe (same ``config_id`` implies the same config), so de-duplicating on ``run_id`` is lossless. Searches restricted to user-pinned ``configs`` (without ``force_autotune``) are excluded as a biased slice (``.csv``/``.log`` still written); setting this without ``autotune_log`` collects nothing and warns once.
