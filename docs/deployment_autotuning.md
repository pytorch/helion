...
A hand-maintained `autotune_foo` may return `None` to abstain for one runtime
key. In evaluate mode, Helion caches that decision for the key and uses the
compiler default configuration for that call without reporting a missing
heuristic; other keys can still select configurations from the same artifact.
Static standalone compilation skips an abstaining call because there is no
selected configuration to compile. Dynamic standalone compilation cannot embed
the compiler-default fallback for an arbitrary future key, so kernels whose
heuristics may abstain must set ``standalone=False``. If an observed dynamic
compile call abstains despite standalone output being enabled, compilation
fails with a clear error instead of emitting an unsafe dispatcher. Missing,
invalid, or failing heuristic artifacts retain the existing warning and
fallback behavior.
