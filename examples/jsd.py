...
def jsd_forward(
    ...
    for tile_bt in hl.tile(BT, block_size=BT_SIZE):
        ...
        for tile_v in hl.tile(V):
            ...
            if beta == 0.0: # Forward KL: KL(P || Q)
                Y_shift = Y - Y_max
                Y_prob = torch.exp(Y_shift) * torch.exp(Y_max)  # Compensate for the shift
                loss[tile_bt, tile_v] = Y_prob * (Y - X)
                dX[tile_bt, tile_v] = -Y_prob
            elif beta == 1.0: # Reverse KL: KL(Q || P)
                X_shift = X - X_max
                X_prob = torch.exp(X_shift) * torch.exp(X_max)  # Compensate for the shift
                ...
                log_M = torch.log(M)  # No need to compensate as M is already in original scale
    ...
    final_loss = torch.sum(loss) # This line raises a warning: helion.exc.TensorOperationInWrapper
...
class HelionJSD(nn.Module):
    ...
    def __init__(self, beta: float = 0.5, ignore_index: int = -100, dtype: torch.dtype = torch.float):
        super().__init__()
        self.beta = beta
    ...
    def forward(
        ...
        loss, dX = jsd_forward(_input, target, shift_labels, self.beta, self.ignore_index)
...
class TorchJSDBaseline(nn.Module):
    ...
    def forward(self, log_q: Tensor, log_p: Tensor, label: Optional[Tensor] =None):
        ...
        # JSD loss
        loss = (
            self.beta * self.kl(torch.log(m), log_p).sum(dim=-1) +
            (1 - self.beta) * self.kl(torch.log(m), log_q).sum(dim=-1)
        )
...
def check_jsd_kernel(
    ...
    def helion_wrapper(log_q, log_p, shift_labels):
        return helion_jsd(log_q, log_p, shift_labels)

    def baseline_wrapper(log_q, log_p, shift_labels):
...
def jsd_tritonbench(tb_op: object, log_q: Tensor, log_p: Tensor) -> Callable:
    ...
    helion_jsd = HelionJSD(beta=baseline_model.beta, ignore_index=baseline_model.ignore_index, dtype=baseline_model.dtype)
...
def main() -> None:
    ...
    for V in [2**i for i in range(12,18)]:
        print(f"Testing JSD: B={B}, T={T}, V={V}, β={beta}, ignore_index={ignore_index}, labels={use_labels}")
