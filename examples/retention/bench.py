"""
Retention / RetNet - local benchmark vs FLA
===========================================

Runs Helion's chunk_retention against FLA on the 6 production shapes from
...
Usage:
    HELION_AUTOTUNE_EFFORT=none python -m examples.retention.bench
    HELION_AUTOTUNE_EFFORT=quick python -m examples.retention.bench --fwd-only
"""
...
from .chunk import fla_chunk_retention_native
from .chunk import naive_recurrent_retention
...
def _accuracy(b: int, t: int, h: int, d: int, scale: float, fwd_only: bool) -> str:
    ...
    Retention's per-head decay gamma_h = 1 - 2^(-5-h) is fixed and derived from
    the head index inside every reference, so the signature is just (q,k,v,scale)
    with no decay tensor to thread. Drift is FLA's get_err_ratio (relative RMS
    error); a tensor passes if Helion's ratio is within ACC_FACTOR of FLA's.
    Returns "<verdict> <h>/<fla>" with the worst ratio on each side, suffixed
    " bwd:oom" when the fp32 naive backward runs out of memory and the gradient
    check is skipped.
    """
    ...
    with torch.no_grad():
        ref = naive_recurrent_retention(q.float(), k.float(), v.float(), scale=scale)
    h_out = chunk_retention(q.detach(), k.detach(), v.detach(), scale=scale)
    f_out = fla_chunk_retention_native(
    ...
    if not fwd_only:
        try:
            ...
            naive_recurrent_retention(qn, kn, vn, scale=scale).backward(go)
            chunk_retention(q, k, v, scale=scale).backward(go.to(DTYPE))
            fla_chunk_retention_native(q_fla, k_fla, v_fla, scale).transpose(
                1, 2
...
def main() -> None:
    ...
    for name, b, t, h, d in SHAPES:
        ...
        def fla_fwd(
            ...
            return fla_chunk_retention_native(q, k, v, scale)
        ...
        if not args.fwd_only:
            ...
            def helion_fb(
                ...
                o = chunk_retention(q, k, v, scale=scale)
                o.backward(go)
            ...
            def fla_fb(
                ...
                fla_chunk_retention_native(q, k, v, scale).backward(go)
                q.grad = k.grad = v.grad = None
