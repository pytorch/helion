...
Forward/backward correctness of each variant vs the fp32 reference kernel, at autotune effort `none`. Cells are `fwd/bwd`: `ok` within tolerance, `FAIL` over it, `HEL-ERR` the Helion kernel errored, `REF-ERR` the reference could not be evaluated (e.g. it OOMs).
...
| delta_rule | ok/ok | ok/ok | ok/ok | ok/ok | ok/REF-ERR | ok/ok |
| gated_delta_rule | ok/ok | ok/ok | ok/ok | ok/ok | ok/REF-ERR | ok/ok |
