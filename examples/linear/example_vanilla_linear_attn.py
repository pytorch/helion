...
Demonstrates chunked linear attention with no decay and no correction term.
...
def benchmark(
    ...
    for bi, hi, ti, di, dvi in configs if configs is not None else BENCH_CONFIGS:
        ...
        def helion_fwd(
            ...
            return chunked_linear_attn(q, ki, vi, None, C=BENCH_C, scale=sc)
        ...
        def helion_fb(
            ...
            o = chunked_linear_attn(q, ki, vi, None, C=BENCH_C, scale=sc)
            o.backward(go)
...
def accuracy(
    ...
    autograd gradients against the chunked reference. Drives the dashboard's
    helion_accuracy metric, so it runs at the benchmark shapes.
    """
    ...
    for bi, hi, ti, di, dvi in configs if configs is not None else BENCH_CONFIGS:
        ...
        out = chunked_linear_attn(q, k, v, None, C=BENCH_C, scale=scale)
        ref = naive_recurrent_reference(q, k, v, g, q_scale=scale)
