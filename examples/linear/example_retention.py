...
def test() -> None:
    ...
    g_rec = g_gamma[None, :, None].expand(B, H, T).contiguous().to(DTYPE)
...
def accuracy(
    ...
    for bi, hi, ti, di, dvi in configs if configs is not None else BENCH_CONFIGS:
        ...
        # Reference takes fp32 gamma; the per-head decay (~0.99997) needs fp32 to
        # stay accurate over long T.
        ref = naive_recurrent_reference(q, k, v, g.float(), q_scale=scale)
        ...
        try:
            ...
            chunked_linear_attn_reference(
                rl[0] * scale, rl[1], rl[2], g.float(), C=BENCH_C
            ).backward(grad_out)
            bwd_ok = all(
