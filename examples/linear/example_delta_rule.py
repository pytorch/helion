...
Includes correctness tests against a naive recurrent reference, FLA, and
...
def test() -> None:
    ...
    # === Forward: vs FLA ===
    try:
        from fla.ops.delta_rule import (  # pyrefly: ignore[missing-import]
            chunk_delta_rule as _chunk_delta_rule,
        )

        chunk_delta_rule: Any = _chunk_delta_rule
...
def accuracy(
    ...
    Forward compares against the naive recurrent reference (q pre-scaled, beta
    correction); backward compares autograd gradients against the chunked
    reference. Drives the dashboard's helion_accuracy metric, so it runs at the
    benchmark shapes.
    """
    ...
    for bi, hi, ti, di, dvi in configs if configs is not None else BENCH_CONFIGS:
        ...
        try:
            ...
            chunked_linear_attn(
                hl[0] * scale, hl[1], hl[2], g, beta=beta, C=BENCH_C
            ).backward(grad_out)
            rl = [x.detach().requires_grad_(True) for x in (q, k, v)]
            chunked_linear_attn_reference(
                rl[0] * scale, rl[1], rl[2], g, beta=beta, C=BENCH_C
            ).backward(grad_out)
            bwd_ok = all(
