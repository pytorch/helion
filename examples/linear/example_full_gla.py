...
def test() -> None:
    ...
    # === Forward: vs FLA ===
    try:
        from fla.ops.gla import (  # pyrefly: ignore[missing-import]
            chunk_gla as _chunk_gla,
        )

        chunk_gla: Any = _chunk_gla
...
def accuracy(
    ...
    Forward compares against the naive recurrent reference (q pre-scaled, as the
    diagonal-decay reference expects); backward compares autograd gradients
    against the chunked reference. Drives the dashboard's helion_accuracy
    metric, so it runs at the benchmark shapes.
    """
