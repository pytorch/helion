...
def layer_norm_bwd(
    ...
    """Compute gradients for input, weight, and optionally bias in a single kernel."""
    ...
    grad_weight_final = grad_weight_blocks.sum(0).to(weight.dtype)
    if compute_bias_grad:
        grad_bias_final = grad_bias_blocks.sum(0).to(weight.dtype)
        return grad_x, grad_weight_final, grad_bias_final
    return grad_x, grad_weight_final, None
