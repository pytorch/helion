...
Per-row min reduction over a jagged 2-D tensor (variable rows, fixed
feature dim).  Same scaffolding as ``jagged_sum.py``; replaces the sum
accumulator with ``torch.minimum`` and ``+inf`` init.
"""
...
def jagged_amin_kernel(
    ...
    """Per-row ``amin`` over a jagged 2-D tensor.
    ...
    Returns:
        2-D tensor ``(num_rows, M)`` with each row's per-feature min.
    """
    M = x_data.shape[1]
    ...
    for tile_b in hl.tile(num_rows):
        ...
        for tile_m in hl.tile(M):
            row_min = hl.full([tile_b, tile_m], float("inf"), dtype=x_data.dtype)
...
def reference_jagged_amin_kernel_pytorch(
    ...
    """PyTorch reference for ``jagged_amin_kernel``."""
...
def jagged_amin_tritonbench(
    ...
    return lambda: jagged_amin_kernel(x_values, x_offsets)
...
def main() -> None:
    ...
    run_example(
        jagged_amin_kernel,
        reference_jagged_amin_kernel_pytorch,
        (x_data, x_offsets),
        kernel_name="helion",
        baseline_name="torch",
    )
