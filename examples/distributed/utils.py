...
_cpp_mod = load_inline(
    "cpp_mod_utils", cpp_sources=_from_blob_cpp, with_cuda=True, functions=["from_blob"]
)


def dev_array_to_tensor(
    ...
    return _cpp_mod.from_blob(dev_array_ptr, shape, dtype)


@triton.jit
def _get_tid():
    """Get the current thread ID in x, y, z dimensions."""
...
@triton.jit
def _get_ntid():
    """Get the number of threads per block in x, y, z dimensions."""
    return tl.inline_asm_elementwise(
        """
        mov.u32 $0, %ntid.x;
        mov.u32 $1, %ntid.y;
...
@triton.jit
def _get_flat_tid():
    """Get the flattened thread ID within the block."""
...
@triton.jit
def _get_flat_bid():
    """Get the flattened block ID in the grid."""
    return (
...
def _send_signal(addrs, sem: tl.constexpr):
    """Send a signal to a remote address using atomic CAS."""
...
def _wait_signal(addrs, sem: tl.constexpr):
    """Wait for a signal from a remote address using atomic CAS."""
    tl.inline_asm_elementwise(
        f"""
        {{
            .reg .u32   %tmp32_<1>;
...
def symm_mem_sync(
    ...
    This function is a building block for expressing different synchronization
    patterns using symmetric memory signal pads. It is NOT a system level
    barrier/fence by itself.
    ...
    Args:
        signal_pad_ptrs: Tensor containing pointers to signal pads on all devices.
        block_id: The block ID used for synchronization. If None, uses the
            flattened block ID from the current program.
        rank: The rank of the current device (compile-time constant).
        world_size: The total number of devices (compile-time constant).
        hasPreviousMemAccess: If True, issues a release fence before signaling
            to ensure previous memory accesses are visible.
        hasSubsequentMemAccess: If True, issues an acquire fence after waiting
            to ensure subsequent memory accesses see the updates.
    """
    if block_id is None:
