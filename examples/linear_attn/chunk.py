...
def chunk_bwd_dqk(
    ...
    for tile_bhn, tile_d in hl.tile([BHN, D]):
        ...
        for tile_dv in hl.tile(DV):
            ...
            dq_cross_acc = torch.baddbmm(dq_cross_acc, dot, ht.transpose(-2, -1).to(dot.dtype))
            dk_state_acc = torch.baddbmm(dk_state_acc, vt, dht.transpose(-2, -1).to(vt.dtype))
...
def fla_chunk_linear_attn(
    ...
    v: torch.Tensor,
    scale: float | None = None,
    C: int = 64,  # noqa: ARG001 — kept for matching signature with chunk_linear_attn
) -> torch.Tensor:
