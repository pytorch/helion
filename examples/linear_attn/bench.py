...
def main() -> None:
    ...
    parser.add_argument("--fwd-only", action="store_true", help="skip forward+backward bench")
    ...
    for name, b, t, h, d in SHAPES:
        ...
        def helion_fwd(q: torch.Tensor = q, k: torch.Tensor = k, v: torch.Tensor = v) -> torch.Tensor:
            return chunk_linear_attn(q, k, v, scale=scale)

        def fla_fwd(q: torch.Tensor = q, k: torch.Tensor = k, v: torch.Tensor = v) -> torch.Tensor:
        ...
        if not args.fwd_only:
            ...
            def helion_fb(
                q: torch.Tensor = q_g, k: torch.Tensor = k_g, v: torch.Tensor = v_g,
