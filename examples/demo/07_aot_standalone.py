"""AOT standalone -> ship without Helion at runtime."""
...
if aot_mode != "disabled":
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "07_aot_standalone_data")
    print(f"Launching aot_runner --standalone -> {output_dir}", flush=True)
    env = {**os.environ, "HELION_AUTOTUNE_EFFORT": "quick"}
    cmd = [sys.executable, "-m", "helion.experimental.aot_runner", "--standalone",
