"""
Gated Linear Attention (GLA) - local benchmark vs FLA
=====================================================

Runs Helion's chunk_gla against FLA on the 6 production shapes from
...
Usage:
    HELION_AUTOTUNE_EFFORT=none python -m examples.gla.bench
    HELION_AUTOTUNE_EFFORT=quick python -m examples.gla.bench --fwd-only
"""
...
from .chunk import fla_chunk_gla_native
from .chunk import naive_recurrent_gla
...
def _accuracy(b: int, t: int, h: int, d: int, scale: float, fwd_only: bool) -> str:
    ...
    g_fla = g.detach().transpose(1, 2).contiguous().requires_grad_(not fwd_only)

    # name -> (helion ratio, fla ratio) vs the fp32 reference.
    ratios: dict[str, tuple[float, float]] = {}

    with torch.no_grad():
        ref = naive_recurrent_gla(
            q.float(), k.float(), v.float(), g.float(), scale=scale
        )
    h_out = chunk_gla(q.detach(), k.detach(), v.detach(), g.detach(), scale=scale)
    f_out = fla_chunk_gla_native(
    ...
    if not fwd_only:
        try:
            ...
            naive_recurrent_gla(qn, kn, vn, gn, scale=scale).backward(go)
            chunk_gla(q, k, v, g, scale=scale).backward(go.to(DTYPE))
            fla_chunk_gla_native(q_fla, k_fla, v_fla, g_fla, scale=scale).transpose(
                1, 2
            ...
            for name, hg, fg, ng in (
                ...
                ("dg", g.grad, g_fla.grad.transpose(1, 2), gn.grad),
            ):
...
def main() -> None:
    ...
    for name, b, t, h, d in SHAPES:
        ...
        def fla_fwd(
            ...
            g: torch.Tensor = g_fla,
            scale: float = scale,
        ) -> torch.Tensor:
            return fla_chunk_gla_native(q, k, v, g, scale=scale)
        ...
        if not args.fwd_only:
            ...
            g_fla_g = g_g.detach().transpose(1, 2).contiguous().requires_grad_(True)
            ...
            def helion_fb(
                ...
                o = chunk_gla(q, k, v, g, scale=scale)
                o.backward(go)
            ...
            def fla_fb(
                ...
                fla_chunk_gla_native(q, k, v, g, scale=scale).backward(go)
                q.grad = k.grad = v.grad = g.grad = None
            ...
            # FLA's chunk_gla backward can OOM shared memory at D=256 on this
            # H100; guard it so the Helion f+b column still prints.
            try:
                ...
                fb_cols = f" {h_fb_ms:>10.3f}ms {'n/a (FLA OOM)':>22}"
