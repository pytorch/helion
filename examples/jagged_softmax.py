    ...
    for b in hl.grid(num_rows):
        ...
        if seq_len != 0:
            for tile_m in hl.tile(0, M):
                ...
                normalizer = hl.full([tile_m], 0.0, dtype=x_data.dtype)

                for tile_k in hl.tile(start, end):
                    x_slice = x_data[tile_k, tile_m]
                    slice_max = x_slice.amax(dim=0)
                    block_new_max = torch.maximum(block_max, slice_max)
                    normalizer *= torch.exp(block_max - block_new_max)
                    normalizer += torch.exp(x_slice - block_new_max[None, :]).sum(dim=0)
                    block_max = block_new_max

                for tile_k in hl.tile(start, end):
                    x_slice = x_data[tile_k, tile_m]
                    block_out = (
                        torch.exp(x_slice - block_max[None, :]) / normalizer[None, :]
                    )
...
def jagged_softmax_tritonbench(
    ...
    return lambda: jagged_softmax_kernel(x_values, x_offsets)
