...
def jagged_layer_norm_kernel(
    ...
    for tile_b in hl.tile(B):
        ...
        for seq_idx in hl.tile(0, max_seq_len):
            ...
            valid_pos = (seq_idx.index[None, :] < seq_len[:, None]) & valid_batch[
                :, None
            ]
            ...
            for feat_idx in hl.tile(M):
                ...
                flat_indices = (
                    base_indices[:, :, None] * M + feat_idx.index[None, None, :]
                )
    ...
    for i in range(B):
        ...
        if end > start:
            ...
            normalized = torch.nn.functional.layer_norm(
                batch_values.view(-1), normalized_shape=[batch_values.numel()], eps=eps
