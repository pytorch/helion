...
def squeeze_and_excitation_net_fwd(x: Tensor, a: Tensor, b: Tensor) -> tuple[Tensor, Tensor, Tensor]:
    """
...
def squeeze_and_excitation_net_bwd_dx(
    ...
    for tile_m, tile_n in hl.tile([m, n]):
        ...
        for tile_k in hl.tile(k):
            ...
            grad_to_d = grad_out[tile_m, :] * x[tile_m, :] * d[tile_m, :] * (1.0 - d[tile_m, :])
...
def squeeze_and_excitation_net_bwd_db(
    ...
    for tile_k, tile_n in hl.tile([k, n]):
        ...
        for tile_m in hl.tile(m):
            grad_d = grad_out[tile_m, tile_n] * x[tile_m, tile_n] * d[tile_m, tile_n] * (1.0 - d[tile_m, tile_n])
...
def check(m: int, k: int, n: int) -> None:
    ...
    for bwd in [True, False]:
        run_example(squeeze_and_excitation_net, squeeze_and_excitation_net_pytorch, (x, a, b), bwd=bwd)
