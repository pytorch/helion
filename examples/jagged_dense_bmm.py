...
def _jagged_dense_bmm_pallas(
    ...
    # Keep logical rows/columns out of Mosaic's physical lane/sublane axes.
    output_4d = torch.empty((L, K, 1, 1), dtype=dtype, device=device)
    for b in hl.grid(B):
        start = seq_offsets[b]
        end = seq_offsets[b + 1]
        ...
        for tile_rows in hl.tile(start, end):
            for tile_k in hl.tile(0, K):
                ...
                output_4d[tile_rows, tile_k, 0, 0] = acc
    return output_4d.reshape(L, K)
