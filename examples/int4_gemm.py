...
def matmul_bf16_int4(A: Tensor, B: Tensor) -> Tensor:
    ...
    for tile_m, tile_n in hl.tile([M, N]):
        ...
        for tile_k_packed in hl.tile(K // 2, block_size=block_size_k_packed):
            ...
            b_unpacked = b_stacked.reshape(tile_k_packed.block_size * 2, tile_n.block_size)
...
# %%
def int4_gemm_tritonbench(x: torch.Tensor, w: torch.Tensor) -> callable:
    """
    Wrapper for TritonBench compatibility.
    
    Args:
        x (torch.Tensor): Left input tensor in bfloat16 format.
        w (torch.Tensor): Right input tensor in int32 format of shape [K, N].
    ...
    w_packed = (w_reshaped[0] & 0xF) | (w_reshaped[1] << 4)
    w_packed = w_packed.to(torch.int8)
...
def check(m: int, k: int, n: int) -> None:
    ...
    # Note: int4 quantization and bfloat16 computation can lead to some precision loss
