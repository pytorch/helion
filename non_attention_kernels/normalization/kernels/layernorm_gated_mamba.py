...
# Source: mamba_ssm/ops/triton/layernorm_gated.py
# Based on the Triton LayerNorm tutorial: https://triton-lang.org/main/getting-started/tutorials/05-layer-norm.html
"""
Gated LayerNorm and RMSNorm kernels from Mamba.

Supports both LayerNorm and RMSNorm with optional gating:
- If z is not None and norm_before_gate=True:  out = norm(x) * silu(z)
- If z is not None and norm_before_gate=False: out = norm(x * silu(z))

This is used in Mamba architectures where normalization is combined with gating.
"""
import math
...
def rms_norm_ref(x, weight, bias, z=None, eps=1e-6, group_size=None, norm_before_gate=True, upcast=True):
    """
    Reference implementation of RMSNorm with optional gating.

    Args:
        x: Input tensor
        weight: Scale parameter
        bias: Bias parameter (can be None)
        z: Optional gate tensor
        eps: Epsilon for numerical stability
        group_size: Group size for GroupNorm-style computation
        norm_before_gate: Whether to apply norm before or after gating
...
def _layer_norm_fwd_1pass_kernel(
    ...
    Z,  # pointer to the other branch (gate)
    ...
    """
    Forward pass kernel for LayerNorm/RMSNorm with optional gating.

    Grid: (M, ngroups) where M = number of rows, ngroups = N // group_size

    Computes:
    - LayerNorm: y = (x - mean) / sqrt(var + eps) * w + b
    - RMSNorm:   y = x / sqrt(mean(x^2) + eps) * w + b

    With optional gating:
    - norm_before_gate=True:  y = norm(x) * silu(z)
    - norm_before_gate=False: y = norm(x * silu(z))
    """
...
def _layer_norm_bwd_kernel(
    ...
    DZ,  # pointer to the other branch gradient
    Mean,   # pointer to the mean
    Rstd,   # pointer to the 1/std
    stride_x_row,  # how much to increase the pointer when moving by 1 row
    stride_z_row,
    ...
    """
    Backward pass kernel for LayerNorm/RMSNorm with optional gating.

    Grid: (nrow_groups, ngroups)

    Computes gradients for input, weight, bias, and gate tensor.
    """
...
def _layer_norm_fwd(x, weight, bias, eps, z=None, out=None, group_size=None, norm_before_gate=True, is_rms_norm=False):
    """
    Forward pass for LayerNorm/RMSNorm with optional gating.
    ...
    Args:
        ...
        eps: Epsilon for numerical stability
        z: Optional gate tensor of shape (M, N)
        out: Optional output tensor
        group_size: Group size for GroupNorm-style computation
        norm_before_gate: Whether to apply norm before or after gating
        is_rms_norm: Whether to use RMSNorm instead of LayerNorm

    Returns:
        out: Output tensor
        mean: Mean tensor (None for RMSNorm)
...
def _layer_norm_bwd(dy, x, weight, bias, eps, mean, rstd, z=None, group_size=None,
    ...
    """
    Backward pass for LayerNorm/RMSNorm with optional gating.
    ...
    Args:
        ...
        eps: Epsilon
        mean: Mean from forward (None for RMSNorm)
        rstd: Reciprocal std from forward
        z: Gate tensor from forward
        group_size: Group size
        norm_before_gate: Whether norm was applied before gating
        is_rms_norm: Whether RMSNorm was used
        recompute_output: Whether to recompute forward output
        dz: Optional pre-allocated gradient tensor for z
        out: Optional pre-allocated output tensor for recomputation

    Returns:
        dx: Gradient w.r.t. input
        dw: Gradient w.r.t. weight
        db: Gradient w.r.t. bias
        dz: Gradient w.r.t. gate
        out (optional): Recomputed output if recompute_output is True
    """
    M, N = x.shape
    if group_size is None:
        group_size = N
...
class LayerNormFn(torch.autograd.Function):
    """
    LayerNorm/RMSNorm with autograd support and optional gating.
    """
    ...
    def forward(ctx, x, weight, bias, z=None, eps=1e-6, group_size=None, norm_before_gate=True,
        ...
        Args:
            ...
            eps: Epsilon for numerical stability
            group_size: Group size for GroupNorm-style computation
            norm_before_gate: If True, apply norm(x) * silu(z), else norm(x * silu(z))
...
def layernorm_fn(x, weight, bias, z=None, eps=1e-6, group_size=None, norm_before_gate=True, is_rms_norm=False):
    """
    Functional interface for LayerNorm with optional gating.
    ...
    Returns:
        Normalized (and optionally gated) output tensor
    """
...
def rmsnorm_fn(x, weight, bias, z=None, eps=1e-6, group_size=None, norm_before_gate=True):
    """
    Functional interface for RMSNorm with optional gating.

    Args:
        x: Input tensor
        weight: Scale parameter
        bias: Bias parameter (usually None for RMSNorm)
        z: Optional gate tensor
        eps: Epsilon
        group_size: Group size for GroupNorm-style computation
        norm_before_gate: Whether to apply norm before gating
    ...
    Returns:
        RMS-normalized (and optionally gated) output tensor
    """
...
class LayerNorm(torch.nn.Module):
    """
    LayerNorm module with optional gating support.

    If group_size is not None, performs GroupNorm with each group having group_size elements.
    group_size=None is equivalent to group_size=hidden_size (standard LayerNorm).
    """
    ...
    def forward(self, x, z=None):
        ...
        If z is not None:
        - norm_before_gate=True:  returns norm(x) * silu(z)
        - norm_before_gate=False: returns norm(x * silu(z))
        """
...
class RMSNorm(torch.nn.Module):
    """
    RMSNorm module with optional gating support.

    If group_size is not None, performs GroupNorm-style RMSNorm with each group having group_size elements.
    group_size=None is equivalent to group_size=hidden_size (standard RMSNorm).
    """
    ...
    def forward(self, x, z=None):
        ...
        If z is not None:
        - norm_before_gate=True:  returns rmsnorm(x) * silu(z)
        - norm_before_gate=False: returns rmsnorm(x * silu(z))
        """
