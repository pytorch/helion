...
# Local quantization imports (standalone implementations)
from non_attention_kernels.quantization.kernels.fp8_kernel_sglang import (
    per_token_group_quant_fp8,
    static_quant_fp8,
)
from non_attention_kernels.quantization.kernels.int8_kernel_sglang import (
    per_token_quant_int8,
    per_token_group_quant_int8,
)


# Standalone utility functions (replaces sglang.srt.utils imports)
def is_cuda() -> bool:
...
def cpu_has_amx_support() -> bool:
    """Check if CPU has AMX support (simplified - returns False)."""
    return False


def is_batch_invariant_mode_enabled() -> bool:
    """Check if batch invariant mode is enabled (simplified - returns False)."""
    return False
...
def scaled_fp8_quant(
    ...
    """
    Scaled FP8 quantization using pure Triton implementation.

    Args:
        x: Input tensor to quantize
        scale: Pre-computed scale (if None, uses dynamic quantization)
        use_per_token_if_dynamic: If True and scale is None, use per-token quantization
    ...
    if scale is not None:
        ...
        return static_quant_fp8(x, scale, repeat_scale=use_per_token_if_dynamic)
    else:
        # Dynamic quantization - use per-token-group with group_size = last dimension
        return per_token_group_quant_fp8(x, x.shape[-1])
