# Helion plain scaled_mm for M=64 K=2048 N=12288
# vLLM=5.95us  helion=6.53us  ratio=1.098x  MISS
# config: Config(block_sizes=[64, 128, 128], indexing=['pointer', 'pointer', 'pointer', 'pointer', 'pointer'], l2_groupings=[1], num_stages=4, num_warps=8, pid_type='persistent_blocked', range_flattens=[None], range_multi_buffers=[None], range_num_stages=[0], range_unroll_factors=[0], range_warp_specializes=[False], split_k=1)
...
def _helion_scaled_mm(x, y, scale_a, scale_b, out, split_k, _NUM_SM: tl.constexpr, _BLOCK_SIZE_2: tl.constexpr):
    ...
    total_pids = tl.cdiv(64, _BLOCK_SIZE_0) * tl.cdiv(12288, _BLOCK_SIZE_1) * tl.cdiv(2048, _BLOCK_SIZE_2)
