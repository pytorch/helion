# Helion into scaled_mm for M=64 K=6144 N=2048
# vLLM=5.86us  helion=5.69us  ratio=0.971x  WIN
# config: Config(atomic_indexing=['pointer'], block_sizes=[64, 128, 256], indexing=['pointer', 'pointer', 'pointer', 'pointer'], num_stages=2, num_warps=4, pid_type='flat', split_k=16)
...
def scaled_mm_into(out: torch.Tensor, x: torch.Tensor, y: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_scaled_mm_into, ((64 + _BLOCK_SIZE_0 - 1) // _BLOCK_SIZE_0 * ((2048 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * ((6144 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2),), x, y, scale_a, scale_b, out, _BLOCK_SIZE_2, num_warps=4, num_stages=2)
