# Helion into scaled_mm for M=1 K=2048 N=12288
# vLLM=6.26us  helion=5.12us  ratio=0.818x  WIN
# config: Config(atomic_indexing=['pointer'], block_sizes=[16, 256, 128], indexing=['pointer', 'pointer', 'pointer', 'pointer'], num_stages=2, num_warps=4, pid_type='flat', split_k=8)
...
def scaled_mm_into(out: torch.Tensor, x: torch.Tensor, y: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_scaled_mm_into, (1 * ((12288 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * ((2048 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2),), x, y, scale_a, scale_b, out, _BLOCK_SIZE_2, num_warps=4, num_stages=2)
