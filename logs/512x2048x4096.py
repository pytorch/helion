# Helion plain scaled_mm for M=512 K=2048 N=4096
# vLLM=6.52us  helion=9.05us  ratio=1.388x  MISS
# config: Config(block_sizes=[128, 128, 128], indexing=['pointer', 'pointer', 'pointer', 'pointer', 'pointer'], l2_groupings=[1], num_stages=4, num_warps=8, pid_type='persistent_blocked', range_flattens=[None], range_multi_buffers=[None], range_num_stages=[0], range_unroll_factors=[0], range_warp_specializes=[False], split_k=1)
...
def _helion_scaled_mm(x, y, scale_a, scale_b, out, split_k, _NUM_SM: tl.constexpr, _BLOCK_SIZE_2: tl.constexpr):
    ...
    total_pids = tl.cdiv(512, _BLOCK_SIZE_0) * tl.cdiv(4096, _BLOCK_SIZE_1) * tl.cdiv(2048, _BLOCK_SIZE_2)
    ...
    for virtual_pid in tl.range(start_pid, end_pid, warp_specialize=False):
        ...
        if eq:
            ...
            tl.atomic_add(out + (indices_0[:, None] * 4096 + indices_1[None, :] * 1), v_3, mask=None, sem='relaxed')
