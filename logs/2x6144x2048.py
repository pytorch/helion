# Helion into scaled_mm for M=2 K=6144 N=2048
# vLLM=5.34us  helion=4.35us  ratio=0.815x  WIN
...
def _helion_scaled_mm_into(x, y, scale_a, scale_b, out, _BLOCK_SIZE_2: tl.constexpr):
    ...
    for offset_3 in tl.range(tl.cast(offset_2, tl.int32), tl.cast(tile_end, tl.int32), _BLOCK_SIZE_3):
        ...
        acc_copy_0 = acc_copy
        # src[scaled_mm.py:115]: acc = hl.dot(x[tile_m, inner_k], y[inner_k, tile_n], acc=acc)
        load = tl.load(x + (indices_0[:, None] * 6144 + indices_3[None, :] * 1), mask_0[:, None] & mask_3[None, :], other=0.0)
...
def scaled_mm_into(out: torch.Tensor, x: torch.Tensor, y: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_scaled_mm_into, ((2 + _BLOCK_SIZE_0 - 1) // _BLOCK_SIZE_0 * ((2048 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * ((6144 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2),), x, y, scale_a, scale_b, out, _BLOCK_SIZE_2, num_warps=4, num_stages=2)
