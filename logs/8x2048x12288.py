# Helion into scaled_mm for M=8 K=2048 N=12288
# vLLM=5.81us  helion=5.17us  ratio=0.890x  WIN
# config: Config(atomic_indexing=['pointer'], block_sizes=[16, 128, 128], indexing=['pointer', 'pointer', 'pointer', 'pointer'], num_stages=2, num_warps=4, pid_type='flat', split_k=4)
...
def _helion_scaled_mm_into(x, y, scale_a, scale_b, out, _BLOCK_SIZE_2: tl.constexpr):
    ...
    for offset_3 in tl.range(tl.cast(offset_2, tl.int32), tl.cast(tile_end, tl.int32), _BLOCK_SIZE_3):
        ...
        acc_copy_0 = acc_copy
        # src[scaled_mm.py:115]: acc = hl.dot(x[tile_m, inner_k], y[inner_k, tile_n], acc=acc)
        load = tl.load(x + (indices_0[:, None] * 2048 + indices_3[None, :] * 1), mask_0[:, None] & mask_3[None, :], other=0.0)
    ...
    load_2 = tl.load(scale_a + indices_0[:, None] * 1, mask_0[:, None], other=0)
    v_0 = acc * load_2
    ...
    tl.atomic_add(out + (indices_0[:, None] * 12288 + indices_1[None, :] * 1), v_2, mask=mask_0[:, None], sem='relaxed')
...
def scaled_mm_into(out: torch.Tensor, x: torch.Tensor, y: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_scaled_mm_into, ((8 + _BLOCK_SIZE_0 - 1) // _BLOCK_SIZE_0 * ((12288 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * ((2048 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2),), x, y, scale_a, scale_b, out, _BLOCK_SIZE_2, num_warps=4, num_stages=2)
