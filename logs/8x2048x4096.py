# Helion into scaled_mm for M=8 K=2048 N=4096
# vLLM=3.89us  helion=3.11us  ratio=0.798x  WIN
...
def _helion_scaled_mm_into(x, y, scale_a, scale_b, out, _BLOCK_SIZE_2: tl.constexpr):
    ...
    tl.atomic_add(out + (indices_0[:, None] * 4096 + indices_1[None, :] * 1), v_2, mask=mask_0[:, None], sem='relaxed')
...
def scaled_mm_into(out: torch.Tensor, x: torch.Tensor, y: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_scaled_mm_into, ((8 + _BLOCK_SIZE_0 - 1) // _BLOCK_SIZE_0 * ((4096 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * ((2048 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2),), x, y, scale_a, scale_b, out, _BLOCK_SIZE_2, num_warps=4, num_stages=2)
