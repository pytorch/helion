# Helion into scaled_mm for M=64 K=2048 N=2048
# vLLM=4.12us  helion=4.13us  ratio=1.003x  MATCH
...
def _helion_scaled_mm_into(x, y, scale_a, scale_b, out, _BLOCK_SIZE_2: tl.constexpr):
    ...
    tl.atomic_add(out + (indices_0[:, None] * 2048 + indices_1[None, :] * 1), v_2, mask=None, sem='relaxed')
...
def scaled_mm_into(out: torch.Tensor, x: torch.Tensor, y: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_scaled_mm_into, ((64 + _BLOCK_SIZE_0 - 1) // _BLOCK_SIZE_0 * ((2048 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * ((2048 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2),), x, y, scale_a, scale_b, out, _BLOCK_SIZE_2, num_warps=4, num_stages=2)
