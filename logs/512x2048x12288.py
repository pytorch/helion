# Helion plain scaled_mm for M=512 K=2048 N=12288
# vLLM=14.98us  helion=26.51us  ratio=1.770x  MISS
# config: Config(block_sizes=[128, 256, 128], indexing=['pointer', 'pointer', 'pointer', 'pointer', 'pointer'], l2_groupings=[1], num_stages=4, num_warps=8, pid_type='flat', range_flattens=[None], range_multi_buffers=[None], range_num_stages=[0], range_unroll_factors=[0], range_warp_specializes=[False], split_k=1)
...
@triton.jit
def _helion_scaled_mm(x, y, scale_a, scale_b, out, split_k, _BLOCK_SIZE_2: tl.constexpr):
    ...
    num_blocks_1 = tl.cdiv(12288, _BLOCK_SIZE_1)
    ...
    for offset_3 in tl.range(tl.cast(offset_2, tl.int32), tl.cast(tile_end, tl.int32), _BLOCK_SIZE_3):
        ...
        acc_copy_0 = acc_copy
        # src[scaled_mm.py:72]: acc = hl.dot(x[tile_m, inner_k], y[inner_k, tile_n], acc=acc)
        load = tl.load(x + (indices_0[:, None] * 2048 + indices_3[None, :] * 1), mask_3[None, :], other=0.0)
        load_1 = tl.load(y + (indices_3[:, None] * 1 + indices_1[None, :] * 2048), mask_3[:, None], other=0.0)
    ...
    if eq:
        ...
        tl.atomic_add(out + (indices_0[:, None] * 12288 + indices_1[None, :] * 1), v_3, mask=None, sem='relaxed')
...
def scaled_mm(x: torch.Tensor, y: torch.Tensor, scale_a: torch.Tensor, scale_b: torch.Tensor, *, _launcher=_default_launcher):
    ...
    _launcher(_helion_scaled_mm, ((512 + _BLOCK_SIZE_0 - 1) // _BLOCK_SIZE_0 * ((12288 + _BLOCK_SIZE_1 - 1) // _BLOCK_SIZE_1) * ((2048 + _BLOCK_SIZE_2 - 1) // _BLOCK_SIZE_2),), x, y, scale_a, scale_b, out, split_k, _BLOCK_SIZE_2, num_warps=8, num_stages=4)
