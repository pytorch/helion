...
class TestAutotunerCheckpoint(TestCase):
    ...
    # Expected auto-serialized keys per algorithm. Production code auto-serializes
    # __dict__ minus _checkpoint_exclude; this set declares what we expect to see.
    # If someone adds a new attr without updating either _checkpoint_exclude or this
    # set, the test will fail, forcing an explicit decision.
    _base_auto_keys: ClassVar[set[str]] = {
        "best_perf_so_far",
        "_current_generation",
        "counters",
    }
    _pattern_search_auto_keys: ClassVar[set[str]] = _base_auto_keys | {
    ...
    _de_auto_keys: ClassVar[set[str]] = _base_auto_keys | {
        "initial_population_strategy",
        "population_size",
        "max_generations",
        "crossover_rate",
    ...
    _lfbo_auto_keys: ClassVar[set[str]] = _pattern_search_auto_keys | {
        "num_neighbors",
        "radius",
        "frac_selected",
        "patience",
    ...
    _de_surrogate_auto_keys: ClassVar[set[str]] = _de_auto_keys | {
        "surrogate_threshold",
        "candidate_ratio",
        "refit_frequency",
        "n_estimators",
    ...
    def _get_autotuner_configs(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            "PatternSearch": {
                ...
                "expected_auto_keys": self._pattern_search_auto_keys,
            },
            ...
            "DESurrogateHybrid": {
                ...
                "expected_auto_keys": self._de_surrogate_auto_keys,
            },
            "LFBOTreeSearch": {
    ...
    # Keys in state_dict that come from metadata or custom hooks, not auto-serialization.
    _state_dict_special_keys: ClassVar[set[str]] = {
        "algorithm",
    ...
    def _verify_state_dict_save_restore(
        ...
        search_a: BaseSearch,
        algorithm: str,
        expected_auto_keys: set[str],
    ) -> None:
        """Assert that state_dict() captures all state and load_state_dict() restores it.

        1. Verifies auto-serialized keys match the expected set
        2. Verifies all instance attributes are either in state_dict or _checkpoint_exclude
        3. Modifies all values in state_a, loads into a fresh search, verifies roundtrip
        """
        state_a = search_a.state_dict()
        state_keys = set(state_a.keys())

        # Verify auto-serialized keys match expected set
        actual_auto_keys = state_keys - self._state_dict_special_keys
        unexpected = actual_auto_keys - expected_auto_keys
        missing_expected = expected_auto_keys - actual_auto_keys
        self.assertFalse(
            unexpected,
            f"{algorithm} has unexpected auto-serialized keys: {unexpected}\n"
            f"If they should be checkpointed, add to expected_auto_keys in "
            f"_get_autotuner_configs(). If not, add to _checkpoint_exclude.",
        )
        self.assertFalse(
            missing_expected,
            f"{algorithm} is missing expected auto-serialized keys: {missing_expected}\n"
            f"These keys were expected but not found in state_dict.",
        )

        # Verify all instance attrs are accounted for (either in state_dict or excluded)
        excluded = type(search_a)._collect_checkpoint_exclude()
        instance_attrs = set(vars(search_a).keys())
        uncovered = instance_attrs - state_keys - excluded
        self.assertFalse(
            uncovered,
            f"{algorithm} has instance attributes not in state_dict or exclusion list: {uncovered}\n"
            f"Add to expected_auto_keys in _get_autotuner_configs() if they should be "
            f"checkpointed, or to _checkpoint_exclude if they're derived/transient.",
        )
    ...
    def _verify_PatternSearch(self, search_a: BaseSearch, search_b: BaseSearch) -> None:
        ...
        for copy_a, copy_b in zip(
            ...
            try:
                # Use copies of visited so we don't mutate the original
                saved_visited_a = search_a.visited.copy()  # type: ignore[attr-defined]
                saved_visited_b = search_b.visited.copy()  # type: ignore[attr-defined]

                candidates_a = search_a._generate_copy_candidates(copy_a)  # type: ignore[attr-defined]
                candidates_b = search_b._generate_copy_candidates(copy_b)  # type: ignore[attr-defined]

                # Restore visited sets
                search_a.visited = saved_visited_a  # type: ignore[attr-defined]
                search_b.visited = saved_visited_b  # type: ignore[attr-defined]
    ...
    def test_checkpoint(self, autotuner: str) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._verify_state_dict_save_restore(
                search_a, autotuner, config["expected_auto_keys"]
            )
