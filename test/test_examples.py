...
def _matmul_ln_kernel(x, y, weight, bias, out, _BLOCK_SIZE_0: tl.constexpr, _BLOCK_SIZE_1: tl.constexpr, _BLOCK_SIZE_2: tl.constexpr):
    ...
    load_3 = tl.load(bias + indices_1 * 1, None)
    var_mean_extra = tl.reshape(tl.sum(acc, 1), [_BLOCK_SIZE_0, 1])
    v_1 = var_mean_extra / _BLOCK_SIZE_1
    v_2 = acc - v_1
    v_3 = v_2 * v_2
    var_mean_extra_2 = tl.reshape(tl.sum(v_3, 1), [_BLOCK_SIZE_0, 1])
    v_4 = var_mean_extra_2 / _BLOCK_SIZE_1
    v_5 = 1e-05
    v_6 = v_4 + v_5
