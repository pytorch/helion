...
class TestExamples(RefEagerTestBase, TestCase):
    ...
    def test_layernorm_bwd(self):
        ...
        weight = torch.randn(
            [dim], device=DEVICE, dtype=torch.float16
        )
        bias = torch.randn(
            [dim], device=DEVICE, dtype=torch.float16
        )
        ...
        _, mean, rstd = configured_kernel(x, [dim], weight, bias)
    ...
    def test_layernorm_bwd_no_bias(self):
        """Test combined backward pass for layer norm without bias."""
        batch_size, dim = 32, 64
        x = -2.3 + 0.5 * torch.randn(
            [batch_size, dim], device=DEVICE, dtype=torch.float16
        )
        ...
        config = helion.Config(block_size=32, num_warps=4, num_stages=3)
        configured_kernel = helion.kernel(layer_norm_fwd.fn, config=config)
        _, mean, rstd = configured_kernel(x, [dim], weight, None)
        ...
        args = (grad_out, x, weight, mean, rstd, False)
