...
@onlyBackends(["triton"])
class TestTritonTwoDotIMA(RefEagerTestDisabled, TestCase):
    ...
    def test_two_dot_batch_loop_ima(self):
        ...
        def two_dot_bwd(
            ...
            K = A.size(0)
            out1 = torch.empty(
                (Batch, N, K), device=x.device, dtype=x.dtype
            )
            out2 = torch.empty(
                (Batch, M, K), device=x.device, dtype=x.dtype
            )
            for tile_b in hl.tile(Batch, block_size=1):
                # dot 1: x[N,M] @ A[K,M].T -> out1[N,K]
                for tile_n, tile_k in hl.tile([N, K]):
                    acc = hl.zeros(
                        [tile_n, tile_k], dtype=torch.float32
                    )
                    ...
                    out1[tile_b.begin, tile_n, tile_k] = acc.to(
                        x.dtype
                    )
                # dot 2: x[N,M].T @ B[N,K] -> out2[M,K]
                for tile_m2, tile_k2 in hl.tile([M, K]):
                    acc2 = hl.zeros(
                        [tile_m2, tile_k2], dtype=torch.float32
                    )
                    ...
                    out2[tile_b.begin, tile_m2, tile_k2] = acc2.to(
                        x.dtype
                    )
            return out1, out2
        ...
        search = FiniteSearch(
            bound, args, configs=[good_config, bad_config]
        )
        winner = search.autotune()

        # On H100, K_bs should be capped to 8 (not 16 or 32)
