...
def test_reviewed_runner_keeps_deepgemm_k_major_and_uses_profile_helion_layout(
    ...
    logical_b = torch.arange(24, dtype=torch.bfloat16).reshape(2, 3, 4)
    sentinels = tuple(torch.empty(0) for _ in range(4))
    monkeypatch.setattr(
        _deepgemm_public_api._SUPPORT,
        "make_case",
        lambda *_args: (sentinels[0], logical_b, *sentinels[1:]),
    )

    _a, deepgemm_b, helion_b, _layout, _reference, _worklist = (
        _deepgemm_public_api._make_reviewed_case(
            shape,
            (1,),
            torch.device("cpu"),
        )
    ...
    assert deepgemm_b is logical_b
    assert torch.equal(helion_b, logical_b)
    if profile.b_major == "k":
        assert helion_b is logical_b
        assert helion_b.stride() == (12, 4, 1)
    else:
        assert helion_b is not logical_b
        assert helion_b.stride() == (12, 1, 3)
...
def test_canonical_layout_gb300_workloads_resolve_only_measured_target_tunings() -> (
    ...
    for case, shape in zip(
        ...
        for group, actual_m in enumerate(case.actual_ms):
            stored_m = common.align(actual_m, profile.source_m_tile)
        ...
        tuning = policy.tuning_for(
            ...
            num_sm=152,
            worklist_rows=tuple(worklist_rows),
        )
        if tuning is not None:
            tuned_rows.add(case.row_index)

    assert tuned_rows == set(range(8))
...
def test_all_helion_selections_use_canonical_k_layout_and_distinct_apis(
    ...
    assert (
        selections["live_autotune"]["config"]["effective"][
    ...
    assert selected_layouts == ["k_major", "k_major"]
    assert all(
