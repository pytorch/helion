"""
Test Helion kernel fusion with PyTorch Inductor.

This module tests prologue and epilogue fusion for Helion kernels with Inductor:

TestBasicFusion:
    Basic prologue/epilogue fusion tests without view operations.
    Tests simple pointwise ops like relu, sigmoid, tanh, add, mul.

TestViewFusion:
    View-related fusion tests including reshape, slice, transpose, permute,
    narrow, expand, contiguous, as_strided, and complex chained operations.
...
- View operations that may prevent or allow fusion
"""
...
def elementwise_two_inputs(x, y):  # pyrefly: ignore
    """Elementwise kernel with two inputs for testing partial prologue fusion.

    Takes two [M, N] inputs, returns [M, N] output = x + y.
    Used to test that if one input has a view prologue (no fusion) and
    another has a pointwise prologue (should fuse), partial fusion works.
...
class TestBasicFusion(TestCase):
    """Test basic prologue and epilogue fusion without view operations.

    These tests verify that pointwise ops (relu, sigmoid, tanh, add, mul, etc.)
    can be fused into Helion kernels as prologue or epilogue operations.
    No view operations (reshape, slice, transpose, etc.) are used here.
    """
    ...
    def test_rms_norm_prologue_epilogue_multi_output(self):
        ...
        - x: [M, N] -> scale -> relu -> add bias -> sigmoid before RMS norm

        Epilogue chains (each tensor output has its own chain):
        - out: tanh((relu(out) + out_bias) * out_scale) - same shape [M, N]
        - residual: sigmoid((residual + res_bias) * res_scale) - same shape [M, N]
        - extra: add bias -> relu - same shape [M, N]

        Multiple inputs:
        - x: input tensor [M, N]
        - x_scale: prologue scale [N]
        - x_bias: prologue bias [N]
        - weight: RMS norm weight [N]
        - out_bias, out_scale: epilogue for out [N]
        - res_bias, res_scale: epilogue for residual [N]
        - extra_bias: epilogue for extra [N]
        ...
        def f(x, x_scale, x_bias, weight, out_bias, out_scale, res_bias, res_scale, extra_bias):
            # Prologue chain: scale -> relu -> add bias -> sigmoid
            x_scaled = x * x_scale
        ...
        inputs = (
            x, x_scale, x_bias, weight, out_bias, out_scale, res_bias, res_scale, extra_bias
        )
        ...
        # Verify fusion: should be single kernel (no views)
        kernel_count, all_code = count_triton_kernels(source_codes)
        self.assertEqual(kernel_count, 1, f"Expected 1 kernel, got {kernel_count}")
...
class TestViewFusion(TestCase):
    """Test view-related prologue and epilogue fusion.

    These tests verify fusion behavior when view operations (reshape, slice,
    transpose, permute, narrow, expand, etc.) are involved. View operations
    may or may not fuse depending on shape/stride compatibility.
    """
    ...
    def test_partially_flat_prologue_correctness(self):
        ...
        # Partial flatten prologue stays unfused (view fusion disabled)
        kernel_count, all_code = count_triton_kernels(source_codes)
        self.assertGreater(
            kernel_count, 1,
            f"Expected >1 kernels (view prologue unfused), got {kernel_count}"
        )
    ...
    def test_single_output_reshape_prologue(self):
        ...
        # Reshape prologue stays unfused (view fusion disabled)
        kernel_count, all_code = count_triton_kernels(source_codes)
        self.assertGreater(
            kernel_count, 1,
            f"Expected >1 kernels (reshape prologue unfused), got {kernel_count}"
        )
    ...
    def test_single_output_partial_flatten_prologue(self):
        ...
        self.assertGreater(
            kernel_count, 1,
            f"Expected >1 kernels (partial flatten prologue unfused), got {kernel_count}"
        )
    ...
    def test_3d_transpose_prologue_swap_last_two_dims(self):
        ...
        kernel_count, _ = count_triton_kernels(source_codes)
        self.assertGreater(kernel_count, 1)
    ...
    def test_multiple_views_prologue(self):
        ...
        Tests that multiple consecutive view operations do not fuse with Helion kernel.
    ...
    def test_expand_prologue(self):
        """Prologue: expand operation - x.expand(B, M, N) -> kernel (no fusion).
    ...
    def test_contiguous_after_transpose_prologue(self):
        """Prologue: transpose + contiguous - x.T.contiguous() -> kernel (no fusion).
    ...
    def test_transpose_then_view_to_3d_epilogue(self):
        """Epilogue: transpose then 2D->3D view - out.T.reshape(D1, D2, D3) -> ops (no fusion).
    ...
    def test_strided_slice_prologue(self):
        """Strided slice prologue x[::2, :] -> kernel (no fusion).
    ...
    def test_strided_slice_epilogue(self):
        """Strided slice epilogue out[::2, ::2] -> ops (no fusion).
    ...
    def test_diagonal_slice_epilogue(self):
        """Diagonal slice epilogue out.diagonal() -> ops (no fusion).
    ...
    def test_partial_prologue_fusion_view_and_pointwise(self):
        """Test partial prologue fusion: one input has view (no fusion), another has pointwise (fuses).

        This tests that we allow partial fusion - if one prologue group has a view,
        only that group is excluded from fusion, not ALL prologue groups.

        Setup:
        - x: [d1, d2*d3] -> view -> [d1*d2, d3] (view prologue - should NOT fuse)
        - y: [d1*d2, d3] -> relu -> scale (pointwise prologue - SHOULD fuse)
        - kernel takes both transformed inputs

        Expected: The pointwise prologue for y should still fuse even though x has a view.
        This requires > 1 kernel (view creates separate kernel) but y's prologue should be fused.
        """
        d1, d2, d3 = 8, 16, 32
        m, n = d1 * d2, d3  # Kernel operates on [M, N] = [128, 32]

        # x starts as [d1, d2*d3] and gets viewed to [d1*d2, d3]
        x_original = torch.randn(d1, d2 * d3, device=DEVICE, dtype=torch.float32)
        # y starts as [d1*d2, d3] - same shape as kernel input
        ...
        def f(x_original, y_original, y_scale):
            # Prologue for x: view operation (should NOT fuse)
            x = x_original.view(m, n)

            # Prologue for y: pointwise operations (SHOULD fuse)
        ...
        # Verify partial fusion happened:
        # - View creates a separate kernel (no fusion for x)
        # - But y's pointwise prologue should be fused into the Helion kernel
        # So we expect > 1 kernel (due to view) but partial fusion should work
        kernel_count, all_code = count_triton_kernels(source_codes)
        self.assertGreater(kernel_count, 1, f"Expected >1 kernels due to view, got {kernel_count}")

        # Verify that the Helion kernel contains y's fused prologue (relu)
        # by checking for relu-related ops in the kernel code
        helion_kernel_found = False
        for code in source_codes:
            if "elementwise_two_inputs" in code or "triton_heuristics" in code:
                # Check if relu is fused into the kernel (partial fusion worked)
                if "relu" in code.lower() or "maximum" in code.lower() or "tl.where" in code:
                    helion_kernel_found = True
                    break

        # Note: This assertion verifies partial fusion is working.
        # If blanket prologue disabling was still in place, y's relu would NOT be fused.
        self.assertTrue(
            helion_kernel_found,
