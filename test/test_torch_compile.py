...
class TestTorchCompile(RefEagerTestDisabled, TestCase):
    ...
    def test_autotune_fusion_aware_benchmarks(self, autotune_with_fusion):
        """Fusion-aware autotuning captures fused benchmarks when enabled, reuses cache when off."""
        if not supports_torch_compile_fusion():
            self.skipTest(
                "torch.compile fusion requires ExternalTritonTemplateKernel support"
            )
    ...
    @skipIfTileIR("torch.compile missing kernel metadata on tileir")
    def test_autotune_fusion_context_isolation(self):
        """Different fusion contexts store separate configs in _fusion_config_cache."""
        if not supports_torch_compile_fusion():
            self.skipTest(
                "torch.compile fusion requires ExternalTritonTemplateKernel support"
            )
        ...
        # Different epilogues produce separate fusion-context cache entries
    ...
    @skipIfTileIR("torch.compile missing kernel metadata on tileir")
    def test_standalone_call_after_fusion_has_unfused_config(self):
    ...
    @skipIfTileIR("torch.compile missing kernel metadata on tileir")
    def test_fusion_autotune_no_configs_skips_unfused_cache(self):
        """Fusion autotuning with no explicit configs must not crash or reuse unfused cache.
        ...
        be accepted by BaseCache (currently crashes with TypeError), and the
        autotuner must skip the disk cache to avoid returning a stale config
        from a prior unfused run.
        """
        if not supports_torch_compile_fusion():
            self.skipTest(
                "torch.compile fusion requires ExternalTritonTemplateKernel support"
