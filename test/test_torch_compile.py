...
class TestTorchCompile(RefEagerTestDisabled, TestCase):
    ...
    def test_epilogue_reads_kernel_output_with_different_indices(
        ...
        out + out.T reads the kernel output at (i,j) AND (j,i). Without a
        guard, fusion intercepts both reads identically, giving 2*out instead
        of out + out.T.
        """
    ...
    def test_epilogue_reduction_not_fused(self, allow_torch_compile_fusion):
        ...
        Without the Pointwise guard, replaying a reduction's inner_fn would
        crash or produce a wrong fused expression.
        """
    ...
    def test_epilogue_transposed_contiguous(self, allow_torch_compile_fusion):
        ...
        Without a guard comparing read vs. write indices, fusion intercepts the
        transposed read with the non-transposed value, producing wrong results.
        """
    ...
    def test_epilogue_gather_not_fused(self, allow_torch_compile_fusion):
        ...
        Without a guard comparing read vs. write indices, fusion substitutes the
        wrong (store-position) value for each gather access, giving wrong results.
        """
    ...
    def test_prologue_flip_fused(self, allow_torch_compile_fusion):
        ...
        intercept_fn in _codegen_prologue_fusion substitutes _fidx_k →
        dim_index_exprs[k] in the sympy access index so the remapped load
        (N-1-i) is preserved, producing flip(x)+x correctly in a single kernel.
        """
    ...
    def test_prologue_flip_plus_bias_not_fused(self, allow_torch_compile_fusion):
        ...
        When Inductor fuses ``flip(x) + b`` into a single ComputedBuffer it
        produces a node with two MemoryDep reads: x (with remapped index) and b
        (with broadcast index).  The old guard skipped multi-read nodes, so this
        fused node slipped through and _build_prologue_specs intercepted both reads
        with the same ``_load_val`` placeholder, collapsing ``flip(x)+b`` into
        ``2*_load_val`` — wrong results.  The updated guard rejects any prologue
        node with more than one MemoryDep read.
        """
        n = 64
        x = torch.randn(n, n, device=DEVICE, dtype=torch.float32)
    ...
    def test_prologue_bias_not_fused_when_multi_read(self, allow_torch_compile_fusion):
        ...
        In ``k_add(flip(x), y + b)``, the flip prologue is correctly rejected by
        the single-read index-remapping guard.  But ``y + b`` forms a separate
        two-read prologue (reads y and b) that the old guard also skipped, letting
        _build_prologue_specs collapse both reads into ``_load_val + _load_val``
        instead of the correct ``y[i,j] + b[j]``.  The updated guard rejects
        multi-read prologue nodes, causing ``y + b`` to run as a separate kernel.
        """
        n = 64
        x = torch.randn(n, n, device=DEVICE, dtype=torch.float32)
        y = torch.randn(n, n, device=DEVICE, dtype=torch.float32)
    ...
    def test_autotune_no_fusion_final_has_fusion(self):
        ...
        torch.testing.assert_close(
            result_direct,
            x + y,
            msg="Direct k_add call after torch.compile fusion must work (BoundKernel._run must be the non-fused kernel)",
        )
        ...
        # Final compiled code SHOULD contain fusion
        k_add.reset()
        torch._dynamo.reset()
