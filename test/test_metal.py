...
class TestMetalElementwise(unittest.TestCase):
    ...
    def test_vector_add_large(self) -> None:
        x = torch.randn(100000, device="mps")
        y = torch.randn(100000, device="mps")
        torch.testing.assert_close(vector_add(x, y), x + y)

    @requires_mps
    def test_vector_add_non_power_of_2(self) -> None:
        x = torch.randn(999, device="mps")
        y = torch.randn(999, device="mps")
        torch.testing.assert_close(vector_add(x, y), x + y)
    ...
    def test_vector_div(self) -> None:
        x = torch.randn(N, device="mps")
        y = torch.randn(N, device="mps").abs() + 0.1
...
class TestMetalMathOps(unittest.TestCase):
    ...
    def test_log(self) -> None:
        x = torch.rand(N, device="mps") + 0.01
...
class TestMetalDtypes(unittest.TestCase):
    ...
    def test_vector_add_bfloat16(self) -> None:
        x = torch.randn(N, device="mps", dtype=torch.bfloat16)
        y = torch.randn(N, device="mps", dtype=torch.bfloat16)
        torch.testing.assert_close(vector_add(x, y), x + y)

    @requires_mps
    def test_vector_add_int32(self) -> None:
        x = torch.randint(-100, 100, (N,), device="mps", dtype=torch.int32)
        y = torch.randint(-100, 100, (N,), device="mps", dtype=torch.int32)
        torch.testing.assert_close(vector_add(x, y), x + y)
...
class TestMetalBoundsMasking(unittest.TestCase):
    ...
    def test_store_no_oob_write(self) -> None:
        ...
        pad = 256
        sentinel = 42.0
        buf = torch.full((n + pad,), sentinel, device="mps")
        x = torch.randn(n, device="mps")
        y = torch.randn(n, device="mps")
        out = buf[:n]
        add_into(x, y, out)
...
class TestMetalMultiDim(unittest.TestCase):
    ...
    def test_elementwise_2d(self) -> None:
        ...
        y = torch.randn(64, 128, device="mps")
        torch.testing.assert_close(elementwise_2d(x, y), x + y)

    @requires_mps
    def test_elementwise_2d_non_aligned(self) -> None:
        x = torch.randn(33, 65, device="mps")
        y = torch.randn(33, 65, device="mps")
        torch.testing.assert_close(elementwise_2d(x, y), x + y)

    @requires_mps
    def test_elementwise_3d(self) -> None:
        x = torch.randn(8, 16, 32, device="mps")
        y = torch.randn(8, 16, 32, device="mps")
