...
class TestMutation(RefEagerTestDisabled, TestCase):
    ...
    def test_mutate_output_alias(self):
        ...
        # Mutation should still allow fusion
        compiled_fn = torch.compile(fn, fullgraph=True, backend="inductor")
        _, source_codes = run_and_get_code(compiled_fn, x.clone())
        kernel_count, _ = count_triton_kernels(source_codes)
        self.assertEqual(kernel_count, 1, f"Expected 1 kernel (fusion), got {kernel_count}")
    ...
    def test_mutation_fusion_copy(self):
        ...
        # copy_ ops prevent fusion
        compiled_fn = torch.compile(fn, fullgraph=True, backend="inductor")
        _, source_codes = run_and_get_code(compiled_fn, a.clone(), b.clone(), c.clone())
        kernel_count, _ = count_triton_kernels(source_codes)
        self.assertGreater(kernel_count, 1, f"copy_ should prevent fusion, expected >1 kernels, got {kernel_count}")
    ...
    def test_input_mutation_view_copy(self):
        ...
        # View + copy_ prevents fusion
        compiled_fn = torch.compile(fn, fullgraph=True, backend="inductor")
        _, source_codes = run_and_get_code(compiled_fn, a.clone())
        kernel_count, _ = count_triton_kernels(source_codes)
        self.assertGreater(kernel_count, 1, f"View + copy_ should prevent fusion, expected >1 kernels, got {kernel_count}")
    ...
    def test_mutation_after_kernel_preserves_output(self):
        ...
        # Mutation after kernel prevents fusion
        compiled_fn = torch.compile(fn, fullgraph=True, backend="inductor")
        _, source_codes = run_and_get_code(compiled_fn, x.clone(), y.clone())
        kernel_count, _ = count_triton_kernels(source_codes)
        self.assertGreater(kernel_count, 1, f"Mutation after kernel should prevent fusion, expected >1 kernels, got {kernel_count}")
    ...
    def test_two_mutated(self):
        ...
        # Two mutated outputs prevent full fusion
        compiled_fn = torch.compile(fn, fullgraph=True, backend="inductor")
        _, source_codes = run_and_get_code(compiled_fn, x.clone(), y.clone())
        kernel_count, _ = count_triton_kernels(source_codes)
        self.assertGreater(kernel_count, 1, f"Two mutated outputs should prevent full fusion, expected >1 kernels, got {kernel_count}")
    ...
    def test_mutation_not_returned_input(self):
        ...
        # Using mutated input after kernel prevents fusion
        compiled_fn = torch.compile(fn, fullgraph=True, backend="inductor")
        _, source_codes = run_and_get_code(compiled_fn, x.clone(), y.clone())
        kernel_count, _ = count_triton_kernels(source_codes)
        self.assertGreater(kernel_count, 1, f"Using mutated input after kernel should prevent fusion, expected >1 kernels, got {kernel_count}")
    ...
    def test_alias_same_tensor(self):
        ...
        # Aliased inputs should still allow fusion
        compiled_fn = torch.compile(fn, fullgraph=True, backend="inductor")
        _, source_codes = run_and_get_code(compiled_fn, x.clone())
        kernel_count, _ = count_triton_kernels(source_codes)
        self.assertEqual(kernel_count, 1, f"Expected 1 kernel (fusion), got {kernel_count}")
    ...
    def test_alias_view(self):
        ...
        # View alias should still allow fusion
        compiled_fn = torch.compile(fn, fullgraph=True, backend="inductor")
        _, source_codes = run_and_get_code(compiled_fn, x.clone())
        kernel_count, _ = count_triton_kernels(source_codes)
        self.assertEqual(kernel_count, 1, f"Expected 1 kernel (fusion), got {kernel_count}")
