...
class TestGenerateBestAvailablePopulation(unittest.TestCase):
    ...
    def test_matmul_heuristic_configs_replace_default_on_b200(self):
        """B200 matmul seeds replace the generic default config."""
        config_gen = self._make_config_gen()
        ...
        mock_search._heuristic_seed_configs = (
            PopulationBasedSearch._heuristic_seed_configs.__get__(
                mock_search,
    ...
    def test_random_population_includes_matmul_heuristic_seed(self):
        """Full autotuning random population includes matching heuristic seeds."""
        config_gen = self._make_config_gen()
        heuristic_configs = [Config(block_sizes=[32, 64], num_warps=8, num_stages=2)]
        mock_search = self._make_mock_search(config_gen, cached_configs=[])
        mock_search.args = [torch.empty(1)]
