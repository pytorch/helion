"""Correctness and perf tests for kernels under ``tutorials/``.
...
from helion._testing import TUTORIALS_DIR
...
def _import_tutorial_module(name):
    # Tutorials-namespaced module name avoids clashing with ``examples/<name>.py``.
    module_name = f"_helion_tutorials_test.{name}"
    if module_name not in sys.modules:
        file_path = TUTORIALS_DIR / name / f"{name}.py"
...
def _run_tutorial_main_and_parse_summary(name):
    module = _import_tutorial_module(name)
    buf = io.StringIO()
    ...
    if match is None:
        raise AssertionError(
            f"Could not find SUMMARY line in tutorial output for {name}.\n"
            f"Output was:\n{output}"
        )
    return {
...
# (atol, rtol) per tutorial. Norm/softmax need looser tolerances than
...
@onlyBackends(["triton"])
@skipIfRefEager("Tutorials are AOT kernels; ref-eager bypasses heuristic logic.")
class TestTutorialsCorrectness(TestCase):
    """Numerical correctness vs. PyTorch eager."""

    def _run_correctness(self, name: str) -> None:
        if not is_cuda():
            self.skipTest("Tutorial kernels require CUDA / ROCm.")
        module = _import_tutorial_module(name)
...
@onlyBackends(["triton"])
@skipIfRefEager("Tutorials are AOT kernels; ref-eager bypasses heuristic logic.")
class TestTutorialsPerformance(TestCase):
    """Run each tutorial's main() and gate on parsed wins / geomean (sm100 only)."""
    ...
    def setUpClass(cls) -> None:
        ...
        if not _on_sm100():
            raise unittest.SkipTest(
                "Pretuned tutorial heuristics target sm100; perf gating runs on B200."
            )

    def _run_tutorial_perf(self, name: str) -> None:
        expected = _EXPECTED_PERF[name]
        actual = _run_tutorial_main_and_parse_summary(name)
        self.assertEqual(
            actual["total"],
            expected["total"],
    ...
    def test_vector_add(self):
        self._run_tutorial_perf("vector_add")

    # softmax/layer_norm sweep enough shapes to need >60s under xdist contention.
    @pytest.mark.timeout(120)
    def test_softmax(self):
        self._run_tutorial_perf("softmax")

    @pytest.mark.timeout(120)
    def test_layer_norm(self):
        self._run_tutorial_perf("layer_norm")

    def test_rms_norm(self):
        self._run_tutorial_perf("rms_norm")

    def test_cross_entropy(self):
        self._run_tutorial_perf("cross_entropy")
