...
def test_aot_heuristic_can_abstain_and_fall_back_per_key(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    source_path = _kernel_source(tmp_path)
    ...
    default = Config(block_sizes=[8])
    cache.mode = "evaluate"
    cache.autotuner = SimpleNamespace(
        config_spec=SimpleNamespace(default_config=lambda: default),
        settings=SimpleNamespace(ignore_warnings=()),
    )

    assert cache.get() is default
    assert cache.get() is default
    assert capsys.readouterr().err == ""
    cache.args = (32,)
    selected = cache.get()
    assert selected is not None
    assert selected["block_sizes"] == [32]
    assert cache.get() is selected

    module = AOTAutotuneCache._heuristic_modules[heuristic_path.resolve()]
    assert module.CALLS == [16, 32]
    assert len(AOTAutotuneCache._heuristic_results) == 2
    assert None not in AOTAutotuneCache._heuristic_results.values()


def test_dynamic_standalone_compile_rejects_observed_abstention(
    tmp_path: Path,
) -> None:
    source_path = _kernel_source(tmp_path)
    heuristic_path = tmp_path / "_helion_aot_kernel_cuda_sm100.py"
    heuristic_path.write_text(
        'SUPPORTED_HARDWARE_NAMES = ("NVIDIA B200",)\n'
        "CONFIGS = [{'block_sizes': [16]}]\n"
        "def autotune_demo(value):\n"
        "    return None\n"
    )
    cache = _heuristic_cache_stub(source_path, heuristic_path, B200)
    cache.kernel.kernel._aot_standalone = True
    cache.kernel.settings = SimpleNamespace(static_shapes=False)

    with pytest.raises(
        RuntimeError,
        match="dynamic standalone compilation cannot encode",
    ):
        cache._maybe_run_compile()

    assert not AOTAutotuneCache._compiled_kernels


def test_aot_heuristic_malformed_result_is_not_an_abstention(
    tmp_path: Path,
    caplog: pytest.LogCaptureFixture,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    module = AOTAutotuneCache._heuristic_modules[heuristic_path.resolve()]
    assert module.CALLS == [16, 16]
    assert not AOTAutotuneCache._heuristic_results
    assert "must return a mapping or None, got int" in caplog.text
    assert capsys.readouterr().err.count("No heuristic found for 'demo'") == 1
...
def test_dynamic_standalone_rejects_abstaining_dispatch_key(tmp_path: Path) -> None:
    output = generate_standalone_file(
        "demo",
        ["def demo(value):\n    return value\n"],
        (
    ...
    module = _load_generated(output, "test_dynamic_standalone_abstention")
    assert module.demo(1) == 1
    with pytest.raises(ValueError, match="AOT heuristic abstained"):
        module.demo(-1)
