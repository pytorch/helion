...
def _matmul_layernorm_bwd_dxdy(
    ...
    """Kernel extracted from the matmul+layernorm example to exercise non power-of-two hl.arange."""
...
class TestIndexing(RefEagerTestBase, TestCase):
    ...
    def test_hl_arange_non_pow2_matmul_layernorm(self):
        # Choose non power-of-two dimensions to stress hl.arange masking behaviour
        m, k, n = 5, 3, 7
        eps = 1e-5
        ...
        # Kernel call (cast buffers to match forward outputs)
        ...
        torch.testing.assert_close(grad_x, ref_grad_x, rtol=1e-2, atol=2e-3)
        torch.testing.assert_close(grad_y, ref_grad_y, rtol=1e-2, atol=2e-3)
        self.assertExpectedJournal(code)
