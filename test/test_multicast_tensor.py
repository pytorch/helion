...
class TestMulticastTensor(TestCase):
    def test_multicast_load_grid(self):
        @helion.kernel
        ...
        code, result = code_and_output(
            multicast_load_kernel, (tensor_ptrs, tensor_list[0])
        )
    ...
    def test_multicast_load_2d_tensors(self):
        ...
        code, result = code_and_output(
            multicast_load_kernel, (tensor_ptrs, tensor_list[0]), block_size=[4, 4]
        )
        torch.testing.assert_close(result, torch.stack(tensor_list))
    ...
    def test_multicast_load_2d_dev_ptrs(self):
        ...
        code_batched, result = code_and_output(
            multicast_load_kernel_2d, (tensor_ptrs, tensor_list[0])
        )
        torch.testing.assert_close(result, torch.stack(tensor_list).reshape(4, 4, -1))

        @helion.kernel
        def multicast_load_2d_looped(
            dev_ptrs: torch.Tensor,
            example_tensor: torch.Tensor,
        ) -> torch.Tensor:
            M1, M2 = dev_ptrs.size()
        ...
        code_looped, result = code_and_output(
            multicast_load_2d_looped, (tensor_ptrs, tensor_list[0])
        )
        torch.testing.assert_close(result, torch.stack(tensor_list).reshape(4, 4, -1))
        self.assertExpectedJournal(code_batched + code_looped)

    def test_multicast_mask(self):
        @helion.kernel
        def multicast_load_w_mask(
            dev_ptrs: torch.Tensor,
            example_tensor: torch.Tensor,
        ) -> torch.Tensor:
            M = dev_ptrs.size(0)
            ...
            for tile in hl.tile(N, block_size=4):
                for multicast_tile in hl.tile(M, block_size=4):
                    ptr_tile = dev_ptrs[multicast_tile]
                    tensors = hl.multicast_like(example_tensor, ptr_tile)
                    out[:, tile] = tensors[tile]
            return out
        ...
        code, result = code_and_output(
            multicast_load_w_mask, (tensor_ptrs, tensor_list[0])
        )
        torch.testing.assert_close(result, torch.stack(tensor_list))
        self.assertExpectedJournal(code)

    def test_multicast_store_grid(self):
        @helion.kernel
        def multicast_store_kernel(
            x: torch.Tensor,
            dev_ptrs: torch.Tensor,
            example_tensor: torch.Tensor,
        ) -> None:
        ...
        code, result = code_and_output(
            multicast_store_kernel, (x, tensor_ptrs, tensor_list[0])
        )
    ...
    def test_multicast_store_broadcast_masked(self):
        @helion.kernel
        def multicast_store_kernel(
            x: torch.Tensor,
            dev_ptrs: torch.Tensor,
    ...
    def test_multicast_store_scatter(self):
        @helion.kernel
        def multicast_store_arange_kernel(
            dev_ptrs: torch.Tensor,
            example_tensor: torch.Tensor,
        ) -> None:
            N = example_tensor.size(0)
        ...
        code, result = code_and_output(
            multicast_store_arange_kernel, (tensor_ptrs, tensor_list[0])
        )
