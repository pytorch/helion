...
class TestShapeBucketing(RefEagerTestBase, TestCase):
    ...
    @skipIfRefEager("specialization keys not relevant in ref eager mode")
    def test_invalid_static_shapes_raises(self) -> None:
        """Test that invalid static_shapes values raise ValueError."""
        with self.assertRaises(ValueError):
            Settings(static_shapes="invalid")
        with self.assertRaises(ValueError):
            Settings(static_shapes="dynamic")

    @skipIfRefEager("specialization keys not relevant in ref eager mode")
    def test_setattr_normalization(self) -> None:
        """Test that direct assignment normalizes static_shapes."""
        s = Settings()
        s.static_shapes = False
        self.assertEqual(s.static_shapes, "ones")
        s.static_shapes = True
        self.assertEqual(s.static_shapes, "all")
        s.static_shapes = "none"
        self.assertEqual(s.static_shapes, "none")

    @skipIfRefEager("specialization keys not relevant in ref eager mode")
    def test_env_var_static_shapes_none(self) -> None:
        """Test HELION_STATIC_SHAPES=none env var."""
        import os
        from unittest.mock import patch

        with patch.dict(os.environ, {"HELION_STATIC_SHAPES": "none"}):
            s = Settings()
            self.assertEqual(s.static_shapes, "none")

    @skipIfNotCUDA()
    @skipIfRefEager("specialization keys not relevant in ref eager mode")
    def test_matmul_none_mode_correctness(self) -> None:
        """Test matmul in 'none' mode covers matmul_utils.py code paths.

        Exercises _resolve_dim_size early-return for "none" mode and
        emit_tl_dot_with_padding min_dot_size registration.
        """

        def simple_matmul(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
            m, k = x.size()
            n = y.size(1)
        ...
        k = self._make_kernel(simple_matmul, "none")

        # Normal shapes
        x = torch.randn(32, 64, device=DEVICE, dtype=torch.float32)
        y = torch.randn(64, 32, device=DEVICE, dtype=torch.float32)
        result = k(x, y)
        expected = torch.matmul(x, y)
        torch.testing.assert_close(result, expected, rtol=1e-2, atol=1e-2)

        # Shape with dim=1 (reuses same kernel in "none" mode)
        x1 = torch.randn(1, 64, device=DEVICE, dtype=torch.float32)
        y1 = torch.randn(64, 32, device=DEVICE, dtype=torch.float32)
        result1 = k(x1, y1)
        expected1 = torch.matmul(x1, y1)
        torch.testing.assert_close(result1, expected1, rtol=1e-2, atol=1e-2)
