...
class TestCuteLowerings(unittest.TestCase):
    ...
    def test_mma_operand_matrix_major_tracks_source_order(self) -> None:
        ...
        with self.assertRaisesRegex(
            AssertionError,
            "source_to_logical_order must swap exactly the two trailing "
            "matrix axes",
        ):
            dataclasses.replace(
                direct,
        ...
        for n in (128, 384):
            for name, kernel in (
                ...
                with self.subTest(n=n, rhs=name):
                    ...
                    logical_y = torch.randn(
                        128, n, device=DEVICE, dtype=torch.bfloat16
                    )
