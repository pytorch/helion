...
class TestPrologueFusionBasic(TestCase):
    ...
    def test_matmul_relu_x_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with relu fused as prologue, got {kernel_count}",
        )
        self.assertTrue(
    ...
    def test_matmul_sigmoid_x_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with sigmoid fused as prologue, got {kernel_count}",
        )
        self.assertTrue(
    ...
    def test_matmul_scalar_mul_x_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with scalar multiply fused as prologue, got {kernel_count}",
        )
...
class TestPrologueFusionWithClosure(TestCase):
    ...
    def test_matmul_add_bias_x_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with bias add fused as prologue, got {kernel_count}",
        )
        self.assertIn(
            "prologue_closure",
            all_code,
            "Expected bias tensor (prologue_closure) in Triton kernel",
        )
    ...
    def test_matmul_mul_scale_x_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with scale multiply fused as prologue, got {kernel_count}",
        )
        self.assertIn(
            "prologue_closure",
            all_code,
            "Expected scale tensor (prologue_closure) in Triton kernel",
        )
...
class TestPrologueFusionBothInputs(TestCase):
    ...
    def test_matmul_relu_both_inputs(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with relu on both inputs fused as prologue, got {kernel_count}",
        )
        self.assertTrue(
            "triton_helpers.maximum" in all_code or "tl.maximum" in all_code,
            "Expected relu (maximum) fused into Triton kernel",
...
class TestPrologueEpilogueCombined(TestCase):
    ...
    def test_matmul_relu_prologue_sigmoid_epilogue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with relu prologue and sigmoid epilogue fused, got {kernel_count}",
        )
        self.assertTrue(
            "triton_helpers.maximum" in all_code or "tl.maximum" in all_code,
            "Expected relu (maximum) fused into Triton kernel",
    ...
    def test_matmul_scale_prologue_bias_epilogue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with scale prologue and bias epilogue fused, got {kernel_count}",
        )
        self.assertIn(
            "prologue_closure",
            all_code,
...
class TestPrologueFusionRequiresProperIndexing(TestCase):
    ...
    def test_matmul_row_scale_x_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with row-wise scale fused as prologue, got {kernel_count}",
        )
        self.assertIn(
            "prologue_closure",
            all_code,
            "Expected row_scale tensor (prologue_closure) in Triton kernel",
        )
    ...
    def test_matmul_col_scale_x_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with column-wise scale fused as prologue, got {kernel_count}",
        )
        self.assertIn(
            "prologue_closure",
            all_code,
            "Expected col_scale tensor (prologue_closure) in Triton kernel",
        )
    ...
    def test_matmul_row_scale_y_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with row-wise scale on y fused as prologue, got {kernel_count}",
        )
        self.assertIn(
            "prologue_closure",
            all_code,
    ...
    def test_matmul_col_scale_y_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with column-wise scale on y fused as prologue, got {kernel_count}",
        )
        self.assertIn(
            "prologue_closure",
            all_code,
    ...
    def test_matmul_different_scales_both_inputs(self):
        ...
        compiled_result, source_codes = run_and_get_code(
            compiled_f, x, y, scale_x, scale_y
        )
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with scales on both inputs fused as prologue, got {kernel_count}",
        )
        self.assertIn(
            "prologue_closure",
            all_code,
            "Expected scale tensors (prologue_closure) in Triton kernel",
        )
    ...
    def test_matmul_2d_transform_x_prologue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with 2D tanh fused as prologue, got {kernel_count}",
        )
        self.assertIn(
            "libdevice",
            all_code.lower(),
            "Expected tanh (libdevice) fused into Triton kernel",
        )
...
class TestPrologueFusionQuantization(TestCase):
    ...
    def test_matmul_dequant_x_prologue(self):
        ...
        compiled_f = torch.compile(f, fullgraph=True, backend="inductor")
        compiled_result, source_codes = run_and_get_code(
            compiled_f, x_int, y, scale, zero_point
        )
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with dequantization fused as prologue, got {kernel_count}",
        )
        # Scale is loaded from prologue_closure tensor, not embedded as constant
        self.assertIn(
            "prologue_closure",
            all_code,
            "Expected scale tensor (prologue_closure) in Triton kernel",
