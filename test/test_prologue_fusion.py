...
def matmul_single_prologue(
    x: torch.Tensor,
    y: torch.Tensor,
) -> torch.Tensor:
    """Simpler matmul - prologue fusion happens automatically."""
    m, k = x.size()
    k2, n = y.size()
    assert k == k2
    out = torch.empty(
...
class TestPrologueFusionBasic(TestCase):
    ...
    def test_matmul_relu_x_prologue(self):
        ...
        def f(x, y):
            ...
            return matmul_single_prologue(torch.relu(x), y)
        ...
        torch.testing.assert_close(eager_result, baseline, rtol=1e-2, atol=1e-2)
    ...
    def test_matmul_sigmoid_x_prologue(self):
        ...
        def f(x, y):
            return matmul_single_prologue(torch.sigmoid(x), y)
    ...
    def test_matmul_scalar_mul_x_prologue(self):
        ...
        def f(x, y):
            return matmul_single_prologue(x * 2.0, y)
...
class TestPrologueFusionWithClosure(TestCase):
    ...
    def test_matmul_add_bias_x_prologue(self):
        ...
        def f(x, y, bias):
            ...
            return matmul_single_prologue(x + bias, y)
        ...
        self.assertIn("prologue_closure", all_code, "Expected bias tensor (prologue_closure) in Triton kernel")
    ...
    def test_matmul_mul_scale_x_prologue(self):
        ...
        def f(x, y, scale):
            return matmul_single_prologue(x * scale, y)
        ...
        self.assertIn("prologue_closure", all_code, "Expected scale tensor (prologue_closure) in Triton kernel")
...
class TestPrologueFusionBothInputs(TestCase):
    ...
    def test_matmul_relu_both_inputs(self):
        ...
        baseline = torch.relu(x) @ torch.relu(y)
        torch.testing.assert_close(eager_result, baseline, rtol=1e-2, atol=1e-2)
        torch.testing.assert_close(compiled_result, baseline, rtol=1e-2, atol=1e-2)
...
class TestPrologueEpilogueCombined(TestCase):
    ...
    def test_matmul_relu_prologue_sigmoid_epilogue(self):
        ...
        def f(x, y):
            ...
            return torch.sigmoid(matmul_single_prologue(torch.relu(x), y))

        eager_result = f(x, y)
        compiled_f = torch.compile(f, fullgraph=True, backend="inductor")
        compiled_result, source_codes = run_and_get_code(compiled_f, x, y)

        baseline = torch.sigmoid(torch.relu(x) @ y)
        torch.testing.assert_close(eager_result, baseline, rtol=1e-2, atol=1e-2)
        torch.testing.assert_close(compiled_result, baseline, rtol=1e-2, atol=1e-2)
    ...
    def test_matmul_scale_prologue_bias_epilogue(self):
        ...
        self.assertIn("prologue_closure", all_code, "Expected scale tensor (prologue_closure) in Triton kernel")
        self.assertIn("epilogue_closure", all_code, "Expected bias tensor (epilogue_closure) in Triton kernel")
...
class TestPrologueFusionRequiresProperIndexing(TestCase):
    ...
    def test_matmul_row_scale_x_prologue(self):
        ...
        def f(x, y, row_scale):
            ...
            return matmul_single_prologue(x * row_scale.unsqueeze(1), y)
        ...
        self.assertIn("prologue_closure", all_code, "Expected row_scale tensor (prologue_closure) in Triton kernel")
    ...
    def test_matmul_row_scale_y_prologue(self):
        ...
        def f(x, y, row_scale):
            ...
            return matmul_single_prologue(x, y * row_scale.unsqueeze(1))

        eager_result = f(x, y, row_scale)
        compiled_f = torch.compile(f, fullgraph=True, backend="inductor")
        compiled_result, source_codes = run_and_get_code(compiled_f, x, y, row_scale)

        baseline = x @ (y * row_scale.unsqueeze(1))
        torch.testing.assert_close(eager_result, baseline, rtol=1e-2, atol=1e-2)
        torch.testing.assert_close(compiled_result, baseline, rtol=1e-2, atol=1e-2)
    ...
    def test_matmul_col_scale_y_prologue(self):
        ...
        def f(x, y, col_scale):
            ...
            return matmul_single_prologue(x, y * col_scale)
    ...
    def test_matmul_different_scales_both_inputs(self):
        ...
        self.assertIn("prologue_closure", all_code, "Expected scale tensors (prologue_closure) in Triton kernel")
    ...
    def test_matmul_2d_transform_x_prologue(self):
        ...
        def f(x, y):
            return matmul_single_prologue(torch.tanh(x), y)
...
class TestPrologueFusionQuantization(TestCase):
    ...
    def test_matmul_dequant_x_prologue(self):
        ...
        def f(x_int, y, scale, zero_point):
            ...
            return matmul_single_prologue(x_dequant, y)
