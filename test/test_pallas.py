...
def pallas_chunked_major_dim_sum(x: torch.Tensor) -> torch.Tensor:
    ...
    for tile_b, tile_c, tile_h, tile_m in hl.tile(
        [batch, nchunks, nheads, headdim], block_size=[1, 1, 8, None]
    ):
        acc = hl.zeros([tile_h, tile_m], dtype=torch.float32)
        for tile_k in hl.tile(chunk_size, block_size=64):
            row = tile_k.index + tile_c.begin * chunk_size
            values = x[tile_b.begin, row, tile_h, tile_m]
            acc = acc + values.to(torch.float32).sum(dim=0)
        out[tile_b.begin, tile_c.begin, tile_h, tile_m] = acc.to(x.dtype)
    return out
...
class TestPallas(TestCase):
    ...
    def test_estimate_pallas_vmem_bytes(self) -> None:
        ...
        try:
            ...
            if "Ran out of memory in memory space vmem" not in str(e):
                raise
    ...
    def test_fori_loop_last_two_dim_ragged_store_uses_clamped_dma_writeback(
        self,
    ) -> None:
        """A fori_loop ragged store in a sublane dim uses exact DMA writeback.
        ...
        def ragged_last_two_dim(
            ...
            for g in hl.grid(n):
                ...
                for tile in hl.tile(st, en):
                    out[tile, :] = x[tile, :] + g * 4.0 + 1.0
            return out

        x = torch.randn(8, 128, device=DEVICE, dtype=torch.float32)
        # Process the upper segment first so an unclamped write from the lower
        # segment would visibly overwrite it instead of being repaired later.
        starts = torch.tensor([4, 0], dtype=torch.int32, device=DEVICE)
        ends = torch.tensor([8, 4], dtype=torch.int32, device=DEVICE)
        code, result = code_and_output(
            ragged_last_two_dim,
            (x, starts, ends),
            block_sizes=[8],
        ...
        self.assertIsNotNone(match, "expected dynamic-begin pl.ds pad dims")
        pad_dims = ast.literal_eval(match.group(1))
        self.assertIn((2, 0, 8, 7), pad_dims)
        if is_pallas_interpret():
            self.assertIn("def _copy_out_body", code)
            self.assertIn("pl.ds(_copy_row", code)
        else:
            self.assertNotIn("def _copy_out_body", code)
        expected = x.clone()
        expected[:4] += 5.0
        expected[4:] += 1.0
        torch.testing.assert_close(result, expected)
    ...
    def test_fori_loop_unaligned_dynamic_begin_ds_load_gets_extra_pad(self) -> None:
        ...
        def ragged_copy(
            ...
            for g in hl.grid(n):
                ...
                for tile_m in hl.tile(0, m):
                    for tile in hl.tile(st, en):
                        out[tile, tile_m] = x[tile, tile_m] + g * 4.0 + 1.0
            return out

        x = torch.arange(527 * 8, device=DEVICE, dtype=torch.float32).reshape(527, 8)
        starts = torch.tensor([520, 0], dtype=torch.int32, device=DEVICE)
        ends = torch.tensor([527, 520], dtype=torch.int32, device=DEVICE)
        code, result = code_and_output(
            ragged_copy,
            (x, starts, ends),
            block_sizes=[8, 16],
        ...
        self.assertIn((3, 0, 16, 15), pad_dims)
        expected = x.clone()
        expected[:520] += 5.0
        expected[520:] += 1.0
        torch.testing.assert_close(result, expected)
    ...
    def test_fori_loop_last_dim_output_only_ragged_store_rejected(self) -> None:
        @helion.kernel(backend="pallas", static_shapes=True)
        def ragged_last_dim(
            x: torch.Tensor, starts: torch.Tensor, ends: torch.Tensor
        ) -> torch.Tensor:
            out = torch.empty_like(x)
            for g in hl.grid(starts.size(0)):
                st = starts[g]
                en = ends[g]
                for tile in hl.tile(st, en):
                    out[:, tile] = x[:, tile] + 1.0
        ...
        x = torch.randn(8, 128, device=DEVICE, dtype=torch.float32)
        starts = torch.tensor([64, 0], dtype=torch.int32, device=DEVICE)
        ends = torch.tensor([128, 64], dtype=torch.int32, device=DEVICE)
        with self.assertRaisesRegex(helion.exc.BackendUnsupported, "lane/sublane"):
            code_and_output(
                ragged_last_dim,
                (x, starts, ends),
    ...
    def test_fori_loop_nested_partial_last_two_dim_scratch_store_rejected(
        self,
    ) -> None:
        """A nested partial store cannot widen inside an enclosing DMA scratch."""

        @helion.kernel(backend="pallas", static_shapes=True)
        def nested_partial(x: torch.Tensor) -> torch.Tensor:
            b, _m, _n = x.size()
            out = torch.empty_like(x)
            for _program in hl.grid(1):
                for tile_b in hl.tile(b, block_size=1):
                    out[tile_b, :, :] = x[tile_b, :, :] + 10.0
                    for tile_m in hl.tile(0, 4, block_size=8):
                        out[tile_b, tile_m, :] = x[tile_b, tile_m, :] + 20.0
            return out

        x = torch.arange(8 * 128, device=DEVICE, dtype=torch.float32).reshape(1, 8, 128)
        with self.assertRaisesRegex(helion.exc.BackendUnsupported, "lane/sublane"):
            nested_partial.bind((x,)).to_code(
    ...
    def test_bool_leading_singleton_broadcast_where_fori_loop_dtypes(self) -> None:
        @helion.kernel(backend="pallas", static_shapes=True)
        ...
        for dtype in (torch.float32, torch.bfloat16):
            with self.subTest(dtype=dtype):
                x = torch.randn(8, 128, 128, device=DEVICE, dtype=dtype)
                code, result = code_and_output(
                ...
                expected = torch.where(
                    (rows >= cols)[None, :, :], x, torch.zeros_like(x)
                )
                torch.testing.assert_close(result, expected)
                self.assertIn("jax.lax.fori_loop", code)
                self._assert_numeric_mask_reshape(code, r"\[1,")
    ...
    def test_chunked_major_dim_offset_load_keeps_dynamic_ds(self) -> None:
        x = torch.randn(2, 256, 8, 32, device=DEVICE, dtype=torch.bfloat16)
        code, result = code_and_output(
            pallas_chunked_major_dim_sum,
            (x,),
            block_sizes=[32],
            pallas_loop_type="emit_pipeline",
        )
        ...
        expected = x.float().reshape(2, 4, 64, 8, 32).sum(dim=2).to(x.dtype)
        torch.testing.assert_close(result, expected)
    ...
    def test_promoted_mixed_scalar_slice_vmem_load(self) -> None:
        ...
        self.assertIn("pltpu.roll(jnp.pad(lax.convert_element_type(x[", code)
        self.assertIn(", jnp.float32)", code)

    def test_indirect_gather_widens_small_scalar_head_dim(self) -> None:
        x = torch.randn(32, 8, 32, device=DEVICE, dtype=torch.bfloat16)
        offsets = torch.tensor(
            list(range(15, -1, -1)), device=DEVICE, dtype=torch.int32
        )
        code, result = code_and_output(
            pallas_indirect_gather_small_head_dim,
            (x, offsets),
            block_sizes=[16, 32],
        )
        self.assertIn("x[:, :, :]", code)
        self.assertIsNone(re.search(r"x\[:, offset_\d+, :\]", code))
        expected = x[offsets].permute(1, 0, 2)
        torch.testing.assert_close(result, expected)
    ...
    def test_fori_loop_outer_grid_partial_tile_is_padded(self) -> None:
        @helion.kernel(backend="pallas", static_shapes=True)
        def outer_tail_dma(x: torch.Tensor) -> torch.Tensor:
            m, n = x.size()
            out = torch.empty_like(x)
            for tile_m in hl.tile(m):
                for tile_n in hl.tile(n):
        ...
        x = torch.randn(10, 256, device=DEVICE, dtype=torch.float32)
        code, result = code_and_output(
            outer_tail_dma,
            (x,),
            block_sizes=[8, 128],
        ...
        match = re.search(r"_ds_pad_dims=(\[[^\]]*\])", code)
        self.assertIsNotNone(match, "expected outer-grid pl.ds pad metadata")
        pad_dims = ast.literal_eval(match.group(1))
        self.assertIn((0, 0, 8, 0), pad_dims)
        self.assertIn((1, 0, 8, 0), pad_dims)
        torch.testing.assert_close(result, x + 1.0)
    ...
    def test_unaligned_small_vector_load_selects_from_full_ref(self) -> None:
        ...
        offsets = torch.arange(129, device=DEVICE, dtype=torch.int32) ** 2
        _code, result = code_and_output(shifted_offsets, (offsets,), block_sizes=[16])
        expected = offsets[1:] - offsets[:-1]
        torch.testing.assert_close(result, expected)
    ...
    def test_scalar_tensor_predicate_if(self) -> None:
        ...
        y = x + 10.0
        gates = torch.tensor([1], device=DEVICE, dtype=torch.int32)
        ...
        gates = torch.tensor([0], device=DEVICE, dtype=torch.int32)
        _, result = code_and_output(
            scalar_tensor_predicate_if,
            (x, y, gates),
            block_sizes=[8],
    ...
    def test_numeric_single_element_tensor_predicate_if(self) -> None:
        ...
        gates = torch.tensor([0.5], device=DEVICE, dtype=torch.float32)
        code, result = code_and_output(
    ...
    def test_jagged_tile_alignment_caps_to_accessed_tensor_dim(self) -> None:
        ...
        def normalized_block_sizes(max_m: int) -> list[int]:
        ...
        small_block_sizes = normalized_block_sizes(8)
        self.assertEqual(small_block_sizes, [32, 8])

        large_block_sizes = normalized_block_sizes(256)
