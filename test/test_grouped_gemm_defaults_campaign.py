...
def test_public_cli_requires_external_output_directory() -> None:
    ...
    assert benchmark.HELION_SELECTIONS == ("compiler_heuristic", "live_autotune")
    with pytest.raises(SystemExit):
        parser.parse_args(("--helion-selection", "final_reviewed_aot"))
...
@pytest.mark.parametrize(
    ("selection", "skip_cache"),
    (
        ("compiler_heuristic", False),
        ("live_autotune", True),
    ),
)
def test_worker_environment_sets_mode_and_fresh_caches(
    ...
    assert environment["HELION_AOT_MODE"] == "disabled"
...
def test_fixed_summary_requires_same_config_across_providers() -> None:
    fixed_selection = "compiler_heuristic"
    results = [
        _result(
            "quack",
            0,
