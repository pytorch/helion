...
from test.test_cute_chained_tcgen05 import _cpu_support  # noqa: F401
from test.test_cute_chained_tcgen05 import _tcgen_config_bound
...
def test_real_specs_keep_seed_multisets_defaults_and_breadth(
    ...
    bound = _tcgen_config_bound(n=n, row_block=row_block, scan=scan)
    with bound.env:
...
def test_population_budget_and_explicit_user_priority_unchanged(size: int) -> None:
    bound = _tcgen_config_bound(n=64)
    with bound.env:
