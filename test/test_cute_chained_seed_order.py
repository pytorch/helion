    ...
    return Config.from_dict(
        {
            ...
            "cute_chained_c_smem_padding": 4 if child & 4 else 0,
        }
    )


def _parent(seed: Config) -> tuple[str, int, tuple[int, ...], str, int]:
    return (
        cast("str", seed.config["cute_chained_mma_schedule"]),
        seed.num_warps,
        tuple(seed.block_sizes),
        seed.pid_type,
        cast("int", seed.config.get("cute_chained_c_smem_padding", 0)),
    )
...
def test_hierarchy_visits_all_parents_before_optional_children() -> None:
    seeds = list(
        itertools.starmap(
            _seed,
            itertools.product(
                ("coalesced", "tcgen05_tmem"), (4, 8), (32, 64), range(8)
            ),
        )
    )
    ...
    expected = [
        (family, warps, (128, width), "flat", padding)
        for width, padding, warps, family in itertools.product(
            (32, 64), (0, 4), (4, 8), ("coalesced", "tcgen05_tmem")
        )
    ]
    assert [_parent(seed) for seed in ordered[:16]] == expected
    ...
    assert [_parent(seed) for seed in ordered[16:32]] == expected


def test_uneven_families_and_pid_geometry_are_not_dropped() -> None:
    seeds = [_seed("coalesced", 4, 32, child) for child in range(8)] + [
        _seed("coalesced", 4, 32, pid="persistent"),
        _seed("tcgen05_tmem", 4, 64),
        _seed("cp_async", 8, 16),
    ]
    ordered = order_chained_seed_configs(seeds)
    assert ordered[:5] == [seeds[0], seeds[9], seeds[10], seeds[4], seeds[8]]
    assert Counter(ordered) == Counter(seeds)


def test_omitted_padding_and_explicit_zero_share_a_parent() -> None:
    omitted = Config(
        block_sizes=[128, 32], num_warps=4, cute_chained_mma_schedule="tcgen05_tmem"
    )
    zero = _seed("tcgen05_tmem", 4, 32)
    padded = _seed("tcgen05_tmem", 4, 32, 4)
    assert order_chained_seed_configs([omitted, zero, padded]) == [
        omitted,
        padded,
        zero,
    ]
    ...
    with bound.env:
        ...
        try:
            ...
            tcgen = [
                ...
                if config.config["cute_chained_mma_schedule"] == "tcgen05_tmem"
            ]
            assert {
                config.config["cute_chained_c_smem_padding"] for config in tcgen
            } == {0, 4}
            wanted = Config(
