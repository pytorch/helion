...
@pytest.mark.parametrize("static_shapes", [False, True])
def test_scan_preserves_alias_specialization_and_codegen(static_shapes: bool) -> None:
...
def test_nested_scan_registers_alias_facts_before_codegen() -> None:
    ...
    assert "scan_warp_prefix" not in bound.to_code(bound.config_spec.default_config())
