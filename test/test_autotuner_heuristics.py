...
class TestTritonStandardReductionHeuristic(TestCase):
    ...
    def test_seed_is_persistent_one_row(self) -> None:
        ...
        # env is a mock, so its device inherits the test lane's DEVICE (e.g. "cpu" on the
        # pallas lane); patch get_num_sm — the Triton reduction heuristic only ever fires on
        # GPU in production, and the SM count does not affect these assertions.
        with (
...
class TestCuteTcgen05ClusterM2Heuristic(TestCase):
    ...
    def test_cute_tcgen05_ab_stages_budget_gate(self) -> None:
        ...
        # compiles + runs, shapeT16 639.7 / shapeT2 460.1 TF). The SMEM budget is
        # MOCKED to the B200 value so the gate is deterministic on any cute host.
        b200_budget = 232448 - 28 * 1024  # optin cap - ab=3 reservation
...
class TestTritonReductionHeuristic(TestCase):
    ...
    def test_t1_reduction_then_normalize_loop_widens_tile(self) -> None:
        ...
        # No curriculum kernel has this shape, so it pins the standard
        ...
        def check(m: int, n: int, expect_looped: bool) -> None:
            ...
            if expect_looped:
                ...
                # P3 (m_block==1 apply cap): the normalize tile is clamped to the SAME
                # ÷M_BLOCK register-resident footprint as the reduction tile, NOT left at
                ...
                # left this tile uncapped at 131072).
