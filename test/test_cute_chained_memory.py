...
def _reference(a: torch.Tensor, b: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
...
def _cpu_code(
    ...
    *,
    ...
    with (
        ...
        kernel.reset()
        return kernel.bind(args).to_code(_config(warps, schedule, full=full))
...
def test_async_memory_codegen_patches_already_imported_support() -> None:
    with patch.object(
        ...
        code = _cpu_code(_memory_chain, _inputs("contiguous", "cpu"), 4)
...
def test_async_memory_codegen_guards(
    ...
    code = _cpu_code(_memory_chain, _inputs(layout, "cpu"), warps)
...
def test_async_memory_codegen_tail_bounds(warps: int) -> None:
    code = _cpu_code(_memory_chain, _inputs("contiguous", "cpu", tails=True), warps)
...
def test_async_memory_codegen_computed_and_gathered_fallback(warps: int) -> None:
    args = _inputs("contiguous", "cpu")
    code = _cpu_code(_computed_memory_chain, args, warps)
    assert "chain_0_mma" in code
    assert "CopyG2SOp" not in code
    gathered = (*args, torch.randperm(64), torch.randperm(64))
    code = _cpu_code(_gathered_memory_chain, gathered, warps)
    assert "chain_0_mma" in code
    assert "CopyG2SOp" not in code
...
def test_async_memory_views(layout: str, warps: int) -> None:
    args = _inputs(layout, DEVICE)
    _memory_chain.reset()
    fn = _memory_chain.bind(args).compile_config(_config(warps))
    _check(fn, args, _reference(*args))


@pytest.mark.parametrize("warps", [1, 2, 4])
def test_async_memory_alignment_is_checked_on_every_call(warps: int) -> None:
    aligned = _inputs("contiguous", DEVICE)
    _memory_chain.reset()
    fn = _memory_chain.bind(aligned).compile_config(_config(warps))
    _check(fn, aligned, _reference(*aligned))
    for offset in (1, 8):
        args = _inputs(f"offset{offset}", DEVICE)
        assert all(
...
def test_register_bridge_codegen_exclusive_region(warps: int) -> None:
    ...
    for kernel, values in (
        ...
        code = _cpu_code(kernel, values, warps, "cp_async_register", full=True)
...
def test_register_bridge_codegen_narrow_producer(warps: int) -> None:
    ...
    code = _cpu_code(_memory_chain, args, warps, "cp_async_register")
...
def test_register_bridge_exclusive_region(warps: int) -> None:
    ...
    for kernel, args, expected in cases:
        ...
        fn = kernel.bind(args).compile_config(
            _config(warps, "cp_async_register", full=True)
        )
        _check(fn, args, expected)


@pytest.mark.parametrize("warps", [1, 2, 4])
def test_register_bridge_tail_tiles(warps: int) -> None:
    args = _inputs("contiguous", DEVICE, tails=True)
    _memory_chain.reset()
    fn = _memory_chain.bind(args).compile_config(_config(warps, "cp_async_register"))
    _check(fn, args, _reference(*args))
...
def test_async_memory_computed_and_gathered_fallback(warps: int) -> None:
    args = _inputs("contiguous", DEVICE)
    _computed_memory_chain.reset()
    fn = _computed_memory_chain.bind(args).compile_config(_config(warps))
    transformed = tuple(
        (value.float() * scale).to(value.dtype)
        for value, scale in zip(args, (0.5, 0.75, 0.25), strict=True)
    )
    _check(fn, args, _reference(*transformed))
    pk, pq = (torch.randperm(64, device=DEVICE) for _ in range(2))
    gathered = (*args, pk, pq)
    _gathered_memory_chain.reset()
    fn = _gathered_memory_chain.bind(gathered).compile_config(_config(warps))
    a, b, v = args
    _check(fn, gathered, _reference(a[..., pk], b[..., pk], v[:, pq, :]))
