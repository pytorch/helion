...
SAME eviction slot under persistent and looped configs (the soundness payoff). They bind on ``DEVICE``
and inspect the slot map + generated Triton (no kernel execution); triton backend, CUDA only.
"""
...
def _dev(*shape: int, dtype: torch.dtype = torch.float32) -> torch.Tensor:
    return torch.empty(list(shape), dtype=dtype, device=DEVICE)
...
class TestMemoryOpSlots(RefEagerTestBase, TestCase):
    def test_slot_map_built(self):
        bk = reread_reduce.bind((_dev(256, 512),))
    ...
    def test_static_unroll_shares_one_indexing_slot(self):
        bk = static_unroll.bind((_dev(256, 3),))
        slot_map = bk.config_spec.mem_op_slot_map
    ...
    def test_tuple_buffers_distinct_slots_no_collision(self):
        ...
        bk = tuple_buffers.bind((tuple(_dev(256, 512) for _ in range(3)),))
        slot_map = bk.config_spec.mem_op_slot_map
        ws_slots = {
            slot
    ...
    def test_uniform_membership_stack_load_inert_slot(self):
        ...
        dev_ptrs = _dev(4, dtype=torch.uint64)
        example = _dev(8, dtype=torch.bfloat16)
