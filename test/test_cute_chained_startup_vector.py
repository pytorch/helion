...
@pytest.mark.parametrize("option", ["m64", "host"])
def test_startup_m64_and_host_domains(option):
    with _cpu_codegen():
        ...
        if option == "m64":
            ...
            config.config["cute_host_selected_fastpath"] = True
            with pytest.raises(exc.BackendUnsupported, match="wrapper"):
...
def test_startup_resource_charge_and_padding_are_additive():
    ...
    with _cpu_codegen():
        ...
        def record(plan, padding=0, *, startup=False):
            for amount in (0, 4):
                plain = chained_tcgen05._shared_memory_bytes(plan, amount)
                charged = chained_tcgen05._shared_memory_bytes(
                    plan, amount, startup=True
                )
                ...
                with patch.object(
                    ...
                    assert not original(plan, amount, startup=True)
                with patch.object(
                    chained_tcgen05.CuteTcgen05Config,
                    "per_cta_smem_capacity_bytes",
                    return_value=charged,
                ):
                    assert original(plan, amount, startup=True)
            assert chained_tcgen05._shared_memory_bytes(
                plan, 4, startup=True
            ) > chained_tcgen05._shared_memory_bytes(plan, 0, startup=True)
