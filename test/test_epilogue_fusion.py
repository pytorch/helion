...
from helion._testing import assert_single_triton_kernel
...
class TestEpilogueFusionTorchCompileExtended(TestCase):
    ...
    def test_matmul_scalar_multiply_epilogue(self):
        ...
        all_code = assert_single_triton_kernel(self, source_codes)
        self.assertTrue(
            "2.0" in all_code, "Expected scalar 2.0 multiply fused into Triton kernel"
        )
    ...
    def test_matmul_neg_epilogue(self):
        ...
        negation_fused = (
            ...
            or "= -" in all_code
            and "_epilogue_tmp" in all_code  # any epilogue negation
            or "(-" in all_code  # old pattern with parentheses
            or ", -v_"
            in all_code  # negation directly in store: tl.store(..., -v_0, ...)
        )
        self.assertTrue(negation_fused, "Expected negation fused into Triton kernel")
...
class TestEpilogueFusionRequiresProperIndexing(TestCase):
    ...
    def test_matmul_nonsquare_bias_epilogue(self):
        ...
        all_code = assert_single_triton_kernel(self, source_codes)
        self.assertTrue(
            "epilogue_closure" in all_code, "Expected bias tensor in Triton kernel"
        )
...
class TestMissingOperations(TestCase):
        ...
        for op_name, op_fn, code_pattern in self.SIMPLE_OPS:
            with self.subTest(op=op_name):
                ...
                torch.testing.assert_close(
                    compiled_result, eager_result, rtol=1e-3, atol=1e-3
                )
                ...
                self.assertEqual(
                    kernel_count,
                    1,
                    f"Expected 1 kernel with {op_name} fused, got {kernel_count}",
                )
    ...
    def test_matmul_reciprocal_epilogue(self):
        ...
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel with reciprocal fused, got {kernel_count}",
        )
...
class TestDtypeHandling(TestCase):
    ...
    FP32_REQUIRED_OPS: ClassVar[list] = [
    ...
    def test_fp16_ops_auto_upcast(self):
        ...
        for op_name, op_fn in self.FP32_REQUIRED_OPS:
            with self.subTest(op=op_name):
                x = (
                    torch.randn(64, 64, device=DEVICE, dtype=torch.float16).abs() + 0.1
                )  # abs+offset for log/sqrt
                y = torch.randn(64, 64, device=DEVICE, dtype=torch.float16)

                def f(x, y, op=op_fn):
                    return op(
                        matmul(x, y).abs() + 0.1
                    )  # abs+offset for log/sqrt safety
                ...
                self.assertEqual(
                    kernel_count,
                    1,
                    f"Expected 1 kernel with {op_name} fused via auto-upcast",
                )
    ...
    def test_bf16_ops_auto_upcast(self):
        ...
        for op_name, op_fn in self.FP32_REQUIRED_OPS:
            with self.subTest(op=op_name):
                ...
                # Correctness is required
                torch.testing.assert_close(
                    compiled_result, eager_result, rtol=1e-1, atol=1e-1
                )
...
class TestEdgeCases(TestCase):
    ...
    def test_matmul_chained_multiple_ops(self):
        ...
        kernel_count, all_code = count_triton_kernels(source_codes)
        self.assertEqual(
            kernel_count, 1, f"Expected 1 kernel with all ops fused, got {kernel_count}"
        )
    ...
    def test_matmul_where_epilogue(self):
        ...
        kernel_count, all_code = count_triton_kernels(source_codes)
        self.assertEqual(
            kernel_count, 1, f"Expected 1 kernel with where fused, got {kernel_count}"
        )
...
class TestMultiOutputKernel(TestCase):
    ...
    def test_multi_output_epilogue_first_only(self):
        ...
        out_eager, residual_eager = f(x, y)
        torch.testing.assert_close(
            out_eager, torch.relu(expected), rtol=1e-3, atol=1e-3
        )
        torch.testing.assert_close(residual_eager, expected, rtol=1e-3, atol=1e-3)
        ...
        out_compiled, residual_compiled = result
        torch.testing.assert_close(
            out_compiled, torch.relu(expected), rtol=1e-3, atol=1e-3
        )
        torch.testing.assert_close(residual_compiled, expected, rtol=1e-3, atol=1e-3)
    ...
    def test_multi_output_epilogue_both(self):
        ...
        torch.testing.assert_close(
            residual_eager, torch.sigmoid(expected), rtol=1e-3, atol=1e-3
        )
        ...
        torch.testing.assert_close(
            residual_compiled, torch.sigmoid(expected), rtol=1e-3, atol=1e-3
        )
    ...
    def test_multi_output_epilogue_combined(self):
        ...
        kernel_count, all_code = count_triton_kernels(source_codes)
        # Multi-output epilogue fusion: epilogue (out + residual) that reads from both
        # outputs is fused into the template as a post-store epilogue.
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel (multi-output fusion), got {kernel_count}",
        )
...
class TestAuxiliaryStores(TestCase):
    ...
    def test_atomic_kernel_with_epilogue(self):
        ...
        self.assertGreaterEqual(
            kernel_count, 2, "Expected separate kernels for atomic + epilogue"
        )
...
class TestMultiOutputEpilogueFusion(TestCase):
    ...
    def test_multi_output_combined_correctness(self):
        ...
        # Should produce 1 kernel - multi-output epilogue fusion is now supported
        kernel_count, all_code = count_triton_kernels(source_codes)
        self.assertEqual(
            kernel_count,
            1,
            f"Expected 1 kernel (fused), got {kernel_count}. "
            "Multi-output epilogue should fuse - it reads from multiple outputs "
            "but this is just an ordering constraint, not a circular dependency.",
        )
...
class TestSchedulerFilteredOps(TestCase):
    ...
    def test_matmul_gather_complex_epilogue(self):
        ...
        def f(x, y):
            ...
            return torch.gather(result, 1, indices)
