...
from ._cute_row_softmax_codegen import _bound
from ._cute_row_softmax_codegen import _config
from ._cute_row_softmax_codegen import _cpu_target
...
def test_actual_reduction_codegen_only_changes_boolean_tree_and_source_hash():
    width, threads = 4096, 512
    config = _config(width=width, threads=threads, rows=2, vector=8)
    with _cpu_target():
