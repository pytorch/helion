...
class TestViews(RefEagerTestBase, TestCase):
    ...
    def test_int4_gemm(self):
        ...
        def matmul_bf16_int4(A: Tensor, B: Tensor, C: Tensor) -> Tensor:
            ...
            b_bf16 = torch.empty([block_size_k_packed, block_size_n, 2], dtype=torch.bfloat16, device=A.device)
            ...
            for tile_m in hl.tile(M):
                for tile_n in hl.tile(N, block_size=block_size_n):
                    ...
                    for tile_k_packed in hl.tile(K // 2, block_size=block_size_k_packed):
                        ...
                        b_lo_bf16 = b_lo.to(torch.bfloat16).unsqueeze(-1)  # [BLOCK_SIZE_K//2, BLOCK_SIZE_N, 1]
                        b_hi_bf16 = b_hi.to(torch.bfloat16).unsqueeze(-1)  # [BLOCK_SIZE_K//2, BLOCK_SIZE_N, 1]
                        b_bf16[:, :, 0] = b_lo_bf16
                        b_bf16[:, :, 1] = b_hi_bf16

                        # Permute to [BLOCK_SIZE_K//2, 2, BLOCK_SIZE_N]
                        b_bf16 = b_bf16.permute(0, 2, 1)
                        
                        # Reshape to [BLOCK_SIZE_K, BLOCK_SIZE_N] - unpacking the int4 values
                        b_bf16 = b_bf16.reshape([tile_k_packed.block_size * 2, tile_n.block_size])
                        ...
                        acc = acc + hl.dot(a_tile, b_bf16).to(torch.bfloat16)  # [BLOCK_SIZE_M, BLOCK_SIZE_N]
