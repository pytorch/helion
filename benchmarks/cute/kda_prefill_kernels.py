...
def _block_inverse(lower: torch.Tensor) -> torch.Tensor:
    """Invert I+lower via nilpotent diagonal powers and a coupling product."""
...
def kda_chunk_recurrence_fp32(
    ...
    """Five-factor recurrence with immutable FP32 input and FP32 state updates.

    Matrix operands use BF16 state copies. The projected state is rounded to
    BF16 before the BF16 residual subtraction. Invalid token residuals are zero.
    """
    ...
    for tile_sequence, tile_head, tile_v in hl.tile(
        ...
        for token_tile in hl.tile(end - begin, block_size=BT):
            ...
            residual = _bf16(torch.where(valid[:, None], value - _bf16(projected), 0.0))
            ...
            state_value = state_value * decay[None, :] + update
