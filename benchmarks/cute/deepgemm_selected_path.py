...
HELION_KERNEL_SOURCE_PRODUCTION_SELECTED = "production-selected"
HELION_KERNEL_SOURCES = (HELION_KERNEL_SOURCE_PRODUCTION_SELECTED,)
HELION_KERNEL_SOURCE_KINDS = {
    HELION_KERNEL_SOURCE_PRODUCTION_SELECTED: "production_selected",
}
HELION_KERNEL_SOURCE_NOTES = {
    HELION_KERNEL_SOURCE_PRODUCTION_SELECTED: (
        "Production selected-segment route in helion._compiler.cute.deepgemm_m_grouped."
    ),
}
...
def import_helion_deepgemm_m_grouped() -> Any:
    return importlib.import_module("helion._compiler.cute.deepgemm_m_grouped")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--helion-kernel-source",
        choices=HELION_KERNEL_SOURCES,
        default=HELION_KERNEL_SOURCE_PRODUCTION_SELECTED,
        help=(
            "Helion kernel source to time. The production-selected source calls "
            "helion._compiler.cute.deepgemm_m_grouped."
        ),
    )
    parser.add_argument(
        "--production-selected-kernel",
        action="store_const",
        const=HELION_KERNEL_SOURCE_PRODUCTION_SELECTED,
        dest="helion_kernel_source",
        help="Shortcut for --helion-kernel-source production-selected.",
    )
    parser.add_argument(
        "--deepgemm-root",
        type=Path,
...
def production_selected_kernel_binding(
    ...
    deepgemm_m_grouped = import_helion_deepgemm_m_grouped()
    ...
    details = {
        "helion_kernel_source": HELION_KERNEL_SOURCE_PRODUCTION_SELECTED,
...
def helion_kernel_binding(
    args: argparse.Namespace,
    a_packed: torch.Tensor,
    b_grouped: torch.Tensor,
    grouped_layout: torch.Tensor,
) -> HelionKernelBinding:
    if args.helion_kernel_source == HELION_KERNEL_SOURCE_PRODUCTION_SELECTED:
        return production_selected_kernel_binding(a_packed, b_grouped, grouped_layout)
    raise BenchmarkSetupError(
        f"unknown Helion kernel source: {args.helion_kernel_source}"
    )
...
def run_shape(
    ...
    *,
    ...
    helion_binding = helion_kernel_binding(
        args,
        a_packed,
        b_grouped,
        layout,
    ...
    row: dict[str, Any] = {
        ...
        "helion_kernel_source": args.helion_kernel_source,
        "helion_kernel_source_kind": HELION_KERNEL_SOURCE_KINDS[
            args.helion_kernel_source
        ],
        "helion_kernel_source_note": HELION_KERNEL_SOURCE_NOTES[
            args.helion_kernel_source
        ],
