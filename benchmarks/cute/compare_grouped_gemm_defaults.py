...
def prepare_helion(
    ...
    """Bind Helion with compiler-heuristic or live-autotune selection."""
    ...
    ordered_seeds = [dict(seed.config) for seed in config_spec.compiler_seed_configs]
    promoted_indexes = [
        index for index, seed in enumerate(ordered_seeds) if seed == compiler_promoted
    ]
    grouped_worklist_seed_indexes = [
        index
        for index, seed in enumerate(ordered_seeds)
        if seed.get("tcgen05_grouped_mode") == "worklist_nm"
        and seed.get("tcgen05_grouped_worklist_source_m_tile") == profile.source_m_tile
    ]
    if (
    ...
    compiler_evidence = {
        ...
        "ordered_seed_sha256": [common.config_sha256(seed) for seed in ordered_seeds],
    ...
    if selection_mode == "compiler_heuristic":
        ...
        live_search = {
            ...
            "benchmark_timeout_seconds": (bound.settings.autotune_benchmark_timeout),
    ...
    selection_evidence = {
        ...
        "benchmark_b_layout_matches_reviewed_profile": (b_layout == reviewed_b_layout),
