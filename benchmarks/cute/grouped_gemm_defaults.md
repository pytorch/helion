...
implementations, and a shared FP32 oracle. The canonical campaign always
disables reviewed AOT. The reviewed manifest selects Helion's A packing and
records the untimed reference configuration; its preferred B layout is recorded
separately when it differs from the canonical benchmark layout. Exact reviewed
AOT is validated by the separate B200 profile-native runner documented in
`pretuned_kernels/README.md`.
Both implementations are compiled, checked with elementwise tolerances and
per-group normalized-error bounds, replayed twice for exact repeatability, and
...
The compiler recognizes the grouped matmul and segmented addressing from DeviceIR;
...
established B200 ranking. The GB300 policy contains only source-256 K-major
workloads reproduced by this canonical-layout campaign; N-major calls retain
the generic structural ranking. Treat exact benchmark-row results as in-sample,
...
benchmark input-generation data and is not passed to the kernel.

The separate B200 profile-native runner validates exact reviewed AOT dispatch
with the profile's own B layout. That dispatch uses G/N/K, exact tensor layouts,
packed M, and normalized worklist rows, and abstains when a call falls outside
the exact reviewed workload and layout. GB300 target-specific overrides likewise
require exact normalized measured worklist rows; unseen rows retain the generic
structural ranking. When multiple source tiles are compatible, dispatch shares
the compiler's preference order: 224, then 256, then the compact-32 fallback.
Ordinary worklist tensors cache that source tile by tensor mutation version,
avoiding a value read on repeated calls. An inference-mode worklist has no
version counter, so dispatch re-reads its contents and keys the compatibility
guard from those values, matching the launcher's value-based metadata key.
