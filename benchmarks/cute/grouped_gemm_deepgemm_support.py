...
def clean_checkout(root: Path, expected_head: str, label: str) -> str:
    if not root.is_dir():
...
def import_deepgemm(root: Path, m_alignment: int) -> tuple[Any, dict[str, object]]:
    ...
    source = {
        "git_head": clean_checkout(root, DEEPGEMM_COMMIT, "DeepGEMM"),
        "cutlass_head": clean_checkout(
            root / "third-party" / "cutlass",
            DEEPGEMM_CUTLASS_COMMIT,
            "DeepGEMM CUTLASS",
        ),
        "fmt_head": clean_checkout(
