...
def _attention_shapes(
    ...
    # the canonical example config.  Second is an LM-scale shape that the
    # autotuner can now reach (under the emit_pipeline default the baseline
    # config compiles instead of OOMing during _compute_baseline).
    configs = [
        (2, 32, 1024, 64),
        (8, 32, 8192, 256),
        (2, 32, 2048, 64),
...
def _matmul_layernorm_shapes(
    ...
    # (4096, 4096, 4096) is reachable now that the autotuner default
    # config (under emit_pipeline) compiles instead of OOMing during
    # _compute_baseline.
