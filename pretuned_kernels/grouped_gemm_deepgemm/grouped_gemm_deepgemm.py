...
_REVIEWED_AOT_HARDWARE_NAME = "NVIDIA B200"


def _argument_device_name(device: torch.device) -> str:
    if device.type != "cuda":
        return device.type
    return torch.cuda.get_device_name(device)


def _validate_reviewed_aot_target(a_packed: torch.Tensor) -> None:
    """Restrict only reviewed-AOT dispatch to its measured B200 target."""
    aot_mode = os.environ.get("HELION_AOT_MODE", "evaluate").lower()
    if aot_mode == "compile":
        raise RuntimeError(
            "grouped_gemm_deepgemm does not support standalone AOT compilation"
        )
    if aot_mode != "evaluate":
        return

    device_name = _argument_device_name(a_packed.device)
    if device_name != _REVIEWED_AOT_HARDWARE_NAME:
        raise RuntimeError(
            "reviewed grouped-GEMM AOT profiles require NVIDIA B200 on the "
            f"kernel argument device, got {device_name!r} ({a_packed.device})"
        )
...
def _selected_key(
    ...
    _validate_reviewed_aot_target(a_packed)
...
def create_grouped_gemm_deepgemm_kernel() -> Kernel[torch.Tensor]:
    """Create an independent deployed grouped-GEMM kernel."""
    ...
    if os.environ.get("HELION_AOT_MODE", "evaluate").lower() == "disabled":
        # Preserve disabled mode's normal-autotuner semantics without allowing
        # the reviewed B200 artifact to bypass its evaluate-only target guard.
        kernel.settings.autotune_cache = "LocalAutotuneCache"
    return kernel
