...
logical shape and source-M tile; the AOT key separately specializes physical B
major and packed work volume. The benchmark compares those profiles with one
specific DeepGEMM public path: contiguous K-major B, `compiled_dims="nk"`,
non-PSUM, upstream-recommended B200 alignment 224, and padding-zero work
disabled. Packing and compilation happen before timing. Both implementations
replay pre-captured graphs, clear L2 before every measurement, warm the GPU
before each case, and reverse measurement order to reduce clock and ordering
bias.
