...
Re-tuned with full-effort AOT autotuning (HELION_AUTOTUNE_EFFORT=full) over the
21-shape benchmark sweep: hidden_size in {2048, 4096, 5120}, num_tokens in
{4, 16, 64, 256, 1024, 2048, 8192}, group_size=128. From the union of the
freshly-tuned configs and the previous hand-tuned set, each shape's config was
re-benchmarked with the harness's exact CUDA-graph + L2-clear timing (the metric
the dashboard reports) and the per-shape fastest config selected. Four distinct
configs cover the whole sweep; small-token shapes (<=64) remain launch-overhead
bound and land closest to, but just under, vLLM's hand-written CUDA kernel.
"""
