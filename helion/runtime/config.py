...
class Config(Mapping[str, object]):
    ...
    def __init__(
        ...
        *,
        ...
        grid_foldings: list[list[int]] | None = None,
    ...
    @property
    def grid_foldings(self) -> list[list[int]]:
        ...
        Each entry is a list of factors (one per tile dimension):
          ...
          k  — partial folding (power of 2, 2 ≤ k ≤ 64), grid shrinks by k
          -1 — full folding, dimension entirely in an inner device loop
        """
        return cast("list[list[int]]", self.config.get("grid_foldings", []))
