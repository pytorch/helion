...
@functools.cache
def _get_mps_gpu_core_count() -> int:
    """Return the number of GPU cores on the current Apple Silicon chip.

    Queries ``system_profiler SPDisplaysDataType`` for the core count.
    Falls back to 16 if the query fails or the output cannot be parsed.
    """
    try:
        import re
        import subprocess
...
def get_num_sm(device: torch.device, *, reserved_sms: int = 0) -> int:
    ...
    # TODO(EikanWang): gpu_subslice_count is an out-of-date term. we change update it to XeCore number.
        ...
        available_sms = _get_mps_gpu_core_count()
