...
def _is_array_arg(x: object) -> TypeGuard[_ArrayLike]:
...
def _pallas_prepare_args(
    ...
    *,
    ...
    all_tensor_positions = [i for i in range(len(args)) if _is_array_arg(args[i])]
    ...
    non_tensor_args: dict[int, object] = {
        i: args[i] for i in range(len(args)) if not _is_array_arg(args[i])
    }
