...
from ._fused_launch import build_fused_key_fn
...
class Kernel(Generic[_R]):
    ...
    def _fused_dispatch_key(self, args: tuple[object, ...]) -> Hashable | None:
        ...
        Encodes the same information as :meth:`_fast_dispatch_key` -- exact
        per-argument metadata, scalar values, and the user ``key=`` function --
        plus a 16-byte pointer-alignment bit per tensor (the fused path bypasses
        the direct-launch key where alignment is normally guarded, so it folds
        that hazard in here).  For speed on this hot path the per-position
        extraction is compiled once into a flat ``eval``'d expression and cached
        (``_fused_key_fn``); it embeds ``type(arg)`` at every position, so a call
        whose argument *layout* differs from the one the expression was built for
        (e.g. a tensor where the builder saw ``None``) yields a different key --
        never a false hit -- and a genuinely new layout is rebuilt on miss.
        """
        key_fn = self._fused_key_fn
        if key_fn is None:
            key_fn = self._fused_key_fn = build_fused_key_fn(args, self._key_fn)
        try:
            return key_fn(args)
        except (AttributeError, TypeError):
            # Argument layout changed since the expression was built (e.g. a
            # scalar/None position now holds a tensor): rebuild for this layout.
            self._fused_key_fn = build_fused_key_fn(args, self._key_fn)
            return self._fused_key_fn(args)
