...
from ... import exc
from ..._compiler.cute.aux_tensor import batched_aux_tma_layout_supported
...
def _append_cute_wrapper_plan(
    ...
    if kind == "chained_paired_leaf_tma":
        ...
        append_wrapper(body, call_args, plan)
        return
    if kind == "tcgen05_aux_direct":
        # A direct epilogue read needs per-call alias/layout guards, but no
        # TensorMap or extra device argument. The original tensor is passed
        # through the normal argument list.
        return
    ...
    if kind == "tcgen05_aux_tma":
        ...
        kernel_args = [str(arg) for arg in cast("list[object]", plan["kernel_args"])]
        c_tensor_name = None
        if bool(plan.get("c_leading_passthrough")):
            c_tensor_name = f"{kernel_args[0]}_c_tma"
            append_permuted_cute_tensor_view(c_tensor_name, c_idx, (1, 2, 0))
        append_tcgen05_epilogue_tma_wrapper(
            tensor_idx=c_idx,
            bm=bm,
            bn=bn,
...
def _positive_stride_byte_span(tensor: torch.Tensor) -> tuple[int, int]:
    """Conservative occupied span for a nonempty nonnegative-stride tensor."""
    begin = tensor.data_ptr()
    elements = 1 + sum(
        (size - 1) * stride
...
def _validate_direct_aux_arguments(
    plan: dict[str, object], args: tuple[object, ...]
) -> None:
    """Keep proven mapped reads valid on every launch, including cache hits."""
    source = args[int(cast("int", plan["c_idx"]))]
    output = args[int(cast("int", plan["d_idx"]))]
    if not isinstance(source, torch.Tensor) or not isinstance(output, torch.Tensor):
        raise exc.BackendUnsupported("cute", "mapped auxiliary tensor arguments")
    for prefix, tensor in (("c", source), ("d", output)):
        if (
            tuple(tensor.shape) != plan[f"{prefix}_shape"]
            or tuple(tensor.stride()) != plan[f"{prefix}_stride"]
            or str(tensor.dtype) != plan[f"{prefix}_dtype"]
            or tensor.is_conj()
            or tensor.is_neg()
            ...
            raise exc.BackendUnsupported(
                "cute", "mapped auxiliary requires its proven shape, stride and dtype"
            )
    if source.device != output.device:
        raise exc.BackendUnsupported("cute", "mapped auxiliary device mismatch")
    source_begin, source_end = _positive_stride_byte_span(source)
    output_begin, output_end = _positive_stride_byte_span(output)
    if source_begin < output_end and output_begin < source_end:
        raise exc.BackendUnsupported("cute", "mapped auxiliary must not overlap output")


def _validate_batched_aux_tma_arguments(
    cute_kernel: object, args: tuple[object, ...]
) -> None:
    """Recheck TensorMap alignment and read/write independence before caches.

    A valid initial binding is not enough: a compiled callable may subsequently
    receive overlapping views or a naturally aligned but non-16B-aligned view.
    Conservative byte spans also cover padded positive-stride matrices.
    """
    for plan in getattr(cast("Any", cute_kernel), "_helion_cute_wrapper_plans", ()):
        if plan.get("kind") == "chained_paired_leaf_tma":
            from .chained_leaf_pipeline import validate_plan
            ...
            validate_plan(plan, args)
            continue
        if plan.get("kind") == "tcgen05_aux_direct":
            _validate_direct_aux_arguments(plan, args)
            continue
        if plan.get("kind") != "tcgen05_aux_tma" or not plan.get(
            "c_leading_passthrough"
        ):
            continue
        source = args[int(plan["c_idx"])]
        output = args[int(plan["d_idx"])]
        if not isinstance(source, torch.Tensor) or not isinstance(output, torch.Tensor):
            raise exc.BackendUnsupported(
                "cute", "batched auxiliary TMA tensor arguments"
            )
        if (
            not batched_aux_tma_layout_supported(source)
            or not batched_aux_tma_layout_supported(output)
            or source.shape != output.shape
            or source.dtype != output.dtype
            or source.device != output.device
            or source.data_ptr() % 16 != 0
            or output.data_ptr() % 16 != 0
        ):
            raise exc.BackendUnsupported(
                "cute", "batched auxiliary TMA requires aligned row-major B/M/N tensors"
            )
        ...
        if source_begin < output_end and output_begin < source_end:
            raise exc.BackendUnsupported(
                "cute", "batched auxiliary TMA source must not overlap output"
            )
...
def default_cute_launcher(
    ...
    **kwargs: object,
    ...
    _validate_batched_aux_tma_arguments(cute_kernel, args_tuple)
