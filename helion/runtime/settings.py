...
class Settings(_Settings):
    ...
    __slots__ = {
        ...
        "triton_fused_launch": (
            ...
            "one priming launch, the grid, constant launch arguments, and any "
            "host-side output allocations (all pure functions of the fast-"
            "dispatch key) are recorded and later calls launch the compiled "
            "kernel directly from Kernel.__call__, skipping the wrapper and "
            "launcher frames. Builds on and shares the safety model of "
            "triton_direct_launch (per-tensor 16-byte alignment and "
            "used_global_vals values are folded into the cache key; any "
            "deviation falls back to the full path). Output-allocating kernels "
            "are supported (allocations are rebuilt with torch.empty_strided); "
            "automatically disabled for a kernel that issues multiple device "
            "launches, returns a view over shared storage, or takes cooperative-"
            "grid launches. Requires triton_direct_launch; Triton-only. Set "
