...
class Settings(_Settings):
    ...
    __slots__ = {
        ...
        "triton_fused_launch": (
            ...
            "launcher frames. The two launch hazards the wrapper/launcher "
            "normally guard are folded into the fused cache key (per-tensor "
            "16-byte alignment and used_global_vals values); any deviation, and "
            "torch.compile tracing, fall back to the full path. Output-"
            "allocating kernels are supported (allocations are rebuilt with "
            "torch.empty_strided); automatically disabled for a kernel that "
            "issues multiple device launches, returns a view over shared "
            "storage, or takes cooperative-grid launches. Triton-only. Set "
