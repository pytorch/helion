...
class _FastLauncher:
    ...
    tensor pointers and which are value-specialized int scalars (so we
    can compute both specializations inline on every subsequent call
    instead of invoking Triton's binder).

    Every call computes a tiny *spec key* — alignment bits + int-scalar
    specialization buckets + the binary-affecting knob state (``debug``,
    ``instrumentation_mode``, ``add_stages_inspection_hook``) — and
    dict-looks-up a cached :class:`_SpecEntry`. On hit, dispatches
    straight into the C launcher. On miss, compiles (via Triton) for
    that spec, caches the resulting entry, and dispatches.

    Helion's :class:`BoundKernel` already specializes on the *Helion*
    axes (dtype, shape, stride, device type), so the only per-call
    specialization left to Triton is pointer alignment, int-scalar
    value specialization (``== 1`` constant-fold + divisibility-by-16),
    and per-call knob state — which we encode in the spec key.
    ...
    * The whole ``JITFunction.run`` Python frame, kwargs munging, and
    ...
    def __init__(
        ...
        *,
        ...
        # ``(arg_index, specialize_on_alignment)`` for every positional
        # int arg Triton *value*-specializes (``== 1`` constant-fold +
        # divisibility-by-16). Walked by the hot path to fold scalar
        # specialization into the spec key — without it, a binary
        # compiled for one scalar value (e.g. the constant-folded
        # ``s == 1``) would be silently reused for another.
        self._int_arg_specs: tuple[tuple[int, bool], ...] = ()
        # Per-spec compiled-kernel cache, keyed on ``(align_bits,
        # int_buckets, debug, instrumentation_mode, stages_hook_id)``.
    ...
    def _prime(self, triton_kernel: object, args: tuple[object, ...]) -> None:
        ...
        try:
            ...
            try:
                ...
                # Identify positional int args Triton *value*-specializes,
                # mirroring ``create_function_from_signature`` (jit.py): a
                # param is value-specialized unless it is ``constexpr``,
                # ``do_not_specialize``, or annotated as a float/bool type.
                # ``bool`` is excluded explicitly — Triton maps it to
                # ``u1`` (not constant-folded) even though ``True == 1`` in
                # Python. The alignment (divisibility-by-16) portion is
                # dropped when ``do_not_specialize_on_alignment`` is set.
                # pyrefly: ignore[missing-attribute]
                params = getattr(triton_kernel, "params", None)
                int_arg_specs: list[tuple[int, bool]] = []
                if params is not None:
                    for i, a in enumerate(args):
                        if type(a) is not int or i >= len(params):
                            continue
                        p = params[i]
                        if getattr(p, "is_constexpr", False) or getattr(
                            p, "do_not_specialize", False
                        ):
                            continue
                        ann = getattr(p, "annotation_type", None)
                        if isinstance(ann, str) and (
                            ann == "u1" or ann[:2] in ("fp", "bf")
                        ):
                            continue
                        align = not getattr(p, "do_not_specialize_on_alignment", False)
                        int_arg_specs.append((i, align))
                ...
                self._int_arg_specs = tuple(int_arg_specs)
            except Exception:
                self._init_failed = True
        finally:
        ...
        **kwargs: object,
        ...
        for i in self._tensor_arg_indices:
            ...
            if args[i].data_ptr() & 15 == 0:
                align_bits |= 1 << i
        # Bucket each value-specialized int arg exactly as Triton's
        # ``native_specialize_impl`` does: ``1`` is constant-folded into
        # the binary (bucket ``1``), a value divisible by 16 gets the
        # "divisible" attribute (bucket ``2``), everything else is plain
        # (bucket ``0``). Two int values in the same bucket share a
        # binary; differing buckets must not. Folding this into the key
        # is what keeps a kernel primed with ``s == 1`` from silently
        # reusing the constant-folded binary for ``s == 5``.
        int_bucket_list = []
        for i, align in self._int_arg_specs:
            v = args[i]
            if v == 1:
                int_bucket_list.append(1)
            # pyrefly: ignore[unsupported-operation]
            elif align and v % 16 == 0:
                int_bucket_list.append(2)
            else:
                int_bucket_list.append(0)
        int_buckets = tuple(int_bucket_list)
        spec_key = (
            align_bits,
            int_buckets,
            runtime_debug,
