...
from helion.language.multicast_tensor import MulticastTensor
...
def wait(
    signal_pad: torch.Tensor | MulticastTensor,
    ...
    Args:
        signal_pad: The signal pad tensor / multicast tensor to wait on
...
def _(
    ...
    assert isinstance(signal_pad, (torch.Tensor, MulticastTensor))
    ...
    if isinstance(signal_pad, MulticastTensor):
...
def _(state: CodegenState) -> ast.AST:
    ...
    if isinstance(signal_pad, tuple):
        signal_pad = MulticastTensor(*signal_pad)
    ...
    if isinstance(signal_pad, torch.Tensor):
    ...
    elif isinstance(signal_pad, MulticastTensor):
        from .._compiler.indexing_strategy import MulticastIndexingStrategy

        subscript_shape = SubscriptIndexing.compute_shape(signal_pad.tensor_like, index)
        multicast_shape = signal_pad.dev_ptrs.shape
        shape = subscript_shape + list(multicast_shape)
        ...
        bar_addrs_expr = expr_from_string(
            f"base.to(tl.pointer_type({dtype})){multicast_broadcast} + offset{tensor_broadcast}",
            base=dev_ptrs_ast,
...
def signal(
    ...
    Args:
        signal_pad: The signal pad tensor / multicast tensor to signal
        index: Indices to index into the signal_pad tensor
...
def _(
    ...
    if isinstance(signal_pad, tuple):
        ...
        multicast_shape = signal_pad.dev_ptrs.shape
        subscript_shape = SubscriptIndexing.compute_shape(signal_pad.tensor_like, index)
        shape = list(multicast_shape) + subscript_shape
