...
def _apply_mask(
    result: torch.Tensor, mask: torch.Tensor | None, other: float | torch.Tensor = 0
) -> torch.Tensor:
    if mask is None:
        return result
    ...
    if isinstance(indices, (list, tuple)) and len(indices) == 2:
        ...
        if isinstance(idx0, torch.Tensor) and isinstance(idx1, slice):
            ...
            if isinstance(value, (float, int)):
                value_tensor = torch.full_like(tensor_view[0], value)
                tensor_view.index_add_(0, idx0, value_tensor)
            else:
