...
def _record_pad_info(
    ...
    if state.device_function is not None:
        tensor_id = id(tensor)
        if tensor_id not in state.device_function.pallas_pad_info:
            state.device_function.pallas_pad_info[tensor_id] = {}
        state.device_function.pallas_pad_info[tensor_id][tensor_dim] = block_id
...
def _pallas_generated_index_code(
    ...
    if isinstance(pattern, TilePattern):
        return _pallas_tile_pattern_code(
            pattern, idx, state, tensor_dim, tensor, in_pipeline, pipeline_block_ids
        )

    if isinstance(pattern, TileIndexWithOffsetPattern):
        return _pallas_tile_index_with_offset_pattern_code(
            pattern, state, tensor, tensor_dim
        )
...
def _pallas_ds_expr(state: CodegenState, block_id: int, tile_offset: str = "") -> str:
    ...
    if block_size is None:
        return ":"
    # Hint to the compiler that offset is a multiple of block_size
    offset_expr = f"pl.multiple_of({offset}, {block_size})"
    return f"pl.ds({offset_expr}, {block_size})"
