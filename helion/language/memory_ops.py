...
def _pallas_index_str(
    ...
    # Check if we're inside an emit_pipeline or fori_loop (DMA-based).
    # For fori_loop reduction paths (no DMA), the loop block_ids should NOT
    # be treated as pipeline-tiled — they need pl.ds() slicing.
    in_pipeline = False
    ...
    for idx in subscript:
        ...
        if block_id is not None:
            ...
            if in_pipeline and block_id in pipeline_block_ids:
                ...
                if loops and any(
                    isinstance(loop, (DeviceLoopState, ForiLoopState)) for loop in loops
                ):
