...
def _(
    ...
    if isinstance(tensor, torch.Tensor):
        ...
        # Preserve tile.index tensor tracking through broadcast-only operations
        from .._compiler.compile_environment import CompileEnvironment
        env = CompileEnvironment.current()
        
        if (block_id := env.get_tile_index_tensor_block_id(tensor)) is not None:
            # Check if all indices are broadcasts (None or full slices)
            if all(idx is None or (isinstance(idx, slice) and repr(idx) == "slice(None, None, None)") for idx in index):
                # Use block symbol directly to preserve symbolic equality
                block_sym = env.block_sizes[block_id].var
                shape = [
                    block_sym if isinstance(idx, slice) else 1 
                    for idx in index
                ]
                out = tensor.new_empty(shape)
                env.register_tile_index_tensor_block_id(out, block_id)
                return out
...
def _(state: CodegenState) -> ast.AST:
    ...
    if isinstance(tensor, torch.Tensor):
        ...
        # In the common pattern of using only broadcast operations (None or full slices)
