...
def _(
    ...
    # For FP8 inputs, default to float32
    # For int8 inputs, default to int32
    # For regular float types, use torch.promote_types
    if mat1.dtype in (torch.float8_e4m3fn, torch.float8_e5m2) or mat2.dtype in (
        torch.float8_e4m3fn,
        torch.float8_e5m2,
    ):
        output_dtype = torch.float32
    elif mat1.dtype == torch.int8 or mat2.dtype == torch.int8:
        output_dtype = torch.int32
    else:
        output_dtype = torch.promote_types(mat1.dtype, mat2.dtype)

    return torch.empty(result_shape, dtype=output_dtype, device=mat1.device)
...
def _(state: CodegenState) -> object:
    ...
    if not is_acc_none and acc_dtype is not None:
        # If accumulator is provided, use its dtype
        out_dtype = acc_dtype
    else:
        # When no accumulator is specified:
        # - int8 inputs -> int32
        # - all other inputs -> float32
        if lhs_dtype == torch.int8 or rhs_dtype == torch.int8:
