...
def _has_traced_loop_bounds(state: CodegenState) -> bool:
    """Return True if the loop begin/end values contain traced (non-static) bounds.
    ...
    ends = state.proxy_arg(2)
    if not isinstance(begins, (list, tuple)):
        begins = [begins]
    if not isinstance(ends, (list, tuple)):
        ends = [ends]
    return any(not isinstance(b, int) for b in begins) or any(
        not isinstance(e, int) for e in ends
    )
