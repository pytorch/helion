...
@has_side_effect
@_decorators.api()
def _if(test: object, graph_id: int, if_args: list[object], else_args: list[object]) -> list[object]:
    """`for` loops are mapped to this op since FX does not support control flow."""
    raise AssertionError("this should never be called")
...
def _(state: CodegenState) -> list[object]:
    ...
    """
    from .._compiler.ast_extension import statement_from_string
    from .._compiler.device_ir import NodeArgsGraphInfo, IfGraphInfo, ElseGraphInfo
    from .._compiler.inductor_lowering import codegen_call_with_graph
    ...
    if_args = state.ast_args[2]
    else_args = state.ast_args[3]
    ...
    assert graph_info.if_output_indices is not None
    assert graph_info.else_output_indices is not None
    if_return_names = [if_outputs[i].id for i in graph_info.if_output_indices]
    else_return_names = [else_outputs[i].id for i in graph_info.else_output_indices]
    ...
    else_fn_def.body = else_body_stmts or [ast.Pass()]  # pyrefly: ignore[bad-assignment]

   
    state.add_statement(if_fn_def)
    state.add_statement(else_fn_def)

    if if_return_names: # can also check else_return_names, they will by phi-ed so they will have the same name
        state.add_statement(
            statement_from_string(
    ...
    return [expr_from_string(n) for n in if_return_names] + [expr_from_string(n) for n in else_return_names]
