...
class KernelMetadata:
    ...
    # Source ref for the lazy ``ir_graph`` dump; repr/compare/hash off so the large
    # IR object never bloats repr or participates in dataclass equality/hashing.
    ...
    def _ir_graph(self) -> dict[str, object] | None:
        ...
        try:
            ...
            # Best-effort: never break autotuning. Logged at DEBUG on the
            # helion.autotuner.metrics logger (enable via HELION_LOGS) so silent
            # degradation (e.g. a changed _compiler API) is traceable.
            log.debug(
