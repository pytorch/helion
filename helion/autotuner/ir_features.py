...
single JSON object per autotune run describing the union of all device-side graphs
as a node-link graph. The output is plain JSON in the node-link schema, so a
consumer can rebuild a graph with no extra effort::

    import json, networkx as nx

    rec = json.loads(line)  # one line of <base>.meta.jsonl
    g = nx.node_link_graph(rec["ir_graph"], edges="links")  # networkx >= 3.4
...
stored once per run, inline on the ``.meta.jsonl`` record under the ``ir_graph``
key (alongside the run-level identity it shares). Host-side state (host -> root
edges) is out of scope because it lives in the host AST / origin maps, not in
device IR.
...
(``IR_SCHEMA_VERSION``), bumped whenever the record shape changes. A consumer
should read the version defensively and gate on it::
...
class IrGraphMeta(TypedDict):
    ...
    Run-level identity (run_id, kernel_name, input_shapes, ...) is NOT duplicated
    here: this record is stored inline on the per-run ``.meta.jsonl`` record, which
    already carries that identity. Only structural graph metadata lives here.
    """
...
class IrGraphRecord(TypedDict):
    """The per-run ``ir_graph`` value; loadable via ``nx.node_link_graph(.., edges="links")``."""
...
# Bump when the ir_graph record shape changes, so consumers can detect format
...
def extract_ir_graph(device_ir: DeviceIR) -> IrGraphRecord:
    ...
    ``networkx.node_link_graph(result, edges="links")``. It carries only
    structural graph metadata: run-level identity (run_id, kernel_name, ...) is
    stored once on the parent ``.meta.jsonl`` record this dump is inlined into.
