...
class LFBOPatternSearch(PatternSearch):
    ...
    def _fit_surrogate(self) -> None:
        ...
        if len(train_y[~np.isinf(train_y)]) > 0:
            train_y_quantile = np.quantile(train_y[~np.isinf(train_y)], self.quantile)
            ...
            self.log("All LFBO targets are inf, using default labels")
