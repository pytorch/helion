...
class GenerateAST(NodeVisitor, CodegenInterface):
    def __init__(
        ...
        *,
        ...
        # Block IDs that should execute as local loops instead of grid dims.
        self.local_loop_block_ids: set[int] = local_loop_block_ids or set()
    ...
    def set_active_loops(self, device_grid: DeviceLoopOrGridState) -> None:
        ...
        local_loop_block_ids: set[int] = set()
        if isinstance(device_grid, DeviceGridState):
            for loop_state in device_grid.local_loop_states:
                local_loop_block_ids.update(loop_state.block_ids)
        for idx in device_grid.block_ids:
            if idx not in local_loop_block_ids:
    ...
    def visit_For(self, node: ast.For) -> ast.AST | None:
        ...
        if node._loop_type == LoopType.GRID:
            ...
            with (
                ...
                elif grid_state.local_loop_states:
                    # Wrap the grid body in local loop(s) executed within the program.
                    loop_state = grid_state.local_loop_states[0]
