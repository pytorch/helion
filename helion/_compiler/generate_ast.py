    ...
    def visit_For(self, node: ast.For) -> ast.AST | None:
        ...
        if node._loop_type == LoopType.GRID:
            ...
            if node._root_id == len(self.host_function.device_ir.root_ids) - 1:
                ...
                # Protect extra params (epilogue store targets, extra inputs)
                # from DCE — they appear only in placeholder strings, not in
                # the AST body, so DCE would otherwise remove them.
                for param in self._extra_params:
                    self.device_function.protected_arg_names.add(param)
    ...
    with func:
        ...
        with codegen.device_function:
            ...
            host_def = func.codegen_function_def(
                ...
                removed_args=codegen.device_function.source_less_prologue_params
                or None,
            )
