...
def apply_dot_requirements(
    ...
    is_scaled_mm = node.target is torch.ops.aten._scaled_mm.default  # pyright: ignore[reportAttributeAccessIssue]

    # Validate argument count
    if is_scaled_mm:
        assert 4 <= len(node.args) <= 7, (
            f"_scaled_mm requires 4-7 arguments, got {len(node.args)}"
        )
    else:
...
def _is_constant_tensor(fx_node: torch.fx.Node, value: float) -> bool:
    from ..language import full as hl_full

    return (
        fx_node.target is hl_full
        and fx_node.args[0] == []
        and isinstance(fx_node.args[1], float)
        and fx_node.args[1] == value
    )
...
def reduce_3d_dot(
    ...
    # Initialize scale nodes / ASTs to None
    scale_a_node: Node | None = None
    scale_b_node: Node | None = None
    scale_a_ast: ast.AST | None = None
    scale_b_ast: ast.AST | None = None

    # Parse arguments based on operation type
    if is_scaled_mm:
        # _scaled_mm: (lhs, rhs, scale_a, scale_b, ...)
        lhs_node, rhs_node, scale_a_node, scale_b_node = node.args[:4]  # pyright: ignore[reportAssignmentType]
    ...
    if is_scaled_mm:
        assert scale_a_node is not None and scale_b_node is not None
        assert scale_a_ast is not None and scale_b_ast is not None

        # Check for constant 1.0 scale tensors to avoid no-op multiplications
        scale_a_is_one = _is_constant_tensor(scale_a_node, 1.0)
        scale_b_is_one = _is_constant_tensor(scale_b_node, 1.0)

        scales = []
        kwargs = {"mm_result": mm_result}

        if not scale_a_is_one:
            scales.append("scale_a")
            kwargs["scale_a"] = scale_a_ast
        if not scale_b_is_one:
            scales.append("scale_b")
            kwargs["scale_b"] = scale_b_ast

        if not scales:
            return mm_result

        scale_expr = " * ".join(["mm_result", *scales])
        return expr_from_string(scale_expr, **kwargs)
