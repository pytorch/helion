...
def _emit_pallas_matmul(
    ...
    *,
    ...
    lhs_dtype, rhs_dtype:
        Operand dtypes. When both are ``torch.float32`` the
        ``lax.dot_general`` fallback carries
        ``precision=jax.lax.Precision.HIGHEST`` so the TPU MXU multiplies
        in full f32 instead of falling back to its default bf16-internal
        accumulation, which costs ~1e-2 of absolute error on K=1024
        matmuls. The two arguments default to ``None`` for callers that
        do not yet thread dtypes through; in that case the precision
        keyword is omitted and JAX's default applies.
    """
