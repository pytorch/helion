"""Triton codegen for Inductor epilogue fusion."""
...
def codegen_epilogue_fusion(
    ...
    value: ast.expr,
    store_index: int,
    store_tensor: torch.Tensor | None = None,
    ...
    Args:
        ...
        store_tensor: The tensor being stored to (used to determine if mutation or output)
    ...
    if os.environ.get("HELION_DEBUG_FUSION"):
        print(f"DEBUG codegen_epilogue_fusion: store_index={store_index}")
        print(f"  _fusion_store_map={tb._fusion_store_map}")
        print(f"  _epilogue_specs keys={list(tb._epilogue_specs.keys())}")
    if store_index in tb._fusion_store_map:
        ...
        if acc_name in tb._epilogue_specs:
            ...
            if os.environ.get("HELION_DEBUG_FUSION"):
                print(f"  -> Found epilogue for acc_name={acc_name}")
        else:
            epilogue_items = []
            if os.environ.get("HELION_DEBUG_FUSION"):
                print(f"  -> No epilogue for acc_name={acc_name}")
    elif not tb._fusion_store_map:
        # Single-output kernel: apply epilogue to the output store, not mutation stores
        # Check if this store is to a mutated input or to the output
        is_mutation_store = False
        if store_tensor is not None and tb._helion_mutated_input_names:
            # Check if store tensor was passed as a kernel argument (mutation) vs created inside (output)
            # Use FX node to trace back the tensor's origin
            fx_node = state.fx_node
            tensor_node_info = "none"
            tensor_name = None
            if fx_node is not None and len(fx_node.args) > 0:
                tensor_node = fx_node.args[0]
                if isinstance(tensor_node, torch.fx.Node):
                    tensor_node_info = f"op={tensor_node.op}, name={tensor_node.name}"
                    tensor_name = tensor_node.name
                    # Check if the tensor's name matches a mutated input
                    if tensor_name in tb._helion_mutated_input_names:
                        is_mutation_store = True

        # Apply epilogue only to non-mutation stores (the output store)
        output_name = tb.get_name()
        if output_name in tb._epilogue_specs and not is_mutation_store:
            epilogue_items = [(output_name, tb._epilogue_specs[output_name])]
        else:
            epilogue_items = []
    else:
        epilogue_items = []
    ...
    for acc_name, nodes in epilogue_items:
        if nodes:
            ...
            if os.environ.get("HELION_DEBUG_FUSION"):
                print(
                    f"  Applying epilogue: acc_name={acc_name}, original_value={ast.unparse(value)}"
                )
            ...
            if template_buffer._output_aliases:
                for alias in template_buffer._output_aliases:
                    acc_map.setdefault(alias, value_str)
            new_value = _invoke_pointwise_with_ops_handler(
                ep_nodes, acc_map, subscript_names, capture_epilogue, "epilogue"
            )
            value = new_value
            if os.environ.get("HELION_DEBUG_FUSION"):
                print(f"  Epilogue result: {ast.unparse(value)}")
