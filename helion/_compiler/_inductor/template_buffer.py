...
def prologue_has_complex_indexing(nodes: Sequence[BaseSchedulerNode]) -> bool:
    """Check if prologue nodes have complex indexing that Helion can't handle.

    When a ComputedBuffer has indexing expressions that contain ModularIndexing
    (e.g., from slices or reshapes of non-contiguous tensors), Helion codegen
    doesn't support these expressions. This check returns True if any prologue
    node has such complex indexing.
    """
    from torch._inductor.ir import ComputedBuffer

    for n in nodes:
        ir = _get_ir_node(n)
        if not isinstance(ir, ComputedBuffer):
            continue

        # Check read dependencies for ModularIndexing in their indices
        read_deps = ir.get_read_writes().reads
        for dep in read_deps:
            if hasattr(dep, "index"):
                index_str = str(dep.index)
                if "ModularIndexing" in index_str:
                    return True
