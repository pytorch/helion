...
def has_view(nodes: list[BaseSchedulerNode]) -> bool:
...
def same_shape_and_stride(lhs: IRNode, rhs: IRNode) -> bool:
    ...
    if not sizevars.statically_known_list_equals(
        ...
        return False
    if hasattr(lhs, "get_stride") and hasattr(rhs, "get_stride"):
        if not sizevars.statically_known_list_equals(
...
class HelionTemplateBuffer(TritonTemplateBuffer):
    ...
    def call_kernel(self, kernel_name: str, template_buffer: Any = None) -> None:  # noqa: ANN401
        ...
        for acc_name, nodes in self._epilogue_specs.items():
            if acc_name in mo_names:
                ...
                target = mo_list[0] if mo_list else output_name
            for ep in nodes:
                if isinstance(ep, BaseSchedulerNode) and isinstance(ep.node, IRNode):
                    alias_map[ep.node.get_name()] = target

        if mo_list:
            default_target = mo_list[0]
            for a in aliases:
                wrapper.writeline(f"{a} = {alias_map.get(a, default_target)}")
        else:
            for a in aliases:
                wrapper.writeline(f"{a} = {output_name}")
    ...
    def codegen_template_override(
        ...
        self.multi_output_nodes = [
            _get_ir_node(n) for n in multi_output_nodes
        ]
            ...
            for ep in fusable_epilogue_nodes:
                ...
                if len(reads) > 1:
                    ...
                    for name in reads:
                        ...
                        if not isinstance(kernel_out, IRNode) or not same_shape_and_stride(
        ...
        for name, buf in self.named_input_nodes.items():
            if p := prologue_groups.get(buf.get_name()):
                if has_view(p):
                    continue
                self._prologue_specs[name] = (list(p), buf.get_name())
        fusable_prologue_nodes = [
            n for nodes in prologue_groups.values() if not has_view(nodes) for n in nodes
        ]
