...
class _FusionAutotuneAdapter(_AutotunableKernel):
    ...
    ``__getattr__``, so this satisfies the ``_AutotunableKernel`` protocol
    and flows through the normal autotuner infrastructure (including disk
    caching) without special-casing.
    """
