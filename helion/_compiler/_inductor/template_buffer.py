...
class HelionTemplateBuffer(TritonTemplateBuffer):
    ...
    def _ensure_config(self) -> None:
        """Trigger autotuning if no config available (benchmarks base kernel without fusion)."""
        if self._bound_kernel._implicit_config() is None and self._autotune_args:
            self._bound_kernel.autotune(self._autotune_args, force=False)
