...
from torch._inductor.ir import TemplateFusionOutput
from torch._inductor.ir import TemplateFusionSpec
...
class HelionTemplateBuffer(TemplateBuffer):
    ...
    Implements ``fuse()`` to integrate prologue/epilogue code snippets from
    Inductor's codegen phase into Helion's kernel source.
    ...
    4. Inductor's ``_codegen_single_template`` calls
       ...
       orchestrates prologue/epilogue codegen, builds a ``TemplateFusionSpec``, and
       calls ``fuse(spec)`` to generate the Triton AST with fused expressions.
    5. Inductor emits the fused kernel source and call using ``TemplateFusionOutput``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        def _make_kernel_render(out_node, hint_override=None):
            ...
            return ExternalTritonTemplateKernel(out_node), lambda: None
    ...
    def fuse(self, spec: TemplateFusionSpec) -> TemplateFusionOutput:
        """Single entry point for fusion.

        Reads TemplateFusionSpec, returns TemplateFusionOutput.
        """
        ...
        self._epilogue_specs_by_param: dict[str, tuple[int, EpilogueSpec]] = {
            s.kernel_output_param: (i, s) for i, s in enumerate(spec.epilogue_specs)
        }
        # Set of param names with active prologues.
        self._prologue_fused_params: set[str] = {
            s.input_param for s in spec.prologue_specs
        }
        # Param names whose prologues have a source buffer (vs source-less
        # prologues like ones_like that are fully inlined).
        self._prologue_has_source: set[str] = {
            s.input_param for s in spec.prologue_specs if s.source_buffer is not None
        }
        # 3. Build extra_params list.
        extra_params = [(param, buf) for buf, param in spec.extra_inputs.items()]
        ...
        for epi in spec.epilogue_specs:
            if epi.store_target_param is not None:
                if (epi.store_target_param, epi.store_target) not in extra_params:
                    extra_params.append((epi.store_target_param, epi.store_target))  # type: ignore[arg-type]
                if epi.can_remove_output:
                    self._epilogue_renames[epi.kernel_output_param] = (
                        epi.store_target_param
                    )
                    removed_buffers.add(epi.kernel_output_buf)
                else:
                    self._epilogue_two_store_params[epi.kernel_output_param] = (
                        epi.store_target_param
                    )
        ...
        prologue_source_buffers: dict[str, str | None] = {
            s.input_param: s.source_buffer for s in spec.prologue_specs
        }
        call_preamble, call_args = self._build_call_args(
            call_order, constant_repr, prologue_source_buffers, extra_params
        )

        # 8. Return TemplateFusionOutput — no mutations to any external objects.
        source = get_needed_imports(root) + unparse(
    ...
    def _generate_triton_ast(
        ...
        with self._bound_kernel.env:
            ...
            root = generate_ast(
                ...
                if self._epilogue_specs_by_param
                else None,
    ...
    def _codegen_epilogue_fusion(
        ...
        entry = self._epilogue_specs_by_param.get(param_name)
        if entry is None:
            return value

        epilogue_idx, cap = entry
        ...
        if cap.output_dtype in (torch.float16, torch.bfloat16):
        ...
        state.add_statement(
            ast.Expr(value=ast.Name(id=f"_STORE_OUTPUT_{epilogue_idx}", ctx=ast.Load()))
        )
    ...
    def _codegen_prologue_fusion(
        ...
        mask_str = ast.unparse(indexing.mask_expr) if indexing.has_mask() else "True"

        # Emit bridge variable definitions as AST statements (prevents DCE of
        # referenced variables like indices_0, indices_1).
        state.add_statement(
            ast.parse(
                f"_prologue_{param_name}_xindex = {offset_str}", mode="exec"
            ).body[0]
        )
        state.add_statement(
            ast.parse(f"_prologue_{param_name}_xmask = {mask_str}", mode="exec").body[0]
        )
        ...
        return ast.Name(id=f"_prologue_{param_name}_result", ctx=ast.Load())
