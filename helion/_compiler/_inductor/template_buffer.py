...
class HelionTemplateBuffer(TritonTemplateBuffer):
    ...
    def call_kernel(
        ...
        # Rebuild container args from mangled names using _rebuild_container_args
        all_args: dict[str, object] = {n: _CodeExpr(v) for n, v in arg_inputs.items()}
        for n, v in self._constant_args_dict.items():
            if n == "__container_specs":
                all_args[n] = v
            elif n not in all_args:
                all_args[n] = _CodeExpr(repr(v))
...
def lower_helion_kernel(
    *,
    ...
    # Rebuild structured output and create MultiOutput nodes
