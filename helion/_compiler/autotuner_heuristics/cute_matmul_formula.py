...
Two design invariants proven by the run-3 hill-climbs + the design-validation smoke:
  ...
  * **The depth-fill fills to a fixed ~196 608-byte AB-SMEM isobar.** ``bk`` and ``ab``
    trade off along it (``TCGEN05_DIRECT_ENTRY_STAGE_TUPLES_BY_BK`` encodes exactly
    this). So ``_pick_bk_ab`` picks the ``(bk, ab)`` that MAXIMIZES AB-SMEM bytes used
    within the per-CTA budget and the dtype cap (tie -> deepest ab). This reproduces
    the pretuned decode bn=32/bk=256/ab=8 (196 608) over bk=128/ab=12 (147 456) AND the
    fp8 compute bk=128/ab=6 AND the fp8 decode bn=64/bk=128/ab=12.
...
Bucket-A fills 16-bit ``ab`` to the dtype cap (6) on the DEFAULT path — the bundled
...
# fp8=1 -> ab cap 12; 16-bit (bf16/fp16)=2 -> cap 6 (Bucket-A clamps to 3, see below).
_DTYPE_AB_CAP = {1: 12, 2: 6, 4: 3}
...
# (fatter/more-contiguous TMA B-read) tile down to ~half-device fill. Measured: dropping the decode
# bar 0.8->0.5 improves 64x3584x3584 (bn64@0.38w 100 -> bn32@0.76w 115, +14%) and 64x1536x1536
# (bn64@0.16w 25 -> bn32@0.32w 30, +20%), and changes NO other curriculum shape (0/40 regime flips)
# — but only because it is DECODE-LOCAL: sharing 0.8 with the compute/medm classifier would risk
# re-routing a medium-M shape into the 256² compute tile (the regression the Phase-2 medm fix removed).
...
def _pick_bk_ab(
    ...
    Every climbed / pretuned answer key lands at ~196 608 AB-SMEM bytes (verified vs
    ``tcgen05_ab_smem_bytes_per_cta`` + ``TCGEN05_DIRECT_ENTRY_STAGE_TUPLES_BY_BK``).
    ``bk`` and ``ab`` trade off along that isobar. Rule: for each candidate bk (dtype
    menu), the deepest ab that fits is ``budget // per_stage`` capped by the dtype cap;
    choose the ``(bk, ab)`` that MAXIMIZES AB-SMEM bytes actually used (fills the isobar),
    tie -> deepest ab, tie -> larger bk. Reproduces:
      fp8 decode bn=32 : bk=256/ab=8 (196 608) > bk=128/ab=12 (147 456)   [pretuned]
      fp8 decode bn=64 : bk=128/ab=12 (196 608, hits the fp8 cap)          [R1 M1]
      fp8 medium-M     : bk=128/ab=8 (196 608)                             [climbed]
      fp8 compute 256² : bk=128/ab=6 (196 608)                            [R2 #3]
      bf16 decode bn=32: bk=256/ab=4 (196 608, cluster_m=1)               [R3 #8 key]
      bf16 compute 256²: bk=64/ab=6 (196 608)  [R2 #11; deep AB on the DEFAULT path]

    **16-bit fills the isobar to the dtype cap (6) on the DEFAULT path — no cluster_m
    special-case.** The bundled bf16-deep-AB prerequisite admits 16-bit ab>3 within the
    SMEM budget on the plain (non-FFI) path, for cluster_m=1 AND cluster_m=2. Measured
    (bf16, cluster_m=2, default path, all COMPILE+ACC PASS): the deep pipeline is a WIN, not
    just legal — compute 256² bk64/ab6 = 1443 vs the old bk128/ab3 = 1428 TFLOP/s (4096³:
    +1%; 2048³: +4%), and rectangular medm [256,128] bk128/ab4 = 382 vs the old ab-3-capped
    bk256/ab2 = 323 (+18%). An earlier draft capped 16-bit cm2 at 3 believing deep bk64/ab6
    "needs the FFI explicit_epi_tile topology" — the prerequisite made that stale (deep AB
    now runs on the default path), so the cap only left perf on the table. The Bucket-B FFI
    alt-seed still ships as a ranked alternative; the autotuner keeps whichever wins.
    """
    cap = _DTYPE_AB_CAP.get(itemsize, 3)
    best: tuple[int, int, int] | None = None  # (bytes_used, ab, bk)
...
def _decode_bm(m: int) -> int | None:
    ...
    The tcgen05 cluster_m=1 static-full-tile runtime path validated on GPU0 only for a
    **power-of-2 M** (M=32->bm=32, M=64->bm=64, M=128->bm=64 all PASS; M=96 = 3*32 FAILS
    at runtime — "validated single-root static full tiles" — even with bm=32). So the
    decode branch requires M to be a power of two; bm = min(M, 64) (also a power of 2)."""
    if not _is_pow2(m):
        return None
...
def _pick_decode_bn(m: int, bm: int, n: int, num_sm: int) -> int:
    ...
    grid reaches the decode occupancy bar ``_DECODE_WAVE_FULL`` (0.5, NOT the compute 0.8), else
    the bn=64 default.

    Decode is MEMORY-bound (M<=64: trivial compute, the kernel just streams B), so the objective
    is per-CTA B-read WIDTH balanced against keeping ~half the device busy — NOT compute-style
    last-wave occupancy. Measured trade-off (fp8, cluster_m=1, bm=64, cold-L2 cudagraph):
      * N=5120: bn=64 (0.54w) = 205 TFLOP/s BEATS bn=32 (1.08w) = 183 and bn=16 (2.16w) = 165 —
        once ~half the SMs are busy, WIDER bn wins (fatter/more-contiguous TMA B-reads); extra CTAs
        from a narrower bn are just fragmentation on an already bandwidth-bound kernel.
      * N=3584: bn=64 gives only 0.38w (56 CTAs — badly under-fills), so bn=32 (0.76w) = 115 BEATS
        bn=64 (0.38w) = 100 (+14%). N=1536: bn=64 = 0.16w, bn=32 (0.32w)/bn=16 (0.65w) both +20%.
    So the objective is a SINGLE consistent rule: **"the WIDEST bn reaching ~>=half the device
    (>=0.5 waves); if NONE does (even the narrowest), the shape is device-starved so grab MAX
    occupancy = the NARROWEST bn."** ``_DECODE_WAVE_FULL=0.5`` (vs the old 0.8) captures the
    3584/1536 wins with ZERO change to any other curriculum shape (0/40 regime flips), decode-local
    so it can't perturb the compute/medium-M classifier (which stays at 0.8).

    The fallback returns the narrowest (``_DECODE_BN_CHOICES[-1]``, bn=16), NOT bn=64 — measured:
    in the fallback regime (even bn=16 < 0.5 waves, i.e. tiny N<=~1168 at M=64) bn=16 ties-or-beats
    bn=64 on every probed shape (64x4096x{256,512}: tie; x1024: bn16=43.7 > bn64=37.5, +14%;
    128x4096x512: tie), and bn=128 is worst. The old ``return 64`` was an unvalidated mid-point
    inconsistent with the loop's own shrink-for-occupancy premise (raised in review). Still an
    approximation — the fully-grounded version keys bn off DRAM read-width / the pretuned (K,N)->bn
    table; see run-log.md "decode-bn"."""
...
def _epilogue_aux_rank(env: CompileEnvironment, device_ir: DeviceIR) -> int:
    ...
    2 = source-C / rank-2 exact-shape ``[M,N]`` residual (residual_add, bias_residual_gelu).

    Only a rank-2 EXACT-SHAPE ``[M,N]`` aux (a "source-C" residual) feeds back into the
    config: it needs its own SMEM ring that competes with the AB pipeline, forcing the
    aux-TMA path capped at ab=2 (the C2/C5 ceiling). A rank-1 rowvec/colvec broadcast
    (``bias[N]``, an fp8 rowwise ``scale_a[M,1]``/``scale_b[1,N]``) is transparent — it
    reuses the compute config unchanged.

    The authoritative discriminator is the aux descriptor's ``broadcast_axis`` (``None`` =
    exact ``[M,N]``; ``1`` = rowvec), which is NOT the ``memory_op_facts`` ``ndim`` (that
    reflects the rank-2 subscript ``aux[tile_m, tile_n]``, so an ``[M,1]`` scale and an
    ``[M,N]`` residual look identical there — an ``ndim``-based rule miscategorizes both a
    16-bit ``[M,1]`` scale AND an 8-bit ``[M,N]`` residual). So use the backend's own
    pre-codegen graph detector ``host_function_has_tcgen05_exact_shape_aux_kernel_pattern``,
    which reuses the epilogue-chain analyzer to key on the real ``[M,N]``-vs-broadcast graph
    shape (dtype-agnostic). It runs off ``device_ir.host_function.device_ir.graphs`` (the
    ``HostFunction`` back-ref on the DeviceIR — available at seed time), and requires the
    ``CompileEnvironment`` to be active, hence ``with env``. The post-seed
    ``cute_tcgen05_exact_shape_aux_kernel_detected`` flag is the SAME detector, just not yet
    populated when ``compiler_seed_configs`` runs — this is the identical call the framework
    makes right after (kernel.py, ``host_function_has_tcgen05_exact_shape_aux_kernel_pattern``).
    """
...
def _fp8_small_grid_fits_one_wave(m: int, n: int, num_sm: int) -> bool:
    ...
    Each 128x128 cluster spans 2 CTAs; the small-grid tile is the right seed only at/below
    ``clusters <= num_sm // 2`` (the existing producer's `_small_grid_within_one_wave`
    threshold). Measured head-to-head (cold-L2 cudagraph): where it holds, small-grid
    [128,128,128]/ab12 BEATS the rectangular [256,bn] tile on fp8 medium-M —
    512x2048x2048 347 vs 298 (+16%), 512x8192x2048 932 vs 762 (+22%), 256x4096x4096 526 vs
    467 (+13%); above it (512x2048x4096 = 128 clusters > 74) they tie and rect is kept."""
...
def _regime_tile(
    ...
    # it fits ~one wave (measured fp8 512x2048x2048, all cm2: small-grid 351 / rect256x128 264 /
    # full256² 232 TFLOP/s — tracks CTA count 128/64/32, the reverse of arithmetic intensity).
    #
    # fp8-ONLY (`itemsize == 1`) is a backend CORRECTNESS gate, NOT a perf/SMEM/hardware limit: the
    # (bm=128, cm2) coordinate is dtype-multiplexed (`cute_mma.py:_tcgen05_use_2cta_instrs`), routing
    # fp8 to the validated CtaGroup.TWO path but bf16/fp16 to a legacy CtaGroup.ONE family whose
    # multi-tile runtime ownership isn't yet validated (guarded off). Experiment (GPU1) confirmed the
    # collective is dtype-general — bf16 128² cm2 is bit-exact + 8–18% faster — so 16-bit is blocked
    # only on that backend gate landing. Kept fp8-gated so the formula never emits a rejected config;
    # follow-up to lift it (backend validation + a 16-bit mirror of this branch) is in SUMMARY.md.
    # Needs M,N multiples of 128 (single-root static tile) + the small-grid one-wave fit test.
    if (
        itemsize == 1
...
def _formula_seed(
    ...
    Pure function of ``(fact, spec, num_sm, budget, aux_rank)`` — the env-dependent
    aux-rank detection is done by the caller (``_epilogue_aux_rank(env, device_ir)``) and
    passed in, so this stays unit-testable without a live CompileEnvironment. ``budget`` is
    the per-CTA AB-SMEM budget in bytes; ``aux_rank`` is 0 (transparent) or 2 (source-C).
    """
    ...
    #           bytes -> [4] wins.
    #         many-wave: each CTA does ~waves tiles, so a block's reuse is spread over many wave-steps;
    #           the panel's required residency exceeds L2 against the churn -> evicted before reuse ->
    #           REFETCHED. Measured (ncu, 4096x8192x8192 bf16 ~7 waves): [4] gets a higher L2 hit RATIO
    #           (69.6 vs 65.5%) but reads +18% DRAM BYTES (715 vs 607 MB) — short-range reuse works,
    #           the cross-block reuse it reached for gets evicted. At ~98% tensor pipe (no slack) that
    #           extra traffic hits the critical path (−3–8%) -> [1] wins.
    #         So [1] many-wave / [4] few-wave; the sign flips because reuse-distance-vs-L2 scales with
    #         waves. (Fact is ncu-solid; exact cause = reuse-distance eviction vs a bad G=4 grid map is
    #         not separable from these counters. _MANY_WAVE=4 is the empirical crossover, least-swept.)
...
def _formula_eligible(env: CompileEnvironment) -> bool:
    ...
    matmul whose shape lands inside the static-full-tile envelope.

    Gates on ``cute_tcgen05_search_enabled`` — the authoritative "tcgen05 applies to this
    kernel" signal (matmul_ops sets it only when the MMA-support probe reports tcgen05 for
    the dtype AND static_m>=64/static_n>=8/static_k>=mma_k). This is a genuine backend
    CAPABILITY gate (gate-2), NOT the cluster_m2 SEARCH-restriction gate the seed must stay
    orthogonal to: when tcgen05 is unavailable (e.g. the warp/universal-only path) the
    formula declines so the non-tcgen05 default is preserved."""
    ...
    if fact.static_m is not None and fact.static_m <= _SKINNY_M_MAX:
        return False
...
class CuteTcgen05FormulaFfiAltHeuristic(AutotunerHeuristic):
    ...
    A SECOND ranked seed (there is no ``get_seed_configs`` plural — the alt-seed is its
    own heuristic, like ``CuteTcgen05ClusterM2FfiHeuristic`` beside the DEFAULT producer).
    The ``explicit_epi_tile`` / flat-role / tvm_ffi direct-entry config won 3 climbed bf16
    keys (R1 M2, R1 M3, R2 #11). Since the bf16-deep-AB prerequisite now lets the Bucket-A
    DEFAULT path reach the same deep ``bk=64/ab=6`` pipeline, this FFI seed is no longer the
    *only* way to get deep 16-bit AB — but it remains a distinct topology (flat-role +
    TMA-store epilogue) that can still win on launch/epilogue-bound shapes, so it ships as a
    ranked alternative. It is NOT promoted-to-default: the DEFAULT seed is Bucket-A, the FFI
    seed is benchmarked beside it and kept only where it wins / compiles (the autotuner drops
    it on any InvalidConfig / accuracy failure).

    Only fires for 16-bit (bf16/fp16) full-tile compute (the FFI ``explicit_epi_tile`` /
    flat-role path is validated only there); fp8 runs deep ab on the DEFAULT path so it
    needs no alt-seed."""
...
# (mirrors TCGEN05_DIRECT_ENTRY_STAGE_TUPLES_BY_BK): bk=64 admits (ab=6,c=4) or (3,2);
# bk=128 admits only (3,2). The deep bk=64/ab=6 pipeline won the bf16 compute keys, so it
# is tried first. When the deep tuple is skipped (a source-C residual — deep AB loses to
# the residual C-ring), the SHALLOW bk=128/ab=3 tuple is preferred over bk=64/ab=3
# (measured: residual_add 4096³ bk128/ab3 = 1017 > bk64/ab3 = 945).
...
def _ffi_bk_ab(
    ...
    (b) fits the per-CTA AB-SMEM budget for the 256² 16-bit tile.

    For a rank-2 source-C residual, the residual C-ring competes with the AB pipeline, so
    the deep (64,6,4) tuple loses to the shallow (bk,3,2): measured on residual_add
    4096³ old_ffi ab3 = 1017 > formula_ffi ab6 = 972.8. So skip the deep tuple when a
    source-C residual is present."""
    k = fact.static_k
