...
_TCGEN05_GROUPED_WORKLIST_ANALYSIS_STATE_KEY = "cute_tcgen05_grouped_worklist_analysis"
_MISSING_GROUPED_WORKLIST_ANALYSIS = object()


def _tcgen05_grouped_worklist_analysis(
    env: CompileEnvironment,
    device_ir: DeviceIR,
    fact: MatmulFact,
) -> Tcgen05GroupedWorklistAnalysis | None:
    """Analyze once for the DeviceIR owned by this compile environment."""
    cached = env.compiler_heuristic_state.get(
        _TCGEN05_GROUPED_WORKLIST_ANALYSIS_STATE_KEY,
        _MISSING_GROUPED_WORKLIST_ANALYSIS,
    )
    if cached is not _MISSING_GROUPED_WORKLIST_ANALYSIS:
        cached_device_ir, cached_fact, cached_analysis = cast(
            "tuple[DeviceIR, MatmulFact, Tcgen05GroupedWorklistAnalysis | None]",
            cached,
        )
        if cached_device_ir is device_ir and cached_fact == fact:
    ...
    env.compiler_heuristic_state[_TCGEN05_GROUPED_WORKLIST_ANALYSIS_STATE_KEY] = (
        device_ir,
        fact,
        analysis,
    )
...
class CuteTcgen05GroupedWorklistHeuristic(AutotunerHeuristic):
    ...
    def register_facts(
        ...
        if fact is None:
            return frozenset()
        analysis = _tcgen05_grouped_worklist_analysis(env, device_ir, fact)
        if analysis is None:
            return frozenset()
