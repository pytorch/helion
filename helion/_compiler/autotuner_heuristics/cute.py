...
class CuteChainedMatmulHeuristic(AutotunerHeuristic):
    ...
    def register_facts(
        ...
        from ..cute.chained_coefficient_cache import has_coefficient_candidate

        spec.cute_chained_coefficient_cache_search_enabled = (
            spec.cute_chained_tcgen05_search_enabled
            and has_coefficient_candidate(device_ir.graphs)
        )
    ...
    def get_seed_configs(
        ...
        if spec.cute_chained_tcgen05_search_enabled:
            ...
            # Keep the existing seed pool. Only multi-dot roots can
            # retain an intermediate FP32 shared boundary to pad.
            if len(spec.matmul_facts) > 1:
                seeds.extend(
                    Config.from_dict(seed.config | {"cute_chained_c_smem_padding": 4})
                    for seed in tcgen_seeds
                )
        ...
        if not spec.cute_chained_initialized_accumulator_search_enabled:
            ...
            return cls._startup_seed(
                ...
                _with_last_read_seed(
                    _with_coefficient_cache_seed(
                        ordered,
                        enabled=spec.cute_chained_coefficient_cache_search_enabled,
                    )
                ),
            )
        result: list[Config] = []
...
def _with_leaf_pipeline_seeds(seeds: list[Config], *, enabled: bool) -> list[Config]:
    """One structural overlap parent, two siblings; legacy objects stay ordered."""
    if enabled:
        for index, seed in enumerate(seeds):
            if seed.config.get("cute_chained_k_schedule") == "overlap64":
                return [
                    ...
                    *(
                        Config.from_dict(
                            seed.config | {"cute_chained_leaf_pipeline": mode}
                        )
...
def _with_k_schedule_seeds(seeds: list[Config], *, enabled: bool) -> list[Config]:
    ...
    parents = [
        ...
        and not seed.config.get("cute_chained_coefficient_cache")
    ]
    if not parents:
        return seeds
    parent = next(
...
def _with_coefficient_cache_seed(seeds: list[Config], *, enabled: bool) -> list[Config]:
    """One optional sibling; retain all old objects, order and multiplicities."""
    if enabled:
        for index, seed in enumerate(seeds):
            if seed.config.get(
                "cute_chained_mma_schedule"
            ) == "tcgen05_tmem" and not seed.config.get("cute_chained_direct_output"):
                return [
                    *seeds[: index + 1],
                    Config.from_dict(
                        seed.config | {"cute_chained_coefficient_cache": True}
                    ),
                    *seeds[index + 1 :],
                ]
    return seeds
...
class CuteTcgen05ThreadLocalEpilogueHeuristic(AutotunerHeuristic):
    ...
    def get_seed_configs(
        ...
        if seed is None:
            return []
        config = env.config_spec._cute_tcgen05_config
        return config.aux_role_local_scheduler_seed_configs(
            config.consumer_register_seed_configs([seed])
        )
