...
class _TritonReductionSeedBase(AutotunerHeuristic):
    ...
    # at N=16384 or already loop), steering half-precision full-width standard rows onto the looped path.
    ...
    def _build_block_sizes(
        ...
        if non_reduction_loop_ids:
            # Match the reduction tile: red_value (user-tiled) or next_pow2(size_hint)
            # (standard, where red_value is None). One rule, no byte-keyed widening — a sane
            # non-size-1 seed.
    ...
    def _persistent_looped(
        ...
        if can_persist:
            ...
            # Persistent: standard encodes the extent as reduction_loops=None; user-tiled
            # as the full pow2 R_BLOCK so the inner `for tile_n` runs once.
...
class TritonStandardReductionHeuristic(_TritonReductionSeedBase):
    ...
    def get_seed_config(
        ...
        # standard rides persistent-vs-looped on `reduction_loops`, so the lever's `extent`
        # (the user-tiled R_BLOCK) is unused here.
...
class TritonUserTiledReductionHeuristic(_TritonReductionSeedBase):
    ...
    - **plain user-tiled** (softmax_two_pass): no cap — persistent full-pow2 R_BLOCK,
    ...
    def get_seed_config(
        ...
        # normalize, so 'last' regardless of persistence; plain user-tiled (softmax_two_pass) only
