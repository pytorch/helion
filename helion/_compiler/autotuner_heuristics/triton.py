...
class _TritonReductionSeedSM100(_TritonReductionSeedBase):
    ...
    target + ``promote_seed_to_default`` and re-tunes constants (as class attributes) only where a
    B200 measurement demands it. The concrete subclasses inherit ``is_eligible`` from their sm90
    track class, which gates on ``cls.HARDWARE_TARGETS`` (sm100 here) — so hardware selection lives
    in ``is_eligible``, not in this ``get_seed_config``. Not registered; the two concrete subclasses
    below are."""
