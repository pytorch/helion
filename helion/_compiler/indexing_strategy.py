...
def validate_alignment_or_throw(
    ...
    *,
    ...
    for axis, stride_expr in enumerate(stride_entries):
        ...
        # Any other runtime-dependent offset (e.g., grouped GEMM) is considered unsafe.
        raise AlignmentInvalidConfig(
            f"{mode.replace('_', ' ')} for {tensor_label} axis {axis} depends on runtime offset '{value}', "
            "so alignment cannot be guaranteed."
        )
