...
def _analyze_mma_operand(
    ...
    return _MmaOperandInfo(
        load=load_node,
        terminal=node if trailing_transpose else load_node,
        source_fake=source_fake,
        logical_fake=source_fake.mT if trailing_transpose else source_fake,
        block_ids=block_ids,
...
def _emit_mma_pipeline(
    ...
    tcgen05_use_tma_store_epilogue = (
        mma_impl == "tcgen05"
        and tcgen05_use_tma_pipeline
        # Narrow FP8 stores are faster through the direct SIMT epilogue.
        and (input_dtype != torch.float8_e4m3fn or bn >= 32)
        and (
            tcgen05_static_full_tiles
            or tcgen05_grouped_worklist_static_full_tiles
            or tcgen05_role_local_k_tail_tma
    ...
    if mma_impl == "tcgen05":
        ...
        if explicit_epi_tile_requested:
            ...
            if (
                ...
                and not fp8_m64_explicit_epi_tile
                and (
                    tcgen05_explicit_epi_tile_m,
                    tcgen05_explicit_epi_tile_n,
                    tcgen05_explicit_d_store_box_n,
    ...
    def _tcgen05_tma_tile_predicate(
        *, k_tile_start_expr: str, full_tile_end_expr: str
        ...
        return " and ".join(
            predicate
            for predicate in (_tcgen05_tma_output_tile_predicate(), k_predicate)
            if predicate
        )
    ...
    if mma_impl == "tcgen05":
        ...
        if tcgen05_use_tma:
            ...
            elif not tcgen05_grouped_static_persistent:
                assert tma_warp is not None
                prefix.append(
                    statement_from_string(
                        f"if {tma_warp}:\n"
