...
def _trailing_matrix_axis_swap(ndim: int) -> tuple[int, ...]:
    """Return the source-to-logical order that swaps only the matrix axes."""
...
class _MmaOperandInfo:
    ...
    def __post_init__(self) -> None:
        if self.source_to_logical_order is None:
            return
        trailing_axis_swap = _trailing_matrix_axis_swap(self.source_fake.ndim)
        assert self.source_to_logical_order == trailing_axis_swap, (
            "Invalid MMA operand analysis: source_to_logical_order must swap "
            "exactly the two trailing matrix axes while preserving every "
            "leading axis. Other mappings are not supported by collective MMA "
            "lowering. Please report a bug to the Helion maintainers with this "
            f"context: source_to_logical_order={self.source_to_logical_order}, "
            f"expected={trailing_axis_swap}, source_shape={self.source_fake.shape}"
        )
