"""Shared cross-graph dependency helpers for CuTe matmul stores.
...
- :func:`resolve_tcgen05_accumulator_boundary` is a narrow plumbing walk
  used by the generic epilogue slicer. It
  only follows ``_phi.args[1]`` (loop body output) and ``_new_var``
  / ``getitem`` passthroughs. ``_phi.args[0]`` (initial value, e.g.
...
def reach_tcgen05_matmul_anchors(
    ...
    site to recover the unique matmul anchor. The generic plan slicer uses the
    narrow boundary resolver separately so it stops at loop plumbing rather
    than absorbing the K-loop graph.
    """
...
def resolve_tcgen05_accumulator_boundary(
    node: torch.fx.Node,
    target_fx_nodes: set[torch.fx.Node],
    inner_outputs_by_graph_id: dict[int, tuple[torch.fx.Node | None, ...]],
) -> torch.fx.Node | None:
    ...
    Caller passes the pre-built ``inner_outputs_by_graph_id`` so plan extraction
    does not rebuild the cross-graph index for every visited node.
    """
    from ...language import _tracing_ops
