...
from .chained_coefficient_cache import make_coefficient_cache
...
def supported_plan(
    plan: ChainedMatmulPlan, c_smem_padding: int = 0, *, startup: bool = False
) -> bool:
    ...
    return _shared_memory_bytes(
        plan, c_smem_padding, startup=startup
...
def codegen_chained_tcgen05(cg: GenerateAST, plan: ChainedMatmulPlan) -> bool:
    ...
    c_smem_padding = cast("int", df.config.config.get("cute_chained_c_smem_padding", 0))
    ...
    try:
        ...
        if df.config.config.get("cute_chained_startup_transfer", "legacy") == "tma":
            ...
            if not supported_plan(plan, c_smem_padding, startup=True):
                raise chain._UnsupportedChain(
        ...
        if leaf_pipeline is not None:
            ...
            if _shared_memory_bytes(
                plan, c_smem_padding, startup=bool(startup_inputs)
        ...
        if df.config.config.get("cute_chained_auxiliary_cache"):
            ...
            cache_lines, early_cached = make_early_auxiliary_cache(
                ...
                - max(
                    ...
                    _shared_memory_bytes(
                        cache_plan, c_smem_padding, startup=bool(startup_inputs)
                    ),
                ),
            )
            lines.extend(cache_lines)
            scans = [*scans, *early_cached]
        operand_boundaries = boundaries
        if df.config.config.get("cute_chained_coefficient_cache", False):
            coefficient_lines, coefficients = make_coefficient_cache(
                cg, plan, boundaries, scans, set(plan.scans) if early_scan else set()
            )
            lines.extend(coefficient_lines)
            operand_boundaries = {**boundaries, **coefficients}
        ...
        for stage, (node, (m, n, k)) in enumerate(
            ...
            if stage == 0 and plan.initialized_accumulator is not None:
                ...
                lines.extend(
                    [
                        f"{prefix}_c = cute.make_tensor(cute.arch.alloc_smem(cutlass.Float32, {m * (n + c_smem_padding)}, alignment=128), cute.make_layout(({m}, {n}), stride=({n + c_smem_padding}, 1)))",
