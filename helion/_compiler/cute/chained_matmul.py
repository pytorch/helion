...
def detect_chained_matmul_search(graphs: Sequence[GraphInfo]) -> bool:
    ...
    for node in dots:
        ...
        if lhs.dtype not in (torch.bfloat16, torch.float16):
            return False
        if node not in _ancestors(store.args[2]):
            return False
    return (
        len(dots) > 1
        or not all(_direct_operand(cast("Node", arg)) for arg in dots[0].args[:2])
        or not _ordinary_mma_supported(dots[0])
    )
...
def plan_chained_matmul(graphs: Sequence[GraphInfo]) -> ChainedMatmulPlan | None:
    ...
    store = exports.primary if exports is not None else stores[0]
    if (
        any(len(node.args) != 4 or node.args[2:] != (None, None) for node in dots)
        or any(not isinstance(arg, Node) for node in dots for arg in node.args[:2])
        or not isinstance(store.args[2], Node)
        or any(node not in _ancestors(store.args[2]) for node in dots)
    ):
        return None
    # Leave a conventional direct-load GEMM to the ordinary MMA pipeline.
    if (
        len(dots) == 1
        and all(_direct_operand(cast("Node", arg)) for arg in first.args[:2])
        and _ordinary_mma_supported(first)
    ):
        return None
    if len(_shape(store.args[2])) != 2:
        return None
    dtype = first.args[0].meta["val"].dtype
    ...
    plan = ChainedMatmulPlan(
        ...
        scan_exports=exports.exports if exports is not None else (),
    ...
    if tcgen:
        ...
        if not supported_plan(
            plan, cast("int", df.config.config.get("cute_chained_c_smem_padding", 0))
        ):
            return None
    return plan
...
class _Expression:
    ...
    def dependencies(self, expression: str) -> set[str]:
        ...
        while pending:
            ...
            if name in self.definitions:
                for node in ast.walk(ast.parse(self.definitions[name])):
    ...
    def value(self, node: Node, coordinates: tuple[str, ...]) -> str:
        ...
        if node in self.fragments:
            ...
            self.lines.extend(ast.unparse(s) for s in statements)
            for statement in statements:
                if (
                    ...
                    self.definitions[statement.targets[0].id] = ast.unparse(
                        statement.value
                    )
