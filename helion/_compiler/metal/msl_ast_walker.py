...
# ---------------------------------------------------------------------------
# Statement emitter (handles Assign, Expr, If, and tl.store calls)
# ---------------------------------------------------------------------------
...
def _emit_tl_store(
    ...
    if len(node.args) >= 3:
        ...
        for kw in node.keywords:
            if kw.arg == "mask":
                mask = _ast_expr_to_msl(kw.value)
                break
    if mask is not None:
        parts.extend(
            (
    ...
    for stmt in stmts:
        if isinstance(stmt, ast.Assign) and len(stmt.targets) == 1:
            ...
            if isinstance(target, ast.Name):
                ...
                val_msl = _ast_expr_to_msl(value)
                parts.append(f"{pad}{target_msl} = {val_msl};")
            else:
                parts.append(
                    f"{pad}{_ast_expr_to_msl(target)} = {_ast_expr_to_msl(value)};"
                )
...
def _emit_for(
    ...
    """Emit MSL for a ``for var in range(N)`` or ``for var in tl.range(...)`` loop.

    Recognises two patterns:
    - ``for var in range(end)`` → ``for (int var = 0; var < end; var += 1)``
    - ``for var in range(start, end)`` / ``for var in range(start, end, step)``
    - ``for var in tl.range(start, end, step)``
    """
    if declared is None:
        declared = set()
    pad = " " * indent
...
def _is_broadcast_subscript(sl: ast.AST) -> bool:
    """Return True if the subscript is a broadcasting no-op (e.g. ``[:, None]``).

    In per-thread Metal dispatch every value is scalar, so Triton-style
    broadcasting subscripts like ``x[:, None]`` or ``x[None, :, None]``
...
def _ast_subscript_to_msl(node: ast.AST) -> str:
    ...
    # Broadcasting subscripts (e.g. x[:, None]) are no-ops for per-thread dispatch
    if _is_broadcast_subscript(sl):
        return buf_name
...
def _collect_block_size_names(stmts: list[ast.stmt]) -> dict[str, int]:
    """Scan AST statements for _BLOCK_SIZE_N references.

    Returns a mapping from name to dimension index (e.g. ``_BLOCK_SIZE_0`` → 0).
    """
    import re
    ...
    for node in ast.walk(ast.Module(body=stmts, type_ignores=[])):
        if isinstance(node, ast.Name) and (m := pattern.fullmatch(node.id)):
            result[node.id] = int(m.group(1))
    return result
