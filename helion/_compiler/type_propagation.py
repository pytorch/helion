...
class TensorType(TypeInfo):
    ...
    def _device_indexing_size(self, key: TypeInfo) -> list[int | torch.SymInt]:
        ...
        broadcast_shape = env.tensor_indexer_broadcast_shape(tensor_indexers) if tensor_indexers else None
        use_broadcast_once = broadcast_shape is not None
        shared_shape: list[int | torch.SymInt] = broadcast_shape or []
        ...
        for k in keys:
            if isinstance(k, LiteralType):
                ...
                raise exc.InvalidIndexingType(k)
        # Check for rank mismatch - consuming too many dimensions
