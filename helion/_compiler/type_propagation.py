...
class TypePropagation(ast.NodeVisitor):
    ...
    def visit_Assert(self, node: ast.Assert) -> TypeInfo:
        ...
        # Try to unify symbols if this is an equality assertion between SymInts
        self._try_unify_symbols_from_assert(node.test)

        return NoType(origin=self.origin())

    def _try_unify_symbols_from_assert(self, test: ast.expr) -> None:
        """Try to unify symbols from an equality assertion.

        Why this is needed:
        -------------------
        When static_shapes='none', each tensor dimension gets its own independent
        unbacked symbolic integer (e.g., u1, u2, u3...). This means that even when
        two dimensions represent the same value, they have different symbols:

            x.shape = [u1, u2]      # e.g., [32, 64]
            weight.shape = [u3]     # e.g., [64]

        Here u2 and u3 both represent 64, but they're different symbols. When we
        try to broadcast x * weight, Helion sees shapes [u1, u2] vs [1, u3] and
        raises ShapeMismatch because it can't prove u2 == u3.

        The fix is to use assert statements to tell Helion about dimension equality:

            m, n = x.size()           # n is u2
            assert weight.size(0) == n  # weight.size(0) is u3, now unified with u2

        After this assert, u3 is replaced with u2 in the shape environment, so
        broadcasting [u1, u2] * [1, u2] succeeds.

        This is essential for kernels like rms_norm where the weight tensor must
        match the input's feature dimension but comes from a separate input tensor.

        Why this didn't happen before the 0/1 specialization PR:
        --------------------------------------------------------
        Before this PR, static_shapes was a boolean:
        - static_shapes=True: All dimensions are concrete integers (no symbols)
        - static_shapes=False: Dimensions are symbolic, BUT PyTorch's ShapeEnv
          still specialized on 0 and 1 values (specialize_zero_one=True)

        With the old static_shapes=False, PyTorch's from_real_tensor() created
        *backed* symbolic integers that were linked to the concrete values. When
        two dimensions had the same concrete value, PyTorch could often prove
        equality through its guard system.

        The new static_shapes='none' mode is different:
        - It sets specialize_zero_one=False in ShapeEnv
        - It creates fresh *unbacked* symints via cached_create_unbacked_symint()
        - Each tensor gets completely independent symbols with no connection to
          concrete values or other tensors' symbols
        - This enables true shape polymorphism but requires explicit symbol
          unification via assert statements for cross-tensor dimension equality
        """
        if not isinstance(test, ast.Compare):
            return

        # Only handle simple equality: `assert a == b` (not `assert a == b == c`)
        if len(test.ops) != 1 or not isinstance(test.ops[0], ast.Eq):
            return

        # Get TypeInfo for both sides of the comparison
        left_type = self.visit(test.left)
        right_type = self.visit(test.comparators[0])

        # Extract SymInt values - these are the symbolic integers we want to unify
        left_symint = self._extract_symint(left_type)
        right_symint = self._extract_symint(right_type)

        if left_symint is None or right_symint is None:
            return

        # Get the underlying sympy symbols
        left_sym = left_symint._sympy_()
        right_sym = right_symint._sympy_()

        # If they're already the same symbol, nothing to do
        if left_sym == right_sym:
            return

        # Both must be simple symbols (not complex expressions like u1 + u2)
        if not isinstance(left_sym, sympy.Symbol) or not isinstance(
            right_sym, sympy.Symbol
        ):
            return

        # Establish equivalence: replace right_sym with left_sym everywhere
        # This uses PyTorch's shape environment replacement mechanism
        env = CompileEnvironment.current()
        try:
            env.shape_env._set_replacement(
                right_sym, left_sym, "assert equality constraint"
            )
        except (AssertionError, ValueError):
            # If _set_replacement fails (e.g., due to conflicting value ranges),
            # silently continue - the assert will still be checked at runtime
            pass

    def _extract_symint(self, type_info: TypeInfo) -> torch.SymInt | None:
        """Extract a SymInt from a TypeInfo if possible."""
        if isinstance(type_info, SymIntType):
            return type_info.value
        if isinstance(type_info, LiteralType) and isinstance(
            type_info.value, torch.SymInt
        ):
            return type_info.value
        return None
