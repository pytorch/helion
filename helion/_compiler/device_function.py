...
class DeviceFunction:
    def __init__(
        ...
        # Pallas: when ``pallas_loop_type == "outer_grid"`` and matmul-
        # pattern detection passes, the inner K loop is lifted into the
        # outer ``pl.pallas_call`` grid as an extra ``"arbitrary"`` axis.
        # Each entry is ``PallasOuterGridLiftedAxis(block_id, nsteps_expr,
        # scratch_name)``; ``_compute_reduction_grid_dims`` (backend.py)
        # reads it to flag the corresponding outer-grid dim as reduction
        # so the launcher marks it ``"arbitrary"`` in
        # ``dimension_semantics``.  The post-codegen body rewrite in
        # :meth:`codegen_function_def` then wraps scratch-init / store
        # statements in ``pl.when`` guards on ``pl.program_id(<grid_dim>)``.
