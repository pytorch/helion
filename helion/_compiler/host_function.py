...
class HostFunction:
    ...
    def codegen_function_def(
        ...
        if removed_args:
            existing_arg_names = {a.arg for a in self.args.args}
            unknown = removed_args - existing_arg_names
            assert not unknown, (
                f"removed_args contains names not in function signature: {unknown}"
            )
