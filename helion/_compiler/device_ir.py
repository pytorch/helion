...
class WalkDeviceAST(NodeVisitor):
    ...
    def visit_Slice(self, node: ast.Slice) -> slice | SliceProxy:
        lower = self.visit(node.lower) if node.lower else None
        upper = self.visit(node.upper) if node.upper else None
        step = self.visit(node.step) if node.step else None
        
        from ..language.slice_proxy import has_symbolic_bounds
        if has_symbolic_bounds(lower, upper, step):
            return hl.slice(lower, upper, step)
