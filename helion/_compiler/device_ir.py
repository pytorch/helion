...
def epilogue_subtiling_pass(graph: torch.fx.Graph, store_count: int) -> None:
    ...
    for node in graph.nodes:
        if node.op == "call_function" and node.target == store_api:
            ...
            stores_to_replace.append((node, pointwise_nodes, store_config_index))
            store_config_index += 1

    for store_node, pointwise_nodes, config_idx in stores_to_replace:
