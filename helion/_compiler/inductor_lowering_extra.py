...
from ._inductor.template_buffer import prologue_has_complex_indexing
...
def _patch_scheduler_fusable_read_and_write() -> None:
    ...
    # Patch prologue fusion heuristics to allow Helion templates to fuse
    # The default heuristics reject prologue fusion when read_bytes > write_bytes * 1.1
    # but this is too conservative for Helion kernels which benefit from prologue fusion
    original_check_prologue = inductor_scheduler.Scheduler.check_prologue_fusion_heuristics_fusable

    def helion_check_prologue_fusion_heuristics(
        self,
        prologue_node: BaseSchedulerNode,
        template_node: BaseSchedulerNode,
        why: object,  # WhyNoFuse type
    ) -> bool:
        template = template_node.get_template_node()
        if isinstance(template, HelionTemplateBuffer):
            # For Helion templates, skip the bytes threshold heuristic
            # but still enforce view constraints that ensure correctness
            prologue_nodes = prologue_node.get_nodes()

            # Block fusion if there are views in the prologue
            # (views can change memory layout and cause incorrect fusion)
            if has_view(prologue_nodes):
                return False

            # Check that all prologue nodes produce ComputedBuffers
            for snode in prologue_nodes:
                if not isinstance(_get_ir_node(snode), ComputedBuffer):
                    return False

            # Block fusion if prologue reads from views with complex indexing
            # (ModularIndexing expressions that Helion codegen doesn't support)
            if prologue_has_complex_indexing(prologue_nodes):
                return False

            # Check template doesn't use internal views
            if template.uses_internal_views():
                return False

            # Check produced buffer matches template input shape/stride
            template_reads = template_node.used_buffer_names()
            produced = prologue_node.get_buffer_names() & template_reads
            if len(produced) != 1:
                return False
            ...
            return True
        return original_check_prologue(self, prologue_node, template_node, why)

    inductor_scheduler.Scheduler.check_prologue_fusion_heuristics_fusable = (
        helion_check_prologue_fusion_heuristics
    )
...
# Apply scheduler patch at module load time so it's active before any torch.compile
# This is needed because scheduling happens during torch.compile, not during Helion kernel lowering
