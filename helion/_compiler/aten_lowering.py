...
def codegen_permute_pallas(ctx: LoweringContext, node: Node) -> object:
    from .pallas.codegen import maybe_codegen_owner_prep_cache_read

    owner_prep_read = maybe_codegen_owner_prep_cache_read(ctx, node)
    if owner_prep_read is not None:
        return owner_prep_read
    tensor, dims = map_arg(node.args, lambda arg: _env_arg(ctx, arg))
    assert isinstance(tensor, ast.AST)
    # pyrefly: ignore [not-iterable]
    dims = [*dims]
