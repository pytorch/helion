...
def _pallas_mask_dot_operands(
    ...
    """Zero-pad matmul operands along K when the reduction block doesn't divide K.

    When BLOCK_K doesn't evenly divide K, the last tile reads OOB.  This
    wraps both operands with ``jnp.where(mask, operand, 0.0)`` so OOB
    elements contribute zero to the dot product accumulator.

    The mask variable is looked up from two sources:
    1. ``GenerateAST.mask_var()`` for the default loop type
    2. ``DeviceFunction.mask_var_cache`` for fori_loop/emit_pipeline where
       the mask is generated by ``_setup_inner_loop_masks`` but not tracked
       in ``active_device_loops``
    """
    from .compile_environment import CompileEnvironment
    from .generate_ast import GenerateAST
    from .generate_ast import expr_from_string
    ...
    # Try to find the mask variable.  For default loop type, GenerateAST
    # tracks it in active_device_loops.  For fori_loop/emit_pipeline,
    # _setup_inner_loop_masks creates it but doesn't register it there.
    k_mask_var: str | None = None
    cg = ctx.cg
    if isinstance(cg, GenerateAST):
        k_mask_var = cg.mask_var(k_block_id)

    # Fallback: check if the mask variable was created by _setup_inner_loop_masks
    # for fori_loop/emit_pipeline inner loops.
    if k_mask_var is None and isinstance(cg, GenerateAST):
        candidate = f"mask_{k_block_id}"
        if candidate in cg.device_function.namespace._used_names:
            k_mask_var = candidate

    if k_mask_var is None:
        return lhs, rhs

    # Build broadcast suffix for mask along K dimension.
    # mask is 1D [BLOCK_K].  For lhs [..., M, K]: K is last dim.
    # For rhs [..., K, N]: K is second-to-last dim.
    lhs_sizes = [*lhs_val.size()]
    rhs_val = rhs_node.meta["val"]
    rhs_sizes = [*rhs_val.size()]

    lhs_k_dim = len(lhs_sizes) - 1
    rhs_k_dim = len(rhs_sizes) - 2

    # Build explicit reshape for the mask to match operand ndim.
    # Direct indexing like mask[None, :, None] causes Mosaic shape_cast errors.
    lhs_ndim = len(lhs_sizes)
    rhs_ndim = len(rhs_sizes)
    block_size_var = cg.device_function.block_size_var(k_block_id)

    # For lhs [..., M, K]: K is last dim → mask shape is (1, ..., 1, BLOCK_K)
    lhs_mask_shape = ", ".join(
        block_size_var if d == lhs_k_dim else "1" for d in range(lhs_ndim)
    )
    # For rhs [..., K, N]: K is second-to-last → mask shape is (1, ..., BLOCK_K, 1)
    rhs_mask_shape = ", ".join(
        block_size_var if d == rhs_k_dim else "1" for d in range(rhs_ndim)
    )

    # Cast bool mask to float32 BEFORE reshape to avoid Mosaic's
    # "unsupported shape cast" error for boolean vectors.
    # Use lax.select (element-wise, same-shape required) with broadcast
    # via float multiplication: nan_to_num ensures OOB NaN * 0 = 0.
    lhs = expr_from_string(
        f"jnp.nan_to_num({{lhs}}) * jnp.reshape({k_mask_var}.astype(jnp.float32), ({lhs_mask_shape})).astype({{lhs}}.dtype)",
        lhs=lhs,
    )
    rhs = expr_from_string(
        f"jnp.nan_to_num({{rhs}}) * jnp.reshape({k_mask_var}.astype(jnp.float32), ({rhs_mask_shape})).astype({{rhs}}.dtype)",
        rhs=rhs,
    )
    return lhs, rhs
...
def _pallas_dot(ctx: LoweringContext, node: Node, with_acc: bool) -> ast.AST:
    ...
    When the reduction dimension (K) has a non-divisible block size, both
    operands are zero-padded via ``jnp.where(mask, operand, 0)`` so that
    OOB elements contribute nothing to the dot product.
    """
    if with_acc:
    ...
    # Zero-pad matmul operands along K when the reduction tile doesn't
    # evenly divide K.  Without this, fori_loop and emit_pipeline read
    # OOB garbage on the last tile; default loop crashes with OOB slice.
    lhs, rhs = _pallas_mask_dot_operands(ctx, lhs, rhs, lhs_node_arg, rhs_node_arg)
