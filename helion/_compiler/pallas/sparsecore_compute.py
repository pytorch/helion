...
def packed_bf16_segments(
    ...
    if (
        ...
        and not isinstance(plan, CachedLoadPlan)
        and plan.layout.logical_dtype is torch.bfloat16
    ):
        result = (plan.layout.value_size,)
    else:
    ...
    for plan in program.stores:
        ...
        while pending:
            ...
            if node in seen:
                continue
            seen.add(node)
            if isinstance(program.plan_by_node.get(node), CachedLoadPlan):
                result.add(node)
...
class SparseCoreCompute:
    ...
    def load_expr(
        ...
        if isinstance(plan, CachedLoadPlan):
            return self._cached_expr(plan.access.tensor, buffer, chunk, part)
    ...
    def emit_body(self, indent: str, *, store_nodes: set[torch.fx.Node]) -> list[str]:
        ...
        self.indent = indent
        if any(
            plan.layout.logical_dtype is torch.bfloat16
