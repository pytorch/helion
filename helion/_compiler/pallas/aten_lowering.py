...
def codegen_topk_pallas(ctx: LoweringContext, node: Node) -> object:
    """``torch.topk(x, k, dim=-1, largest=True, sorted=True)`` via ``jax.lax.top_k``.

    ``jax.lax.top_k`` returns the ``k`` largest values over the LAST axis in
    descending order plus their int32 indices -- matching
    ``torch.topk(largest=True, sorted=True)``. It only reduces the last axis, so k
    must be a static int (use ``hl.specialize(k)`` in the kernel).
    """
    tensor = map_arg(node.args[0], lambda arg: _env_arg(ctx, arg))
    assert isinstance(tensor, ast.AST)
    k = node.args[1]
    assert isinstance(k, int), f"pallas topk requires a static int k, got {type(k)!r}"
    ...
    ctx.cg.add_statement(
        statement_from_string(f"{result} = jax.lax.top_k({{t}}, {k})", t=tensor)
    )
