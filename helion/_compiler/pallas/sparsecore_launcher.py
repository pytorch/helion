...
def _allocation_kind(node: torch.fx.Node, tensor: torch.Tensor) -> str:
    from ...language._tracing_ops import _host_tensor
    from ..compile_environment import CompileEnvironment
    from ..host_function import HostFunction

    source = node.args[0]
    if not isinstance(source, torch.fx.Node) or source.target is not _host_tensor:
        return "preserved"
    name = source.args[0]
    if not isinstance(name, str):
        return "preserved"
    if any(
        candidate.untyped_storage() is tensor.untyped_storage()
        for candidate in CompileEnvironment.current().input_sources
    ):
        return "preserved"
    allocations = {
        statement.targets[0].id: statement.value.func.attr
    ...
    if allocation in ("zeros", "zeros_like", "new_zeros"):
        return "zeros"
    if allocation in ("empty", "empty_like", "new_empty"):
        return "empty"
    return "preserved"
...
def build_sparsecore_launcher_spec(
    ...
    for plan in program.stores:
        ...
        if isinstance(plan, DirectStorePlan):
            ...
            pad_outputs.append(
                [
                    position,
                    [geometry.num_cores, shared_acc_items(logical[0]), *logical[1:]],
                ]
            )
            combine_outputs.append([position, logical])
    ...
    return {
        ...
        "num_subcores": geometry.num_subcores,
