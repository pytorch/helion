...
def _cute_vec_compatible_kernel() -> bool:
    """Compat wrapper preserved for callers that only need a boolean."""
    return _cute_vec_kernel_mode() == "vec"
...
class LoopedReductionStrategy(ReductionStrategy):
    ...
    def codegen_device_loop(self, state: CodegenState) -> DeviceLoopState:
        ...
        # Hoists added by the load dispatcher BEFORE the constexpr V-loop
        # (so a single vec load is reused across the V scalar uses inside
        # the loop).  Wired by ``_cute_vector_load_ctx`` when the
        # ``unroll`` mode picks the Uint16 bitcast fast-path.
        self._cute_lane_hoist_anchor: list[ast.AST] = []
