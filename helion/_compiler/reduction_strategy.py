...
from .tile_strategy import EmitPipelineScratchVar
...
class LoopedReductionStrategy(ReductionStrategy):
    ...
    def codegen_reduction(
        ...
        with install_inductor_kernel_handlers(state.codegen, {}):
            ...
            assert isinstance(device_loop, (DeviceLoopState, EmitPipelineLoopState))
            ...
            if isinstance(device_loop, EmitPipelineLoopState):
                # Register accumulator as a scratch variable for emit_pipeline.
                # The init and suffix are handled via pl.when guards in the
                # pipeline body rather than outer_prefix/outer_suffix.
                scratch_var = EmitPipelineScratchVar(
                    var_name=acc,
                    init_value=constant_repr(default),
                    init_dtype=acc_dtype,
                )
                device_loop.scratch_vars.append(scratch_var)
            else:
            ...
            if not backend.is_indexed_reduction(reduction_type):
                ...
                index_dtype = env.index_dtype
                if isinstance(device_loop, EmitPipelineLoopState):
                    scratch_var_idx = EmitPipelineScratchVar(
                        var_name=acc_index,
                        init_value=repr(torch.iinfo(index_dtype).max),
                        init_dtype=index_dtype,
                    )
                    device_loop.scratch_vars.append(scratch_var_idx)
                else:
                    device_loop.outer_prefix.append(
                        statement_from_string(
            ...
            suffix_stmt = statement_from_string(f"{result} = {expr}")
            if isinstance(device_loop, EmitPipelineLoopState):
                # Collect suffix statements into the first scratch var so
                # they can be emitted inside @pl.when(last iteration) guard.
                device_loop.scratch_vars[0].suffix_stmts.append(suffix_stmt)
                if env.settings.debug_dtype_asserts:
                    device_loop.scratch_vars[0].suffix_stmts.append(
                        statement_from_string(
                            f"tl.static_assert({result}.dtype == {_dtype_str(fake_output.dtype)})"
                        )
                    )
            else:
                device_loop.outer_suffix.append(suffix_stmt)
                if env.settings.debug_dtype_asserts:
