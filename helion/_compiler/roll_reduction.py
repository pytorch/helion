        ...
        if node.target in (_for_loop, _if):
            ...
            if info.used_rdim:
                if not info.can_be_rolled_by_caller:
                    ...
                    # For now, treat mixed reduction dim loops as outer graph nodes
                    # This allows matmul_layernorm example to work
                    return False
                return True
            return False
    ...
    def process(self, graph: torch.fx.Graph) -> torch.fx.Graph:
        ...
        for node in graph.nodes:
            if self.should_go_in_inner_graph(node):
                ...
                print(f"Processing node: {node}, target: {node.target}, args: {node.args}, kwargs: {node.kwargs}")
