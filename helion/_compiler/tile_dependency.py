...
def owner_roots_by_graph_id(device_ir: DeviceIR) -> tuple[tuple[int, ...], ...]:
    """Resolve every DeviceIR graph body to all owning top-level roots.

    A nested graph is reusable code rather than a unique execution point.  It
    may therefore be reached from several callsites or even several roots.
    Callsite identity remains in :class:`ExecutionScope`; this mapping only
    determines which root-level memory histories must contain each graph's
    accesses.
    """
