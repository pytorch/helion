...
def _patched_addmm(
    ...
    *,
    ...
    `torch.addmm(fp32, bf16, bf16) -> fp32` is a common usage pattern in Helion kernels,
    and it throws dtype mismatch error in unpatched torch.addmm op."""

    # Perform matmul in original dtype
    ...
    # Add the bias term, converting result to input's dtype if needed
...
def _patched_baddbmm(
    ...
    *,
    ...
    `torch.baddbmm(fp32, bf16, bf16) -> fp32` is a common usage pattern in Helion kernels.
    and it throws dtype mismatch error in unpatched torch.baddbmm op."""

    # Perform batch matmul in original dtype
...
def _patched_matmul(input: torch.Tensor, other: torch.Tensor) -> torch.Tensor:  # noqa: A002
    """Patched matmul that handles dtype mismatches and also FP8 support."""
    ...
    if input_is_e5m2 and other_is_e5m2 and input.dim() == 2 and other.dim() == 2:
        ...
        return torch.ops.helion.e5m2_matmul(input, other)  # pyright: ignore[reportCallIssue]
