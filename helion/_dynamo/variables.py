...
class HelionKernelVariable(VariableTracker):
    ...
    def _get_num_outputs(self) -> int:
        ...
        ann = self._kernel.signature.return_annotation
        ...
        if isinstance(ann, str):
            try:
                ann = typing.get_type_hints(
                    self._kernel.fn,
                    globalns=getattr(self._kernel.fn, "__globals__", {}),
    ...
    def _infer_output_spec(self, args: Sequence[VariableTracker]) -> dict[str, Any]:
        ...
        fake_args, num_outputs, num_tiled = (
            [self._get_example_value(a) for a in args],
            self._get_num_outputs(),
            None,
        )
        try:
                ...
                for name, vtype in (bound.host_function.local_types or {}).items():
                    if isinstance(vtype, TensorType) and name not in input_names:
                        ...
                        return {
                            ...
                            "dtype": t.dtype,
                            "device": str(t.device),
                            "num_tiled_dims": num_tiled or len(t.shape),
        ...
        for arg in fake_args:
            if isinstance(arg, torch.Tensor):
                return {
                    ...
                    "num_tiled_dims": num_tiled or len(arg.shape),
                    "num_outputs": num_outputs,
                }
