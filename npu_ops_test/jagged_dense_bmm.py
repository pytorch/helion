...
@helion.kernel(
    ...
    autotune_baseline_fn=jagged_dense_bmm_reference,
)
        ...
        for tile_len in hl.tile(0, max_seq_len):
            ...
            for tile_k in hl.tile(0, K):
                ...
                for tile_d in hl.tile(0, D):
                    ...
                    # Use multiply+sum instead of matmul: triton-ascend's unstructured
                    # addptr rewrite assumes single-use tt.addptr; matmul lowering can
                    # share pointer arithmetic across users and trip that assert.
                    acc = acc + torch.sum(
                        jagged_data.unsqueeze(-1) * dense_data.unsqueeze(1),
                        dim=2,
                    )
...
def random_input(
    ...
    lengths = torch.randint(max_seq_len + 1, size=(batch_size,), device=dev)
    seq_offsets = torch.zeros((batch_size + 1,), dtype=torch.int64, device=dev)
    seq_offsets[1:] = torch.cumsum(lengths, dim=0)
    jagged_size = int(seq_offsets[-1].item())
    jagged = (
        torch.empty((jagged_size, D), dtype=dtype, device=dev)
        .uniform_(-1.0, 1.0)
        .requires_grad_()
    )
    dense = (
        torch.empty((batch_size, D, K), dtype=dtype, device=dev)
        .uniform_(-1.0, 1.0)
        .requires_grad_()
    )
    bias = (
        torch.empty((batch_size, K), dtype=dtype, device=dev)
        .uniform_(-1.0, 1.0)
        .requires_grad_()
    )
