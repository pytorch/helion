...
def test(
    ...
    def reference_flex_attention(q, k, v, scale=None):
        ...
        scores = torch.matmul(q, k.transpose(-2, -1))  # [B, H, M, N]
        scores = scores * scale_factor

        if score_mod is not None:
            b_idx = torch.arange(B, device=q.device).view(B, 1, 1, 1).expand(B, H, M, N)
            h_idx = torch.arange(H, device=q.device).view(1, H, 1, 1).expand(B, H, M, N)
            m_idx = torch.arange(M, device=q.device).view(1, 1, M, 1).expand(B, H, M, N)
            n_idx = torch.arange(N, device=q.device).view(1, 1, 1, N).expand(B, H, M, N)
            scores = score_mod(scores, b_idx, h_idx, m_idx, n_idx)

        scores = scores * log_2_e
    ...
    def baseline_fn(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        ...
        return reference_flex_attention(q, k, v, scale)
